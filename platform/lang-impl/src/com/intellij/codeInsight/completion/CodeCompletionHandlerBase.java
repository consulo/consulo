...
public class CodeCompletionHandlerBase {
  ...
  public final void invokeCompletion(@NotNull final Project project,
  ...
  private LookupImpl obtainLookup(Editor editor, Project project) {
    ...
    LookupImpl lookup =
            (LookupImpl)LookupManager.getInstance(project).createLookup(editor, LookupElement.EMPTY_ARRAY, "", new LookupArranger.DefaultArranger());
  ...
  private void doComplete(CompletionInitializationContext initContext,
    ...
    CompletionContext context =
            createCompletionContext(hostCopy, hostMap.getOffset(CompletionInitializationContext.START_OFFSET), hostMap, initContext.getFile());
    ...
    final CompletionProgressIndicator indicator =
            new CompletionProgressIndicator(editor, initContext.getCaret(), parameters, this, freezeSemaphore, initContext.getOffsetMap(), hasModifiers,
                                            lookup);
  ...
  private AutoCompletionDecision shouldAutoComplete(final CompletionProgressIndicator indicator, List<LookupElement> items) {
    ...
    AutoCompletionContext context =
            new AutoCompletionContext(parameters, items.toArray(new LookupElement[items.size()]), indicator.getOffsetMap(), indicator.getLookup());
      ...
      if (decision == AutoCompletionDecision.SHOW_LOOKUP) {
        ...
        if (CompletionService.getCompletionService().getCurrentCompletion() == null &&
          ...
          CompletionServiceImpl
                  .setCompletionPhase(hasModifiers ? new CompletionPhase.InsertedSingleItem(indicator, restorePrefix) : CompletionPhase.NoCompletion);
        }
      }
  ...
  private static CompletionAssertions.WatchingInsertionContext insertItemHonorBlockSelection(final CompletionProgressIndicator indicator,
    ...
    if (editor.getCaretModel().supportsMultipleCarets()) {
      ...
      hostEditor.getCaretModel().runForEachCaret(new CaretAction() {
        ...
        public void perform(Caret caret) {
          ...
          CompletionAssertions.WatchingInsertionContext currentContext =
                  insertItem(indicator, item, completionChar, items, update, targetEditor, targetFile == null ? hostFile : targetFile, targetCaretOffset, idEnd,
                             injectedMap);
      ...
      context = insertItem(indicator, item, completionChar, items, update, editor, indicator.getParameters().getOriginalFile(), caretOffset, idEndOffset,
  ...
  private static CompletionAssertions.WatchingInsertionContext insertItem(final CompletionProgressIndicator indicator,
    ...
    final CompletionAssertions.WatchingInsertionContext context =
            new CompletionAssertions.WatchingInsertionContext(offsetMap, psiFile, completionChar, items, editor);
    ApplicationManager.getApplication().runWriteAction(() -> {
      ...
      assert context.getStartOffset() >= 0 : "stale startOffset: was " +
                                             initialStartOffset +
                                             "; selEnd=" +
                                             caretOffset +
                                             "; idEnd=" +
                                             ...
                                             "; file=" +
                                             context.getFile();
      assert context.getTailOffset() >= 0 : "stale tail: was " +
                                            initialStartOffset +
                                            "; selEnd=" +
                                            caretOffset +
                                            "; idEnd=" +
