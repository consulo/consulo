...
public class GeneratedParserUtilBase {
  ...
  public static void enterErrorRecordingSection(PsiBuilder builder_, int level, @NotNull String sectionType, @Nullable String frameName) {
    int modifiers =
      sectionType == _SECTION_GENERAL_ ? _NONE_ : sectionType == _SECTION_NOT_ ? _NOT_ : sectionType == _SECTION_AND_ ? _AND_ : _NONE_;
  ...
  private static boolean reportError(PsiBuilder builder_, ErrorState state, Frame frame, boolean force, boolean advance) {
    ...
    if (force || notEmpty || advance) {
      String gotText = builder_.eof()
                       ? "unexpected end of file"
                       : notEmpty ? "got '" + builder_.getTokenText() + "'" : "'" + builder_.getTokenText() + "' unexpected";
  ...
  public static boolean parseAsTree(ErrorState state,
                                    final PsiBuilder builder_,
                                    int level,
    ...
    if (checkParens) {
      ...
      LighterASTNode doneMarker =
        builder_.rawLookup(tokenIdx) == state.braces[0].getLeftBraceType() ? builder_.getLatestDoneMarker() : null;
      if (doneMarker != null &&
          doneMarker.getStartOffset() == builder_.rawTokenTypeStart(tokenIdx) &&
