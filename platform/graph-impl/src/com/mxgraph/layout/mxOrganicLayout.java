  ...
  protected void performRound() {
    ...
    for (int i = 0; i < v.length; i++) {
      ...
      for (int j = 0; j < triesPerCell; j++) {
        ...
        double energyDelta =
          calcEnergyDelta(index, oldNodeDistribution, oldEdgeDistance, oldEdgeCrossing, oldBorderLine, oldEdgeLength, oldAdditionFactors);
      ...
      for (int j = 0; j < e.length; j++) {
        ...
        if (j != i) {
          ...
          if (((iP1X != jP1X) && (iP1Y != jP1Y)) &&
            ...
            boolean intersects =
              ((Line2D.relativeCCW(iP1X, iP1Y, iP2X, iP2Y, jP1X, jP1Y) != Line2D.relativeCCW(iP1X, iP1Y, iP2X, iP2Y, jP2X, jP2Y)) &&
               (Line2D.relativeCCW(jP1X, jP1Y, jP2X, jP2Y, iP1X, iP1Y) != Line2D.relativeCCW(jP1X, jP1Y, jP2X, jP2Y, iP2X, iP2Y)));
