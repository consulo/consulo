...
public class ComboBoxWithWidePopup2 extends JComboBox {

  private boolean myLayingOut = false;
  private int myMinLength = 20;

  public ComboBoxWithWidePopup2(final ComboBoxModel aModel) {
    super(aModel);

    if (SystemInfo.isMac && UIUtil.isUnderAquaLookAndFeel()) setMaximumRowCount(25);
  }

  public ComboBoxWithWidePopup2(final Object[] items) {
    super(items);

    if (SystemInfo.isMac && UIUtil.isUnderAquaLookAndFeel()) setMaximumRowCount(25);
  }

  public ComboBoxWithWidePopup2(final Vector<?> items) {
    super(items);

    if (SystemInfo.isMac && UIUtil.isUnderAquaLookAndFeel()) setMaximumRowCount(25);
  }

  public ComboBoxWithWidePopup2() {
  }
  ...
  private class AdjustingListCellRenderer implements ListCellRenderer {
    ...
    private final ComboBoxWithWidePopup2 myComboBox;

    public AdjustingListCellRenderer(ComboBoxWithWidePopup2 comboBox, ListCellRenderer oldRenderer) {
      myComboBox = comboBox;
