...
/**
 * Headless {@link WindowManagerEx}: keeps a project-&gt;frame map and creates a headless frame, but
 * every geometry/focus/window method is a no-op returning null/empty.
 *
 * @author VISTALL
 */
@Singleton
@ServiceImpl
public class HeadlessWindowManagerImpl extends WindowManagerEx {
    private final Map<Project, HeadlessIdeFrame> myProject2Frame = new HashMap<>();
    ...
    public IdeFrameEx allocateFrame(Project project, @Nullable IdeFrameState state) {
        ...
        HeadlessIdeFrame frame = new HeadlessIdeFrame(project);
    ...
    public void releaseFrame(IdeFrameEx frame) {
        myProject2Frame.remove(frame.getProject());
    }
