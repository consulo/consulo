...
public class CodeCompletionHandlerBase {
  ...
  private void doComplete(CompletionInitializationContextImpl initContext, boolean hasModifiers, boolean isValidContext, long startingTime) {
    ...
    CompletionProgressIndicator indicator =
            new CompletionProgressIndicator(editor, initContext.getCaret(), initContext.getInvocationCount(), this, initContext.getOffsetMap(), initContext.getHostOffsets(), hasModifiers, lookup);
  ...
  private AutoCompletionDecision shouldAutoComplete(@Nonnull CompletionProgressIndicator indicator, @Nonnull List<LookupElement> items, @Nonnull CompletionParameters parameters) {
    if (!invokedExplicitly) {
  ...
  private static OffsetsInFile findInjectedOffsetsIfAny(@Nonnull Caret caret, boolean wasInjected, @Nonnull OffsetsInFile topLevelOffsets, @Nonnull Editor hostEditor) {
  ...
  @Nonnull
  private static WatchingInsertionContext createInsertionContext(@Nullable Lookup lookup,
                                                                 LookupElement item,
                                                                 char completionChar,
                                                                 Editor editor,
                                                                 PsiFile psiFile,
