...
class MultiThreadSearcher implements SESearcher {
   ...
   */
  MultiThreadSearcher(@Nonnull Listener listener, @Nonnull Executor notificationExecutor, @Nonnull Collection<? extends SEResultsEqualityProvider> equalityProviders) {
    myListener = listener;
  ...
  @Override
  public ProgressIndicator search(@Nonnull Map<? extends SearchEverywhereContributor<?>, Integer> contributorsAndLimits, @Nonnull String pattern) {
    ...
    if (!contributors.isEmpty()) {
      ...
      accumulator = new FullSearchResultsAccumulator(contributorsAndLimits, myEqualityProvider, myListener, myNotificationExecutor, indicatorWithCancelListener);
  ...
  private static Runnable createSearchTask(String pattern, ResultsAccumulator accumulator, ProgressIndicator indicator, SearchEverywhereContributor<?> contributor, Runnable finalCallback) {
    ...
    ContributorSearchTask<?> task = new ContributorSearchTask<>((SearchEverywhereContributor<Object>)contributor, pattern, accumulator, indicator, finalCallback);
  ...
  private static abstract class ResultsAccumulator {
    ...
    protected Map<SEResultsEqualityProvider.SEEqualElementsActionType, Collection<SearchEverywhereFoundElementInfo>> getActionsWithOtherElements(SearchEverywhereFoundElementInfo newElement) {
      Map<SEResultsEqualityProvider.SEEqualElementsActionType, Collection<SearchEverywhereFoundElementInfo>> res = new EnumMap<>(SEResultsEqualityProvider.SEEqualElementsActionType.class);
