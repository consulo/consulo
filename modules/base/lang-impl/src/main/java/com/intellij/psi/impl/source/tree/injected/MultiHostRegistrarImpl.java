...
public class MultiHostRegistrarImpl implements MultiHostRegistrar, ModificationTracker {
  ...
  public MultiHostRegistrarImpl(@Nonnull Project project, @Nonnull PsiFile hostPsiFile, @Nonnull PsiElement contextElement) {
    myProject = project;
  ...
  public MultiHostRegistrar addPlace(@NonNls @Nullable String prefix,
    ...
    shreds.add(new ShredImpl(manager.createSmartPsiFileRangePointer(myHostPsiFile, relevantRangeInHost, true),
                             manager.createSmartPsiElementPointer(host, myHostPsiFile), prefix, suffix, new ProperTextRange(startOffset, endOffset), false));
    return this;
  }
  ...
  public void doneInjecting() {
    try {
      ...
      VirtualFileWindowImpl virtualFile = new VirtualFileWindowImpl(myHostVirtualFile.getName(), myHostVirtualFile, documentWindow, myLanguage, outChars);
      ...
      synchronized (InjectedLanguageManagerImpl.ourInjectionPsiLock) {
        ...
        assert ((FileElement)parsedNode).textMatches(outChars) : exceptionContext(
                "Before patch: doc:\n'" + documentText + "'\n---PSI:\n'" + parsedNode.getText() + "'\n---chars:\n'" + outChars + "'");
        ...
        try {
          List<Trinity<IElementType, SmartPsiElementPointer<PsiLanguageInjectionHost>, TextRange>> tokens =
                  obtainHighlightTokensFromLexer(myLanguage, outChars, escapers, place, virtualFile, myProject);
  ...
  @Nonnull
  private static DocumentEx createDocument(@Nonnull LightVirtualFile virtualFile) {
  ...
  private static boolean cacheEverything(@Nonnull Place place,
                                         @Nonnull DocumentWindowImpl documentWindow,
                                         @Nonnull InjectedFileViewProvider viewProvider,
                                         @Nonnull PsiFile psiFile,
                                         @Nonnull SmartPsiElementPointer<PsiLanguageInjectionHost> pointer) {
  ...
  private static void patchLeaves(@Nonnull ASTNode parsedNode,
                                  @Nonnull List<LiteralTextEscaper<? extends PsiLanguageInjectionHost>> escapers,
  ...
  private static List<Trinity<IElementType, SmartPsiElementPointer<PsiLanguageInjectionHost>, TextRange>> obtainHighlightTokensFromLexer(Language language,
                                                                                                                                         ...
                                                                                                                                         Project project) {
    List<Trinity<IElementType, SmartPsiElementPointer<PsiLanguageInjectionHost>, TextRange>> tokens =
            new ArrayList<Trinity<IElementType, SmartPsiElementPointer<PsiLanguageInjectionHost>, TextRange>>(10);
  ...
  @Nonnull
  public static DocumentWindow freezeWindow(@Nonnull DocumentWindowImpl window) {
