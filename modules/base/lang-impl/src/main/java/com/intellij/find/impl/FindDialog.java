...
public class FindDialog extends DialogWrapper implements FindUI {
  ...
  private void revealWhitespaces(@Nonnull ComboBox comboBox) {
  ...
  private void initCombobox(@Nonnull final ComboBox comboBox) {
    ...
    if (!myHelper.getModel().isReplaceState()) {
      ...
      makeResultsPreviewActionOverride(comboBox, KeyStroke.getKeyStroke(KeyEvent.VK_PAGE_UP, 0), "scrollUp", () -> ScrollingUtil.movePageUp(myResultsPreviewTable));

      makeResultsPreviewActionOverride(comboBox, KeyStroke.getKeyStroke(KeyEvent.VK_PAGE_DOWN, 0), "scrollDown", () -> ScrollingUtil.movePageDown(myResultsPreviewTable));
  ...
  private boolean isResultsPreviewTabActive() {
    return myResultsPreviewTable != null && myContent.getSelectedIndex() == RESULTS_PREVIEW_TAB_INDEX;
  }
  ...
  private void handleComboBoxValueChanged(@Nonnull ComboBox comboBox) {
  ...
  private void handleAnyComboBoxValueChanged(@Nonnull ComboBox comboBox) {
  ...
  private void findSettingsChanged() {
    if (haveResultsPreview()) {
      ...
      GlobalSearchScope scope = GlobalSearchScopeUtil.toGlobalSearchScope(FindInProjectUtil.getScopeFromModel(myProject, findModel), myProject);
            ...
            return new Continuation(() -> {
              if (!isCancelled()) {
                ...
                myContent.setTitleAt(RESULTS_PREVIEW_TAB_INDEX, PREVIEW_TITLE +
                                                                ...
                                                                " in " +
                                                                (foundAllUsages ? Integer.valueOf(filesWithOccurrences) : filesWithOccurrences + "+") +
  ...
  private void validateFindButton() {
    boolean okStatus = myHelper.canSearchThisString() || myRbDirectory != null && myRbDirectory.isSelected() && StringUtil.isEmpty(getDirectory());
  ...
  private JPanel createResultsOptionPanel(JPanel optionsPanel, GridBagConstraints gbConstraints) {
    ...
    myScopePanel = new HideableTitledPanel(FindBundle.message("results.options.group"), resultsOptionPanel, myPreviousResultsExpandedState);
  ...
  private JComponent createFilterPanel() {
    ...
    filterPanel.setBorder(IdeBorderFactory.createTitledBorder(FindBundle.message("find.filter.file.name.group"), true));
  ...
  public static void initFileFilter(@Nonnull final JComboBox fileFilter, @Nonnull final JCheckBox useFileFilter) {
  ...
  public void doHelpAction() {
    FindModel myModel = myHelper.getModel();
    String id = myModel.isReplaceState() ? myModel.isMultipleFiles() ? HelpID.REPLACE_IN_PATH : HelpID.REPLACE_OPTIONS : myModel.isMultipleFiles() ? HelpID.FIND_IN_PATH : HelpID.FIND_OPTIONS;
  ...
  private JPanel createFindOptionsPanel() {
    ...
    mySearchContext = new ComboBox(
            new Object[]{getPresentableName(FindModel.SearchContext.ANY), getPresentableName(FindModel.SearchContext.IN_COMMENTS), getPresentableName(FindModel.SearchContext.IN_STRING_LITERALS),
                    getPresentableName(FindModel.SearchContext.EXCEPT_COMMENTS), getPresentableName(FindModel.SearchContext.EXCEPT_STRING_LITERALS),
  ...
  private void updateFileTypeForEditorComponent(@Nonnull ComboBox inputComboBox) {
  ...
  @Nonnull
  private static StateRestoringCheckBox createCheckbox(@Nonnull String message) {
  ...
  @Nonnull
  private static StateRestoringCheckBox createCheckbox(boolean selected, @Nonnull String message) {
  ...
  private static void setStringsToComboBox(@Nonnull String[] strings, @Nonnull ComboBox<String> combo, String selected) {
  ...
  private void setDirectories(@Nonnull List<String> strings, String directoryName) {
  ...
  protected void registerNavigateToSourceShortcutOnComponent(@Nonnull final JBTable c, JComponent component) {
