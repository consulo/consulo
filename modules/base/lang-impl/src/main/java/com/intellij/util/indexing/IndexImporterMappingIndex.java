...
final class IndexImporterMappingIndex<Key, Value, Input> implements SnapshotInputMappingIndex<Key, Value, Input> {
  ...
  @Nullable
  private static <Key, Value, Input> SnapshotInputMappingIndex<Key, Value, Input> createImportersWrapper(@Nonnull IndexExtension<Key, Value, Input> indexExtension) {
    List<SnapshotInputMappingIndex<Key, Value, Input>> importers;
    try {
      importers = IndexImporterFactory.EP_NAME.getExtensionList().stream().map(f -> f.createImporter(indexExtension)).filter(Objects::nonNull).collect(Collectors.toList());
    }
    catch (Exception e) {
      LOG.error(e);
