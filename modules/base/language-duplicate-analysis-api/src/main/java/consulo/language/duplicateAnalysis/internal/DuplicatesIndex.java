...
public class DuplicatesIndex extends FileBasedIndexExtension<Integer, IntList> {
    public static boolean ourEnabled = SystemProperties.getBooleanProperty("idea.enable.duplicates.online.calculation", true);
    public static final boolean ourEnabledLightProfiles = true;
    ...
    private static class MyFragmentsCollector implements FragmentsCollector {
        ...
        public void add(int hash, int cost, @Nullable PsiFragment frag) {
            ...
            if (list == null) {
                myMap.put(hash, list = IntLists.newArrayList());
            }
    ...
    public static boolean isIndexedFragment(@Nullable PsiFragment frag, int cost, DuplicatesProfile profile, DuplocatorState duplocatorState) {
        if (frag == null) {
            return false;
        }
