...
public class DuplocatorUtil {
  ...
  public static boolean shouldSkip(PsiElement element, PsiElement elementToMatchWith) {
    ...
    if (element.getFirstChild() == null && element.getTextLength() == 0 && !(element instanceof LeafPsiElement)) {
      return true;
    }
  ...
  public static PsiElement skipNodeIfNeccessary(PsiElement element, EquivalenceDescriptor descriptor, NodeFilter filter) {
    ...
    if (descriptor != null) {
      PsiElement onlyChild = getOnlyChildFromDescriptor(descriptor, filter);
  ...
  private static PsiElement getOnlyChildFromDescriptor(EquivalenceDescriptor equivalenceDescriptor, NodeFilter filter) {
    ...
    if (!singleChildren.isEmpty()) {
      ...
      PsiElement[] codeBlock = codeBlocks.get(0);
  ...
  public static boolean match(@Nonnull EquivalenceDescriptor descriptor1,
    ...
    for (int i = 0, n = descriptor1.getCodeBlocks().size(); i < n; i++) {
      ...
      PsiElement[] codeBlock2 = descriptor2.getCodeBlocks().get(i);
  ...
  private static boolean match(@Nonnull SingleChildDescriptor childDescriptor1,
    ...
    if (duplicatesProfile != null) {
      ...
      PsiElementRole role2 = element2 != null ? duplicatesProfile.getRole(element2) : null;
