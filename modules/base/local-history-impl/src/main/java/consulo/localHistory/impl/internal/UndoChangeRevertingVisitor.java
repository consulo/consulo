...
public class UndoChangeRevertingVisitor extends ChangeVisitor {
    ...
    private void unregisterDelayedApplies(VirtualFile fileOrDir) {
        ...
        for (DelayedApply a : myDelayedApplies) {
            if (VirtualFileUtil.isAncestor(fileOrDir, a.getFile(), false)) {
                toRemove.add(a);
            }
        }
    ...
    private static class DelayedContentApply extends DelayedApply {
        ...
        public void apply() throws IOException {
            ...
            UndoManagerInternal undoManagerInternal = (UndoManagerInternal) ApplicationUndoManager.getInstance();

            try (var ignored = undoManagerInternal.underDocumentUndo(doc)) {
