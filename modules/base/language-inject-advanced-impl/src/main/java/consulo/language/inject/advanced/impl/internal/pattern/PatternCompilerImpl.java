...
package consulo.language.inject.advanced.impl.internal.pattern;
...
public class PatternCompilerImpl<T> implements PatternCompiler<T> {
    ...
    public synchronized ElementPattern<T> compileElementPattern(String text) {
        Node node = processElementPatternText(text, (Function<Frame, Object>) frame -> {
            ...
            return new Node(
                (Node) frame.target,
                myStringInterner.intern(frame.methodName),
                args.length == 0 ? ArrayUtil.EMPTY_OBJECT_ARRAY : args
            );
        });
    ...
    private static Set<Method> getStaticMethods(List<Class> patternClasses) {
        return new HashSet<>(ContainerUtil.concat(
            patternClasses,
            aClass -> ContainerUtil.findAll(aClass.getMethods(), method -> Modifier.isStatic(method.getModifiers()) &&
        ...
        while (true) {
            ...
            switch (curFrame.state) {
                ...
                case escape:
                    ...
                    break;
                case invoke:
                    curResult = executor.apply(curFrame);
                    if (ch == 0 && stack.isEmpty()) {
                        ...
                        throwError(
                            curPos,
                            ch,
                            (stack.isEmpty() ? "'.' or <eof>" : "'.' or ')'") + "expected after '" + curFrame.methodName + "' call"
                        );
                    }
                    break;
                case invoke_end:
    ...
    private static @Nullable Method findMethod(String methodName, Object[] arguments, Collection<Method> methods, Ref<Boolean> convertVarArgs) {
        ...
        for (Method method : methods) {
            ...
            for (int i = 0, parameterTypesLength = parameterTypes.length; i < arguments.length; i++) {
                Class<?> type =
                    getNonPrimitiveType(i < parameterTypesLength ? parameterTypes[i] : parameterTypes[parameterTypesLength - 1]);
    ...
    private static void printClass(Class aClass, Map<Class, Collection<Class>> classes, StringBuilder sb) {
        ...
        Class rawSuperClass =
            (Class) (superClass instanceof ParameterizedType ? ((ParameterizedType) superClass).getRawType() : superClass);
    ...
    private static void dumpTypeParametersArray(
        GenericDeclaration owner,
        Type[] typeVariables,
        StringBuilder sb,
        String prefix,
        String suffix,
    ...
    public class LazyPresentablePattern<T> implements CompilableElementPattern<T> {
        ...
        private StringBuilder toString(Node node, StringBuilder sb) {
            ...
            for (Object arg : node.args) {
                ...
                if (arg instanceof Node nodeArg) {
                    toString(nodeArg, sb);
                }
