...
public class GeneralCommandLine implements UserDataHolder {
  ...
  private void checkWorkingDirectory() throws ExecutionException {
    ...
    if (!myWorkDirectory.exists()) {
      throw new ExecutionException(ProcessBundle.message("run.configuration.error.working.directory.does.not.exist", myWorkDirectory.getAbsolutePath()));
    }
  ...
  private static class MyStrictMap extends DelegateMap<String, String> {
    public MyStrictMap() {
      super(Maps.newHashMap(SystemInfo.isWindows ? HashingStrategy.caseInsensitive() : ContainerUtil.<String>canonicalStrategy()));
    }
