...
class MyersLCS {
  ...
  public void executeWithThreshold() throws FilesTooBigForDiffException {
    int threshold = Math.max(20000 + 10 * (int)Math.sqrt(myCount1 + myCount2), FilesTooBigForDiffException.DELTA_THRESHOLD_SIZE);
    ...
    if (oldStart < oldEnd && newStart < newEnd) {
      ...
      for (int d = 0; d <= halfD; ++d) {
        ...
        for (int k = L; k <= R; k += 2) {
          ...
          int y = x - k + newLength;
          x += commonSubsequenceLengthForward(oldStart + x, newStart + y, Math.min(oldEnd - oldStart - x, newEnd - newStart - y));
          VForward[k] = x;
        }
        ...
        for (int k = L; k <= R; k += 2) {
          ...
          int y = x - k + newLength;
          x += commonSubsequenceLengthBackward(oldEnd - 1 - x, newEnd - 1 - y, Math.min(oldEnd - oldStart - x, newEnd - newStart - y));
          VBackward[k] = x;
        }
