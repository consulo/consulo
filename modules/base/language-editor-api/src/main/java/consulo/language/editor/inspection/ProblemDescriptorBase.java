...
public class ProblemDescriptorBase extends CommonProblemDescriptorBase implements ProblemDescriptor {
    ...
    public ProblemDescriptorBase(
        ...
        LOG.assertTrue(
            startElement == endElement || endContainingFile != null && endContainingFile.isValid() || endElement.isValid(),
            endElement
        );
    ...
    protected void assertPhysical(PsiElement element) {
        if (!element.isPhysical()) {
            LOG.error(
                "Non-physical PsiElement. Physical element is required to be able to anchor the problem " +
                "in the source tree: " + element + "; file: " + element.getContainingFile()
            );
        }
    }
  ...
  @RequiredReadAction
    ...
    public String toString() {
        return ProblemDescriptorUtil.renderDescriptionMessage(this).get();
    }
}
