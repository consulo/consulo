...
public final class LiveTemplateBuilder {
    ...
    public Template buildTemplate() {
        ...
        variableOccurrences.sort(Comparator.comparingInt(occurrence -> occurrence.myOffset));
        ...
        for (int i = 0; i < template.getVariableCount(); i++) {
            ...
            if (!Template.INTERNAL_VARS_SET.contains(varName)) {
                ...
                Variable var = new Variable(
                    newVarName,
                    template.getExpressionStringAt(i),
                    template.getDefaultValueStringAt(i),
                    template.isAlwaysStopAt(i)
                );
                myVariables.add(var);
