...
public class TargetElementUtil {
  ...
  public static PsiElement findTargetElement(@Nonnull Editor editor, @Nonnull Set<String> flags, int offset) {
    ...
    PsiElement target = TargetElementUtilExtender.EP.computeSafeIfAny(Application.get(), it -> it.modifyTargetElement(targetElement, flags));
    if (target != null) {
      return target;
    }
  ...
  private static PsiElement getReferenceOrReferencedElement(PsiFile file, Editor editor, Set<String> flags, int offset) {
    ...
    PsiElement psiElement = TargetElementUtilExtender.EP.computeSafeIfAny(Application.get(), it -> it.modifyReferenceOrReferencedElement(referenceOrReferencedElement, file, editor, flags, offset));
    if (psiElement != null) {
      return psiElement;
    }
    return referenceOrReferencedElement;
  ...
  private static PsiElement getReferenceOrReferencedElementImpl(PsiFile file, Editor editor, Set<String> flags, int offset) {
    ...
    PsiElement referenceOrReferencedElement = TargetElementUtilExtender.EP.computeSafeIfAny(Application.get(), it -> it.getReferenceOrReferencedElement(ref, flags));
    if (referenceOrReferencedElement != null) {
      return referenceOrReferencedElement;
    }
  ...
  public static Collection<PsiElement> getTargetCandidates(PsiReference reference) {
    ...
    Collection<PsiElement> targetCandidates = TargetElementUtilExtender.EP.computeSafeIfAny(Application.get(), it -> it.getTargetCandidates(reference));
