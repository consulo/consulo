...
public class ExternalStoragePluginManager implements PluginActionListener {
    ...
    public boolean updatePlugins(@Nonnull ProgressIndicator indicator) {
        try {
            ...
            StoragePlugin[] beans =
                WebServiceApiSender.doPost(WebServiceApi.STORAGE_API, "/plugins/merge", inPlugins, StoragePlugin[].class);
            ...
            if (wantAnyInstall) {
                List<PluginDescriptor> repositoryPlugins = RepositoryHelper.loadOnlyPluginsFromRepository(
                    indicator,
                    UpdateSettings.getInstance().getChannel(),
                    EarlyAccessProgramManager.getInstance()
                );

                Map<PluginId, PluginDescriptor> repo =
                    ContainerUtil.newMapFromValues(repositoryPlugins.iterator(), PluginDescriptor::getPluginId);
                ...
                for (Map.Entry<PluginId, PluginActionInfo> entry : pluginActions.entrySet()) {
                    ...
                    Set<PluginDescriptor> pluginsForInstall =
                        PluginInstallUtil.getPluginsForInstall(List.of(repositoryPlugin), repositoryPlugins);
