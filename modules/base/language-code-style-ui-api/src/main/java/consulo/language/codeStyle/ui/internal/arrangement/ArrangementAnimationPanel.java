 ...
 *
 * FIXME [VISTALL] we need this animation? 
 */
public class ArrangementAnimationPanel extends JPanel {
  ...
  private void prepareForAnimation() {
    ...
    myAnimationIterationStep = Math.max((myHorizontal ? size.width : size.height) / (expectedDurationMillis / ArrangementConstants.ANIMATION_STEPS_TIME_GAP_MILLIS), 1);
    ...
    if (myExpand) {
      ...
      if (myHorizontal) {
        myCurrentImage = myImage.getSubimage(0, 0, myImage.getWidth() - myAnimationIterationStep, myImage.getHeight());
      }
      else {
        myCurrentImage = myImage.getSubimage(0, 0, myImage.getWidth(), myImage.getHeight() - myAnimationIterationStep);
      }
    }
    invalidate();
  }
