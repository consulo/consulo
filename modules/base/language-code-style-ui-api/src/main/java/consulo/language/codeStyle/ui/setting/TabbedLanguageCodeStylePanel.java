...
public abstract class TabbedLanguageCodeStylePanel extends CodeStyleAbstractPanel {
    ...
    private void fillPredefined(JMenuItem parentMenu) {
        for (PredefinedCodeStyle predefinedCodeStyle : myPredefinedCodeStyles) {
            JMenuItem predefinedItem = new JBMenuItem(predefinedCodeStyle.getName());
            parentMenu.add(predefinedItem);
            predefinedItem.addActionListener(e -> applyPredefinedStyle(predefinedCodeStyle.getName()));
        }
    }
    ...
    private PredefinedCodeStyle[] getPredefinedStyles() {
        ...
        return Application.get().getExtensionPoint(PredefinedCodeStyle.class)
            .collectFiltered(codeStyle -> codeStyle.getLanguage().equals(language))
            .toArray(PredefinedCodeStyle.EMPTY_ARRAY);
    }
...
//========================================================================================================================================
    ...
    protected class MyIndentOptionsWrapper extends CodeStyleAbstractPanel {
        ...
        @RequiredUIAccess
        protected MyIndentOptionsWrapper(
            ...
            if (myEditor != null) {
                ...
                scroll.setPreferredSize(new Dimension(
                    panel.getPreferredSize().width + scroll.getVerticalScrollBar().getPreferredSize().width + 5,
                    -1
                ));
        ...
        public boolean isModified(CodeStyleSettings settings) {
            CommonCodeStyleSettings.IndentOptions indentOptions = getIndentOptions(settings);
            return indentOptions != null && myEditor.isModified(settings, indentOptions);
        }
