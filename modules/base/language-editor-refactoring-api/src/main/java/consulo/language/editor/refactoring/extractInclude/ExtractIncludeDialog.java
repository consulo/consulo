...
public class ExtractIncludeDialog extends DialogWrapper {
    ...
    private void validateOKButton() {
        String fileName = myNameField.getText().trim();
        setOKActionEnabled(
    ...
    protected void doOKAction() {
        Project project = myCurrentDirectory.getProject();
        ...
        FileType type = FileTypeRegistry.getInstance().getKnownFileTypeOrAssociate(targetFileName);
        if (type == null) {
            return;
        }
        ...
        CommandProcessor.getInstance().newCommand()
            ...
            .run(() -> {
                try {
                    ...
                    String webPath = PsiFileSystemItemUtil.getRelativePath(myCurrentDirectory, targetDirectory);
                    myTargetDirectory = webPath == null ? null : targetDirectory;
                }
                catch (IncorrectOperationException e) {
                    CommonRefactoringUtil.showErrorMessage(REFACTORING_NAME, LocalizeValue.of(e), null, project);
                }
            });
