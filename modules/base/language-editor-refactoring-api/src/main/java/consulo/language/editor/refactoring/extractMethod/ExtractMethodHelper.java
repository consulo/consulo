...
public class ExtractMethodHelper {
    ...
    public static void replaceDuplicates(
        ...
        if (!duplicates.isEmpty()) {
            ...
            boolean isUnittest = Application.get().isUnitTestMode();
            Project project = callElement.getProject();
            int exitCode = !isUnittest
            ...
            if (exitCode == Messages.YES) {
                ...
                Map<SimpleMatch, RangeHighlighter> highlighterMap = new HashMap<>();
                for (SimpleMatch match : duplicates) {
                    if (!match.getStartElement().isValid() || !match.getEndElement().isValid()) {
                        continue;
                    }
                    Pair<SimpleMatch, PsiElement> replacement = Pair.create(match, callElement);
                    if (!replaceAll) {
                        ...
                        if (promptResult == FindManager.PromptResult.SKIP) {
                            ...
                            RangeHighlighter highlighter = highlighterMap.get(match);
                            if (highlighter != null) {
    ...
    private static void replaceDuplicate(
        ...
        Pair<SimpleMatch, PsiElement> replacement
    ) {
