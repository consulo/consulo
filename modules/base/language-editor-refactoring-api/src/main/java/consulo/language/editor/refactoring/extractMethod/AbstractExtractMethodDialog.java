...
import consulo.language.editor.refactoring.ui.SimpleParameterTablePanel;
...
public class AbstractExtractMethodDialog extends DialogWrapper implements ExtractMethodSettings {
    ...
    public AbstractExtractMethodDialog(Project project,
                                       ...
                                       ExtractMethodDecorator decorator,
                                       FileType type) {
        super(project, true);
        myProject = project;
    ...
    protected void doOKAction() {
        String error = myValidator.check(getMethodName());
        if (error != null) {
    ...
    private void updateOutputVariables() {
        ...
        for (String variable : myOutputVariables) {
            if (myVariablesMap != null) {
                AbstractVariableData data = myVariablesMap.get(variable);
                String outputName = data != null ? data.getName() : variable;
                if (first) {
                    first = false;
                }
                else {
