...
public class DefaultRemoteContentProvider extends RemoteContentProvider {
    ...
    private static void downloadContent(String url, File file, DownloadingCallback callback) {
        ...
        try {
            ...
            callback.setProgressText(VirtualFileSystemLocalize.downloadProgressConnecting(presentableUrl), true);
            ...
            if (responseCode != HttpURLConnection.HTTP_OK) {
                throw new IOException(HttpLocalize.errorConnectionFailedWithHttpCodeN(responseCode).get());
            }

            int size = connection.getContentLength();
            output = new BufferedOutputStream(new FileOutputStream(file));
            callback.setProgressText(VirtualFileSystemLocalize.downloadProgressDownloading(presentableUrl), size == -1);
            if (size != -1) {
            ...
            LOG.info(e);
            callback.errorOccurred(VirtualFileSystemLocalize.cannotLoadRemoteFile(url, e.getMessage()), false);
        }
        finally {
            if (input != null) {
                try {
