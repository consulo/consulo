...
import consulo.language.codeStyle.impl.internal.formatting.FormatterImpl;
import consulo.language.codeStyle.internal.CoreCodeStyleUtil;
import consulo.language.codeStyle.internal.FormatterEx;
import consulo.language.codeStyle.internal.IndentOld;
...
public class CodeStyleManagerImpl extends CodeStyleManager implements FormattingModeAwareIndentAdjuster {
    ...
    @RequiredReadAction
    private static PsiElement postProcessElement(PsiFile file, PsiElement formatted) {
    ...
    private static void postProcessText(PsiFile file, TextRange textRange) {
    ...
    @RequiredReadAction
    private static void removeEndingWhiteSpaceFromEachRange(PsiFile file, FormatTextRanges ranges) {
    ...
    private int doAdjustLineIndentByOffset(PsiFile file, int offset, FormattingMode mode) {
    ...
    private static CodeStyleSettings getSettings(PsiFile file) {
    ...
    private static class CaretPositionKeeper {
        ...
        private void restoreVisualPosition() {
            if (myVisualColumnToRestore < 0) {
                EditorInternalUtil.runWithAnimationDisabled(myEditor, () -> myEditor.getScrollingModel().scrollToCaret(ScrollType.RELATIVE));
                return;
            }
    ...
    private static void postProcessEnabledRanges(PsiFile file, TextRange range, CodeStyleSettings settings) {
    ...
    void setCurrentFormattingMode(FormattingMode mode) {
