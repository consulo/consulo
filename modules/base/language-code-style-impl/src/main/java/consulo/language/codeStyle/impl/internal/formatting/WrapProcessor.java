...
public class WrapProcessor {
    ...
    private WrapImpl getWrapToBeUsed(ArrayList<WrapImpl> wraps, LeafBlockWrapper currentBlock) {
        ...
        if (myWrapCandidate == currentBlock) {
            return wraps.get(0);
        }
    ...
    public LeafBlockWrapper processWrap(LeafBlockWrapper currentBlock) {
        ...
        if (wrap != null || wrapIsPresent) {
            ...
            if (wrap != null && wrap.getChopStartBlock() != null) {
                // getWrapToBeUsed() returns the block only if it actually exceeds the right margin.
                // In this case, we need to go back to the first block that has the CHOP_IF_NEEDED wrap type
                // and start wrapping from there.
