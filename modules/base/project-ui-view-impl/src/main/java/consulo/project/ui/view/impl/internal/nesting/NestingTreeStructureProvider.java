...
public final class NestingTreeStructureProvider implements TreeStructureProvider, DumbAware {
    ...
    public @Nonnull Collection<AbstractTreeNode> modify(@Nonnull AbstractTreeNode parent,
        ...
        ArrayList<PsiFileNode> childNodes = new ArrayList<>();
        ...
        Function<PsiFileNode, String> fileNameFunc = psiFileNode -> {
            PsiFile file = psiFileNode.getValue();
            if (file == null) {
                return null;
            }
            return file.getName();
        ...
        for (AbstractTreeNode<?> node : children) {
            ...
            Collection<PsiFileNode> childrenOfThisFile = parentToChildren.get((PsiFileNode) node);
    ...
    public static Collection<ChildFileInfo> getFilesShownAsChildrenInProjectView(@Nonnull Project project,
        ...
        Collection<ProjectViewFileNestingService.NestingRule> rulesWhereItCanBeParent = filterRules(rules, parentFile.getName(), true);
        if (rulesWhereItCanBeParent.isEmpty()) {
            return Collections.emptyList();
        }
        Collection<ProjectViewFileNestingService.NestingRule> rulesWhereItCanBeChild = filterRules(rules, parentFile.getName(), false);
        ...
        SmartList<ChildFileInfo> result = new SmartList<>();
        ...
        for (VirtualFile child : children) {
            ...
            for (ProjectViewFileNestingService.NestingRule rule : rulesWhereItCanBeChild) {
                ...
                Couple<Boolean> c = FileNestingBuilder.checkMatchingAsParentOrChild(rule, childName);
                boolean matchesParent = c.first;

                if (matchesParent) {
                    String baseName = childName.substring(0, childName.length() - rule.getParentFileSuffix().length());
            ...
            for (ProjectViewFileNestingService.NestingRule rule : rulesWhereItCanBeParent) {
                ...
                Couple<Boolean> c = FileNestingBuilder.checkMatchingAsParentOrChild(rule, childName);
                boolean matchesChild = c.second;

                if (matchesChild) {
                    String baseName = childName.substring(0, childName.length() - rule.getChildFileSuffix().length());
