...
public abstract class PackageSetBase implements PackageSet {
  ...
  public boolean contains(@Nonnull PsiFile file, NamedScopesHolder holder) {
    PluginExceptionUtil.reportDeprecatedDefault(getClass(), "contains(VirtualFile, Project, NamedScopesHolder)", "Need proper implementation");
    return contains(file.getVirtualFile(), file.getProject(), holder);
  }

  /**
   * @see PackageSetBase#getPsiFile(VirtualFile, NamedScopesHolder)
   */
  @Deprecated
  @Nullable
