...
public class PrattBuilderImpl extends PrattBuilder {
    ...
    private LocalizeValue myExpectedMessage = LocalizeValue.empty();
    ...
    @Override
    public PrattBuilder expecting(LocalizeValue expectedMessage) {
    ...
    protected void doParse() {
        if (isEof()) {
            error(myExpectedMessage.orIfEmpty(LanguageLocalize.unexpectedEof()));
            return;
        }

        TokenParser parser = findParser();
        if (parser == null) {
            error(myExpectedMessage.orIfEmpty(LanguageLocalize.unexpectedToken()));
            return;
        }
    ...
    @Override
    public void error(LocalizeValue errorText) {
