...
public class MultiHostInjectorExtensionPoint extends AbstractExtensionPointBean {
  ...
  private final Supplier<Class<PsiElement>> myForClassHandler = LazyValue.notNull(() -> {
  ...
  private final Supplier<Class<MultiHostInjector>> myImplementationClassHandler = LazyValue.notNull(() -> {
  ...
  public Class<PsiElement> getKey() {
    return myForClassHandler.get();
  }

  @Nonnull
  public MultiHostInjector getInstance(@Nonnull ComponentManager componentManager) {
    if (myInstance == null) {
      myInstance = AbstractExtensionPointBean.instantiate(myImplementationClassHandler.get(), componentManager.getInjectingContainer());
    }
    return myInstance;
  }
}
