...
public class SemVerTest {
  ...
  private static void assertParsed(@Nonnull String version, int expectedMajor, int expectedMinor, int expectedPatch, @Nullable String expectedPreRelease) {
    Assert.assertEquals(new SemVer(version, expectedMajor, expectedMinor, expectedPatch, expectedPreRelease), parse(version));
  }

  private static void assertNotParsed(String version) {
    Assert.assertNull(SemVer.parseFromText(version));
  }

  private static SemVer parse(String text) {
    SemVer semVer = SemVer.parseFromText(text);
    Assert.assertNotNull(text, semVer);
    return semVer;
  }
}
