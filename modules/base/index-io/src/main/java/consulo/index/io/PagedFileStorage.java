...
public class PagedFileStorage implements Forceable {
  private static final Logger LOG = LoggerFactory.getLogger(PagedFileStorage.class);
  ...
  public static class StorageLock {
    ...
    private final ConcurrentIntObjectMap<PagedFileStorage> myIndex2Storage = IntMaps.newConcurrentIntObjectHashMap();
      ...
      while (true) {
        try {
          ...
          if (newSize < 0) {
            ...
            throw new MappingFailedException("Cannot recover from OOME in memory mapping: -Xmx=" +
