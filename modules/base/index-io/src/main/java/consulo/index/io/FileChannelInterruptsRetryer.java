...
public final class FileChannelInterruptsRetryer implements AutoCloseable {
    private static final Logger LOG = LoggerFactory.getLogger(FileChannelInterruptsRetryer.class);
     ...
     */
    private static final int LOG_STACKTRACE_IF_RETRY_CHAIN_LONGER =
        SystemProperties.getIntProperty("idea.vfs.LOG_STACKTRACE_IF_RETRY_CHAIN_LONGER", 32);

    private final Lock myOpenCloseLock = new ReentrantLock();
     ...
     */
    private volatile @Nullable FileChannel myChannel;

    /**
     * Total number of retries across all instances of this class. Only retries count -- a first
     * successful attempt doesn't.
     */
    private static final AtomicLong ourTotalRetriedAttempts = new AtomicLong();

    public static long totalRetriedAttempts() {
        return ourTotalRetriedAttempts.get();
    }


    public FileChannelInterruptsRetryer(Path path,
    ...
    public <T> T retryIfInterrupted(FileChannelIdempotentOperation<T> operation) throws IOException {
        boolean interruptedStatusWasCleared = false;
        try {
            for (int attempt = 0; ; attempt++) {
                FileChannel channelLocalCopy = myChannel;
                ...
                try {
                    ...
                    ourTotalRetriedAttempts.incrementAndGet();

                    if (attempt >= MAX_RETRIES) {
                        IOException ioe = new IOException(
                            "Channel[" + myPath + "][@" + System.identityHashCode(channelLocalCopy) + "] " +
                    ...
                    //         (See @Disabled test ResilientFileChannel_MultiThreaded_Test.onceClosed_FileChannelInterruptsRetryer_RemainsClosedForever
                    //         which fails now)
                    ...
                    if (logStackTrace) {
                        LOG.warn("Channel[" + myPath + "][@" + System.identityHashCode(channelLocalCopy) + "] is closed during " + operation
    ...
    public void close() throws IOException {
        ConcurrencyUtil.withLock(myOpenCloseLock, this::tryClose);
    }
    ...
    private void reopenChannel() throws IOException {
        ConcurrencyUtil.withLock(myOpenCloseLock, () -> {
            try {
                ...
                LOG.info("Can't close channel[" + myPath + "]: " + e.getMessage());
            }
            myChannel = FileChannel.open(myPath, myOpenOptions);
        });
    }
