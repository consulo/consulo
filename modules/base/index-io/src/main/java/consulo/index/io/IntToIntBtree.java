...
public class IntToIntBtree {
  ...
  private IntIntMap myCachedMappings;
  ...
  public IntToIntBtree(int pageSize, @Nonnull File file, @Nonnull PagedFileStorage.StorageLockContext storageLockContext, boolean initial) throws IOException {
    ...
    if (hasCachedMappings) {
      myCachedMappings = IntMaps.newIntIntHashMap(myCachedMappingsSize = 4 * maxLeafNodes);
    }
    else {
  ...
  public boolean get(int key, @Nonnull int[] result) {
    ...
    if (hasCachedMappings) {
      if (myCachedMappings.containsKey(key)) {
        result[0] = myCachedMappings.getInt(key);
        return true;
      }
    }
  ...
  public void put(int key, int value) {
    ...
    if (hasCachedMappings) {
      myCachedMappings.putInt(key, value);
  ...
  private void flushCachedMappings() {
    if (hasCachedMappings) {
      ...
      Arrays.sort(keys);
      for (int key : keys) doPut(key, myCachedMappings.getInt(key));
  ...
  private static class BtreeIndexNodeView extends BtreePage {
      ...
      if (indexLeaf && hashedLeaf) {
        ...
        if (defaultSplit) {
          ...
          for (int i = 0; i < avg; ++i) {
            ...
            key = keys[avg + i];
            newIndexNode.insert(key, map.getInt(key));
          }
    ...
    private void insertToRightSiblingWhenHashed(BtreeIndexNodeView parent, HashLeafData hashLeafData, int indexInParent, BtreeIndexNodeView sibling) {
