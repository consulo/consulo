  ...
  public final void unmap() {
    ...
    if (myBuffer != null) {
      ...
      if (!UnsafeDelegate.get().invokeCleaner(myBuffer)) {
        LoggerFactory.getLogger(MappedBufferWrapper.class).error("Unmapping failed for: " + myFile);
      }
      myBuffer = null;
    }
    ...
    if (buffer != null && isDirty()) {
      for (int i = 0; i < MAX_FORCE_ATTEMPTS; i++) {
        try {
          ...
          LoggerFactory.getLogger(MappedBufferWrapper.class).info(e.getMessage(), e);
