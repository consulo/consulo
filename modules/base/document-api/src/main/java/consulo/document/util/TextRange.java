...
public class TextRange implements Segment, Serializable {
    ...
    public static ArrayFactory<TextRange> ARRAY_FACTORY = count -> count == 0 ? EMPTY_ARRAY : new TextRange[count];
    ...
    public @Nullable TextRange intersection(TextRange range) {
        if (!intersects(range)) {
            return null;
        }
