...
public final class IndexVersion {
    ...
    public static IndexVersionDiff versionDiffers(ID<?, ?> indexId, int currentIndexVersion) {
        ...
        long timestamp = ManagingFS.getInstance().getCreationTimestamp();
    ...
    public static synchronized void rewriteVersion(ID<?, ?> indexId, int version) throws IOException {
        Path file = IndexInfrastructure.getVersionFile(indexId).toPath();
        ...
        try (DataOutputStream os = FileUtil.doIOOperation(lastAttempt -> {
            try {
    ...
    private static IndexVersion getIndexVersion(ID<?, ?> indexName) {
        ...
        synchronized (IndexVersion.class) {
            ...
            try {
                Path versionFile = IndexInfrastructure.getVersionFile(indexName).toPath();
