...
public class StubUpdatingIndex extends SingleEntryFileBasedIndexExtension<SerializedStubTree>
    ...
    public static boolean canHaveStub(@Nonnull ProjectLocator projectLocator, @Nullable Project project, @Nonnull VirtualFile file) {
        FileType fileType = SubstitutedFileType.substituteFileType(
            file,
            file.getFileType(),
            project == null ? projectLocator.guessProjectForFile(file) : project
        );
        if (fileType instanceof LanguageFileType languageFileType) {
            ...
            if (parserDefinition.getFileNodeType() instanceof IStubFileElementType stubFileElementType) {
                if (stubFileElementType.shouldBuildStubFor(file)) {
                    return true;
                }
