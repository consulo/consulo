...
public class LiveTemplateCompletionContributor extends CompletionContributor implements DumbAware {
    ...
    public LiveTemplateCompletionContributor() {
        extend(CompletionType.BASIC, PlatformPatterns.psiElement(), (parameters, context, result) -> {
            ...
            List<? extends Template> availableTemplates = TemplateManager.getInstance(parameters.getPosition().getProject()).listApplicableTemplates(TemplateActionContext.expanding(file, editor));
            Map<Template, String> templates = filterTemplatesByPrefix(availableTemplates, editor, offset, false, false);
    ...
    private static void ensureTemplatesShown(AtomicBoolean templatesShown, Map<Template, String> templates, List<? extends Template> availableTemplates, CompletionResultSet result, boolean isAutopopup) {
        if (!templatesShown.getAndSet(true)) {
            List<String> templateKeys = ContainerUtil.map(availableTemplates, Template::getKey);
            ...
            for (Map.Entry<Template, String> entry : templates.entrySet()) {
                ProgressManager.checkCanceled();
                if (isAutopopup && entry.getKey().getShortcutChar() == TemplateConstants.NONE_CHAR) {
                    continue;
                }
