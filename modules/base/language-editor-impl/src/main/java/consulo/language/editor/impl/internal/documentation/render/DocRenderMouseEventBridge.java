...
final class DocRenderMouseEventBridge implements EditorMouseListener, EditorMouseMotionListener {
    ...
    private static @Nullable DocRenderer.EditorInlineHtmlPane redispatchEvent(EditorMouseEvent event,
        ...
        if (foldRegion instanceof CustomFoldRegion cfr) {
            ...
            if (r instanceof DocRenderer renderer) {
                Rectangle relativeBounds =
                    renderer.getEditorPaneBoundsWithinRenderer(cfr.getWidthInPixels(), cfr.getHeightInPixels());
                ...
                if (x >= 0 && x < relativeBounds.width && y >= 0 && y < relativeBounds.height) {
                    DocRenderer.EditorInlineHtmlPane editorInlineHtmlPane =
                        renderer.getRendererComponent(editor, relativeBounds.width, null);
                    if (eventId != MouseEvent.MOUSE_DRAGGED || targetPane == editorInlineHtmlPane) {
                        int button = mouseEvent.getButton();
                        dispatchEvent(editorInlineHtmlPane, new MouseEvent(editorInlineHtmlPane, eventId, 0,
                            mouseEvent.getModifiersEx(), x, y,
    ...
    private static void dispatchEvent(DocRenderer.EditorInlineHtmlPane editorInlineHtmlPane, MouseEvent event) {
        editorInlineHtmlPane.doWithRepaintTracking(() -> GuiUtils.processEvent(editorInlineHtmlPane, event));
    }

    private static void dispatchMouseExitEvent(DocRenderer.EditorInlineHtmlPane editorInlineHtmlPane) {
        dispatchEvent(editorInlineHtmlPane,
            new MouseEvent(editorInlineHtmlPane, MouseEvent.MOUSE_EXITED, 0, 0, 0, 0, 0, false));
    }
}
