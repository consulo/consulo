...
public class GlobalInspectionContextBase extends UserDataHolderBase implements GlobalInspectionContext {
  ...
  protected void launchInspections(@Nonnull final AnalysisScope scope) {
    ...
    ProgressManager.getInstance().run(new Task.Backgroundable(getProject(), InspectionsBundle.message("inspection.progress.title"), true, createOption()) {
      @Override
  ...
  public void performInspectionsWithProgress(@Nonnull final AnalysisScope scope, final boolean runGlobalToolsOnly, final boolean isOfflineInspections) {
  ...
  public void initializeTools(@Nonnull List<Tools> outGlobalTools, @Nonnull List<Tools> outLocalTools, @Nonnull List<Tools> outGlobalSimpleTools) {
  ...
  private static void classifyTool(@Nonnull List<Tools> outGlobalTools,
