...
@ExtensionImpl
public class LigaturesReaderModeProvider implements ReaderModeProvider {
    @Override
    public void applyModeChanged(Project project, Editor editor, boolean readerMode, boolean fileIsOpenAlready) {
        EditorColorsScheme scheme = editor.getColorsScheme();
        scheme.setUseLigatures(
            readerMode
                ? ReaderModeSettings.getInstance(project).isShowLigatures()
                : EditorColorsManager.getInstance().getGlobalScheme().getFontPreferences().useLigatures()
        );
    }
}
