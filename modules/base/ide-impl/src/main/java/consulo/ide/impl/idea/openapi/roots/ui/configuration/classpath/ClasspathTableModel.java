...
class ClasspathTableModel extends AbstractTableModel implements ItemRemovable {
    ...
    public static final LocalizeValue EXPORT_COLUMN_NAME = ProjectLocalize.modulesOrderExportExportColumn();
    private static final LocalizeValue SCOPE_COLUMN_NAME = ProjectLocalize.modulesOrderExportScopeColumn();
    ...
    public void init() {
        for (OrderEntry orderEntry : getModel().getOrderEntries()) {
            addItem(ClasspathTableItem.createItem(
                orderEntry,
                myState.getProject(),
                myState.getModulesConfigurator(),
                myState.getLibrariesConfigurator()
            ));
        }
    }
    ...
    public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
        ...
        else if (columnIndex == SCOPE_COLUMN && aValue instanceof DependencyScope dependencyScope) {
            item.setScope(dependencyScope);
        }
    }
    ...
    public String getColumnName(int column) {
        if (column == EXPORT_COLUMN) {
            return EXPORT_COLUMN_NAME.get();
        }
        if (column == SCOPE_COLUMN) {
            return SCOPE_COLUMN_NAME.get();
        }
        return "";
    }
