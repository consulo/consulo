...
public abstract class AbstractTreeClassChooserDialog<T extends PsiNamedElement> extends DialogWrapper implements TreeChooser<T> {
    ...
    protected static class MyGotoClassModel<T extends PsiNamedElement> extends GotoClassModel2 {
        ...
        public MyGotoClassModel(Project project, AbstractTreeClassChooserDialog<T> treeClassChooserDialog) {
            super(project);
        ...
        @Override
        public Object[] getElementsByName(String name, FindSymbolParameters parameters, ProgressIndicator canceled) {
            String patternName = parameters.getLocalPatternName();
            Collection<T> classes = myTreeClassChooserDialog.getClassesByName(
                name,
                parameters.isSearchInLibraries(),
                patternName,
                myTreeClassChooserDialog.getScope()
            );
            if (classes.size() == 0) {
    ...
    protected abstract Collection<T> getClassesByName(
        String name,
    ...
    private static class SubclassGotoClassModel<T extends PsiNamedElement> extends MyGotoClassModel<T> {
        ...
        public SubclassGotoClassModel(
            Project project,
            AbstractTreeClassChooserDialog<T> treeClassChooserDialog,
            TreeClassInheritorsProvider<T> inheritorsProvider
        ) {
        ...
        public String[] getNames(boolean checkBoxState) {
            ...
            myFastMode = myInheritorsProvider.searchForInheritorsOfBaseClass().forEach(new Predicate<>() {
                private int count;

                @Override
                @RequiredReadAction
                public boolean test(T aClass) {
                    if (count++ > 1000) {
                        return false;
                    }
        ...
        protected boolean isAccepted(T aClass) {
            if (myFastMode) {
                ...
                return (aClass == getTreeClassChooserDialog().getBaseClass() || myInheritorsProvider.isInheritorOfBaseClass(aClass)) && getTreeClassChooserDialog().getFilter()
                    .test(aClass);
            }
        }
    }
