...
public class ShowIntentionsPass extends TextEditorHighlightingPass {
    ...
    public static boolean markActionInvoked(@Nonnull Project project, @Nonnull final Editor editor, @Nonnull IntentionAction action) {
        ...
        for (HighlightInfo info : infos) {
            List<consulo.util.lang.Pair<HighlightInfoImpl.IntentionActionDescriptor, RangeMarker>> list =
                ((HighlightInfoImpl)info).quickFixActionMarkers;
            if (list != null) {
    ...
    public void doCollectInformation(@Nonnull ProgressIndicator progress) {
        if (!ApplicationManager.getApplication().isHeadlessEnvironment() && !HasFocus.hasFocus(myEditor.getContentUIComponent())) {
            return;
        }
