...
public class PsiVFSListener implements BulkFileListener {
    ...
    private void filesDeleted(List<? extends VFileEvent> events) {
        ...
        for (VFileEvent event : events) {
            ...
            VirtualFile vFile = de.getRequiredFile();
            VirtualFile parent = vFile.getParent();
    ...
    private void beforePropertyChange(VFilePropertyChangeEvent event) {
        VirtualFile vFile = event.getRequiredFile();
        ...
        runExternalAction(() -> {
            ...
            if (VirtualFile.PROP_NAME.equals(propertyName)) {
                ...
                if (vFile.isDirectory()) {
                    ...
                    else if ((!Registry.is("ide.hide.excluded.files") || !isExcludeRoot(vFile))
    ...
    private void filesMoved(List<? extends VFileEvent> events) {
        ...
        for (VFileEvent e : events) {
            ...
            PsiElement oldElement =
                vFile.isDirectory() ? myFileManager.getCachedDirectory(vFile) : myFileManager.getCachedPsiFileInner(vFile);
    ...
    class MyFileDocumentManagerAdapter implements FileDocumentManagerListener {
        ...
        public void fileContentReloaded(VirtualFile file, Document document) {
            ...
            if (!file.isValid()
                || psiFile == null
                || !RawFileLoader.getInstance().isTooLarge(file.getLength())
                || psiFile instanceof PsiLargeFile) {
                return;
            }
    ...
    public void before(List<? extends VFileEvent> events) {
        ...
        for (VFileEvent event : events) {
            if (event instanceof VFileDeleteEvent deleteEvent) {
                ...
                beforePropertyChange(propertyChangeEvent);
            }
            else if (event instanceof VFileMoveEvent moveEvent) {
                beforeFileMovement(moveEvent);
            }
        }
    }
