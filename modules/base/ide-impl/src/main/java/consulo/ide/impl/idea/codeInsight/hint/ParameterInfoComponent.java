...
public class ParameterInfoComponent extends JPanel {
    ...
    private static final Color BACKGROUND =
        JBColor.namedColor("ParameterInfo.background", TargetAWT.to(HintColorUtil.getInformationColor()));
    private static final Color FOREGROUND = JBColor.namedColor("ParameterInfo.foreground", new JBColor(0x1D1D1D, 0xBBBBBB));
    private static final Color HIGHLIGHTED_COLOR =
    ...
    private static final Color CONTEXT_HELP_FOREGROUND =
        JBColor.namedColor("ParameterInfo.infoForeground", new JBColor(0x787878, 0x878787));
    ...
    class MyParameterContext implements ParameterInfoUIContextEx {
        ...
        public String setupUIComponentPresentation(
            ...
            TextRange highlightRange = highlightStartOffset >= 0 && highlightEndOffset >= highlightStartOffset ? new TextRange(
            ...
            ParameterInfoController.SignatureItem item =
                new ParameterInfoController.SignatureItem(plainLine.toString(), strikeout, isDisabled, startOffsets, endOffsets);
            result.signatures.add(item);

            String resultedText = myPanels[i].setup(
                text,
                myEscapeFunction,
                highlightStartOffset,
                highlightEndOffset,
    ...
    public ParameterInfoController.Model update(boolean singleParameterInfo) {
        ...
        if (myShortcutLabel != null) {
            myShortcutLabel.setVisible(!singleParameterInfo);
        }
    ...
    private class OneElementComponent extends JPanel {
        ...
        public String setup(
            ParameterInfoController.Model result,
            String[] texts,
            Function<? super String, String> escapeFunction,
            ...
            ParameterInfoController.SignatureItem item =
                new ParameterInfoController.SignatureItem(fullLine.toString(), false, false, startOffsets, endOffsets);
            result.signatures.add(item);
            OneLineComponent component = getOneLineComponent(index);
            buf.append(component.setup(escapeString(line.toString(), escapeFunction), flagsMap, background));
    ...
    private final class OneLineComponent extends JPanel {
        ...
        private String setup(
            ...
            if (disableBeforeHighlight) {
                flagsMap.put(
                    new TextRange(0, highlightRange == null ? text.length() : highlightRange.getStartOffset()),
