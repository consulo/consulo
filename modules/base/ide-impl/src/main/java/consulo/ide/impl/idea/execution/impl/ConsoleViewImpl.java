...
public class ConsoleViewImpl extends JPanel implements ConsoleView, ObservableConsoleView, UiDataProvider, OccurenceNavigator {
        ...
        project.getMessageBus().connect(this).subscribe(DumbModeListener.class, new DumbModeListener() {
            ...
            public void exitDumbMode() {
                myProject.getApplication().invokeLater(() -> {
                    if (myEditor == null || project.isDisposed() || DumbService.getInstance(project).isDumb()) {
                        return;
                    }
    ...
    private void addFlushRequest(int millis, FlushRunnable flushRunnable) {
    ...
    private void performLaterWhenNoDeferredOutput(Runnable runnable) {
     ...
     */
    public final void addLayerToPane(JComponent component) {
    ...
    private void updateStickToEndState(boolean useImmediatePosition) {
        ...
        int scrollBarPosition =
            useImmediatePosition ? scrollBar.getValue() : myEditor.getScrollingModel().getVisibleAreaOnScrollingFinished().y;
    ...
    @RequiredUIAccess
    private void sendUserInput(CharSequence typedText) {
        UIAccess.assertIsUIThread();
            ...
            if (backspacePrefixLength > 0) {
                ...
                if (lineCount != 0) {
                    ...
                    document.deleteString(
                        Math.max(lineStartOffset, document.getTextLength() - backspacePrefixLength),
    ...
    private static void registerActionHandler(Editor editor, String actionId) {
    ...
    private ActionGroup getPopupGroup(MouseEvent mouseEvent) {
        ActionManager actionManager = ActionManager.getInstance();
        HyperlinkInfo info = myHyperlinks != null ? myHyperlinks.getHyperlinkInfoByPoint(mouseEvent.getPoint()) : null;
    ...
    public void rehighlightHyperlinksAndFoldings() {
        if (myEditor == null || myProject.isDisposed()) {
            return;
        }
    ...
    private void runHeavyFilters(int line1, int endLine) {
        ...
        myHeavyAlarm.addRequest(() -> {
            ...
            try {
                myFilters.applyHeavyFilter(
                    documentCopy,
                    startOffset,
                    startLine,
                    additionalHighlight -> addFlushRequest(0, new FlushRunnable(true) {
                        @Override
                        @RequiredUIAccess
                        public void doRun() {
    ...
    @RequiredUIAccess
    private void updateFoldings(int startLine, int endLine) {
        UIAccess.assertIsUIThread();
    ...
    private void addFoldRegion(Document document, ConsoleFolding folding, int startLine, int endLine) {
        ...
        if (oStart > 0 && folding.shouldBeAttachedToThePreviousLine()) {
            oStart--;
        }
        int oEnd =
    ...
    @RequiredUIAccess
    private void type(Editor editor, String text) {
        flushDeferredText();
    ...
    private abstract static class ConsoleAction extends AnAction implements DumbAware {
        ...
        protected abstract void execute(ConsoleViewImpl console, DataContext context);
    ...
    private static class TabHandler extends ConsoleAction {
        @Override
        @RequiredUIAccess
        protected void execute(ConsoleViewImpl console, DataContext context) {
    ...
    protected @Nullable OccurenceInfo calcNextOccurrence(int delta) {
        ...
        return EditorHyperlinkSupport.getNextOccurrence(
            myEditor,
            delta,
            next -> {
    ...
    @RequiredUIAccess
    private void insertUserText(int offset, String text) {
    ...
    @RequiredUIAccess
    private void doInsertUserInput(int offset, String text) {
        UIAccess.assertIsUIThread();
    ...
    private static void moveScrollRemoveSelection(Editor editor, int offset) {
