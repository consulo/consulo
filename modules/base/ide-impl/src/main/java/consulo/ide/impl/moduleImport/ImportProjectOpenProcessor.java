 ...
 * @since 31-Jan-17
 */
public class ImportProjectOpenProcessor implements ProjectOpenProcessor {
    ...
    public boolean canOpenProject(@Nonnull Path file) {
        for (ModuleImportProvider provider : myProviders) {
            if (provider.canImport(file.toFile())) {
                return true;
            }
        }
    ...
    public AsyncResult<Project> doOpenProjectAsync(@Nonnull VirtualFile virtualFile, @Nonnull UIAccess uiAccess, @Nonnull ProjectOpenContext openContext) {
        ...
        if (!uiAccess.isHeadless() && DefaultProjectOpenProcessor.getInstance().canOpenProject(Path.of(expectedProjectPath))) {
