...
public class GroupNode extends Node implements Navigatable, Comparable<GroupNode> {
    ...
    private UsageNode tryMerge(@Nonnull Usage usage) {
        ...
        for (UsageNode node : getUsageNodes()) {
            ...
            if (original instanceof MergeableUsage originalMergeableUsage && originalMergeableUsage.merge(mergeableUsage)) {
                return node;
            }
        }
    ...
    void incrementUsageCount() {
        ...
        while (true) {
            ...
            if (!(groupNode.getParent() instanceof GroupNode parentNode)) {
                return;
            }
    ...
    protected boolean isDataReadOnly() {
        ...
        while (enumeration.hasMoreElements()) {
            if (enumeration.nextElement() instanceof Node node && node.isReadOnly()) {
                return true;
            }
        }
        return false;
    ...
    public synchronized Collection<UsageNode> getUsageNodes() {
        ...
        for (Node n : myChildren) {
            if (n instanceof UsageNode usageNode) {
                list.add(usageNode);
            }
        }
        return list;
