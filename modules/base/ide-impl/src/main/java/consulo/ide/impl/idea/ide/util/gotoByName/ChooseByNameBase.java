...
public abstract class ChooseByNameBase implements ChooseByNameViewModel {
    ...
    protected void initUI(
        ...
        if (checkBoxText.isEmpty()) {
            ...
            String text = "<html>" + checkBoxText.get() + (
                myCheckBoxShortcut.getShortcuts().length > 0
                    ? " <b color='" + ColorUtil.toHex(color) + "'>" +
                    KeymapUtil.getShortcutsText(myCheckBoxShortcut.getShortcuts()) + "</b>"
                    : ""
            ) + "</html>";
        ...
        ActionToolbar actionToolbar = ActionManager.getInstance().createActionToolbar("ChooseByNameBase", group, true);
        ...
        new MyCopyReferenceAction().registerCustomShortcutSet(
            ActionManager.getInstance().getAction(IdeActions.ACTION_COPY).getShortcutSet(),
            myTextField
        );
                            ...
                            if (popup != null) {
                                ...
                                if (oppositeComponent != null && !(oppositeComponent instanceof JFrame) &&
                                    ...
                                    ProjectIdeFocusManager.getInstance(myProject)
                                        .requestFocus(myTextField, true);// Otherwise me may skip some KeyEvents
                                    return;
                                }
    ...
    private static Set<KeyStroke> getShortcuts(String actionId) {
    ...
    private void updateDocumentation() {
        JBPopup hint = myTextFieldPanel.getHint();
        Object element = getChosenElement();
        if (hint != null) {
            if (element instanceof PsiElement) {
                ...
                Object o = dataProvider.getData(PsiElement.KEY);
    ...
    private synchronized String[] ensureNamesLoaded(boolean checkboxState) {
        ...
        if (checkboxState && myModel instanceof ContributorsBasedGotoByModel
    ...
    protected void showTextFieldPanel() {
        ...
        Dimension preferredTextFieldPanelSize = myTextFieldPanel.getPreferredSize();
        ...
        Disposer.register(myTextPopup, this::cancelListUpdater);
    ...
    private JLayeredPane getLayeredPane() {
        ...
        if (window instanceof JFrame jFrame) {
            ...
            throw new IllegalStateException(
                "cannot find parent window: project=" + myProject + (myProject != null ? "; open=" + myProject.isOpen() : "") +
                    "; window=" + window
            );
        }
        return layeredPane;
    ...
    void rebuildList(
        ...
        String pattern = patternToLowerCase(transformPattern(text));
        Matcher matcher = buildPatternMatcher(isSearchInAnyPlace() ? "*" + pattern : pattern);
        if (cellRenderer instanceof MatcherHolder matcherHolder) {
            matcherHolder.setPatternMatcher(matcher);
        }
    ...
    @RequiredUIAccess
    public void scheduleCalcElements(
        String text,
        boolean checkboxState,
        ModalityState modalityState,
        SelectionPolicy policy,
        @RequiredUIAccess Consumer<? super Set<?>> callback
    ) {
    ...
    private void setElementsToList(SelectionPolicy pos, Collection<?> elements) {
        if (checkDisposed()) {
            return;
        }
    ...
    public int calcSelectedIndex(Object[] modelElements, String trimmedText) {
        ...
        String statContext = statisticsContext();
    ...
    private void appendToModel(List<? extends ModelDiff.Cmd> commands, SelectionPolicy selection) {
    ...
    private void applySelection(SelectionPolicy selection) {
        ...
        if (!indices.isEmpty()) {
            ScrollingUtil.ensureIndexIsVisible(myList, indices.get(0), 0);
        }
    }
    ...
    protected final class MyTextField extends JTextField implements PopupOwner, TypeSafeDataProvider {
        ...
        private void fillInCommonPrefix(String pattern) {
            List<String> list = myProvider.filterNames(ChooseByNameBase.this, getNames(myCheckBox.getValueOrError()), pattern);
            if (list.isEmpty()) {
                return;
            }
            ...
            if (!list.isEmpty()) {
                for (String name : list) {
                    String string = StringUtil.toLowerCase(name);
    ...
    private class CalcElementsThread extends ReadTask {
        ...
        @Override
        public Continuation runBackgroundProcess(ProgressIndicator indicator) {
        ...
        private void scheduleIncrementalListUpdate(Set<Object> elements, int lastCount) {
            ...
            myUpdateListAlarm.addRequest(() -> {
                if (myCalcElementsThread != this || !myProgress.isRunning()) {
                    return;
                }
    ...
    private abstract class ShowFindUsagesAction extends DumbAwareAction {
        ...
        private void fillUsages(
            Collection<Object> matchElementsArray,
        ...
        private void showUsageView(
            ...
            myProject.getInstance(UsageViewManager.class).showUsages(usageTargets, usages.toArray(Usage.EMPTY_ARRAY), presentation);
        }
    ...
    private static class MyUsageInfo2UsageAdapter extends UsageInfo2UsageAdapter {
        ...
        @RequiredReadAction
        MyUsageInfo2UsageAdapter(PsiElement element, boolean separateGroup) {
