...
public class SearchUtil {
    ...
    public static void processComponent(JComponent component, Set<OptionDescription> configurableOptions, String path) {
        if (component instanceof SkipSelfSearchComponent) {
            return;
        }
        if (Objects.equals(UIUtil.getClientProperty(component, SkipSelfSearchComponent.KEY), Boolean.TRUE)) {
            return;
        }
        ...
        if (component instanceof JLabel label) {
            ...
            if (labelText != null) {
                processUILabel(labelText, configurableOptions, path);
            }
        }
        else if (component instanceof JCheckBox checkBox) {
            String checkBoxTitle = checkBox.getText();
            ...
            String radioButtonTitle = radioButton.getText();
        ...
        if (component instanceof JTabbedPane tabbedPane) {
            ...
            for (int i = 0; i < tabCount; i++) {
                ...
                if (tabbedPane.getComponentAt(i) instanceof JComponent tabComponent) {
            ...
            if (components != null) {
                for (Component child : components) {
                    if (child instanceof JComponent childComponent) {
                        processComponent(childComponent, configurableOptions, path);
                    }
                }
            }
        }
    ...
    private static void processUILabel(String title, Set<OptionDescription> configurableOptions, String path) {
        ...
        for (String option : words) {
            configurableOptions.add(new OptionDescription(
                option,
                HTML_PATTERN.matcher(title).replaceAll(" ").replaceAll(regex, " "),
                path
            ));
        }
    }
    ...
    private static boolean traverseComponentsTree(
        SearchableConfigurable configurable,
        GlassPanel glassPanel,
    ...
    public static boolean isComponentHighlighted(String text, String option, boolean force, SearchableConfigurable configurable) {
        ...
        if (!force) {
            ...
            return highlight || text.toLowerCase().contains(option.toLowerCase());
        }
        else {
    ...
    private static String markup(String textToMarkup, Pattern insideHtmlTagPattern, String option) {
        ...
        if (styleIdx < 0 || styleEndIdx < 0) {
            ...
            result = markup(textToMarkup.substring(0, styleIdx), insideHtmlTagPattern, option) +
    ...
    private static void appendSelectedWords(
        String text,
    ...
    private static JBPopup createPopup(
        ...
        if (model.size() > 0) {
            Runnable onChosen = () -> {
                if (list.getSelectedValue() instanceof OptionDescription description) {
                    searchField.setText(description.getHit());
                    searchField.addCurrentTextToHistory();
                    SwingUtilities.invokeLater(() -> {
                        //do not show look up again
                ...
                public void keyPressed(KeyEvent e) {
                    if (e.getKeyCode() != KeyEvent.VK_ENTER
                        ...
                        if (e.getKeyChar() != KeyEvent.CHAR_UNDEFINED) {
                            searchField.process(new KeyEvent(
                                searchField,
                                KeyEvent.KEY_TYPED,
                                e.getWhen(),
                                e.getModifiers(),
    ...
    public static void showHintPopup(
        ConfigurableSearchTextField searchField,
        JBPopup[] activePopup,
        Alarm showHintAlarm,
        Consumer<String> selectConfigurable,
        Project project
    ...
    public static void registerKeyboardNavigation(
        ...
        Consumer<Integer> shower = direction -> {
            ...
            JBPopup popup = createPopup(searchField, activePopup, showHintAlarm, selectConfigurable, project, direction);
            if (popup != null) {
                popup.showUnderneathOf(searchField);
            }
            activePopup[0] = null;
    ...
    public static List<Configurable> expandGroup(Configurable[] configurables) {
        ...
        result = ContainerUtil.filter(
            result,
            configurable -> !(configurable instanceof SearchableConfigurable.Parent searchableParent) || searchableParent.isVisible()
        );
