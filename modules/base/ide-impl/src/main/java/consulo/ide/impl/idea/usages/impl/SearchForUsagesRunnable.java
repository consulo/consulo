...
class SearchForUsagesRunnable implements Runnable {
    ...
    SearchForUsagesRunnable(
        ...
        UsageViewManager.@Nullable UsageViewStateListener listener
    ) {
        myProject = project;
    ...
    private static void notifyByFindBalloon(
        ...
        if (!largeFiles.isEmpty()) {
            String shortMessage = "(<a href='" + LARGE_FILES_HREF_TARGET + "'>" +
                UsageLocalize.largeFilesWereIgnored(largeFiles.size())
                + "</a>)";

            resultLines.add(shortMessage);
            resultListener = addHrefHandling(resultListener, LARGE_FILES_HREF_TARGET, () -> {
                LocalizeValue detailedMessage = detailedLargeFilesMessage(largeFiles);
                List<String> strings = new ArrayList<>(lines);
                strings.add(detailedMessage.get());
                //noinspection SSBasedInspection
                ToolWindowManager.getInstance(project)
                    .notifyByBalloon(ToolWindowId.FIND, info, wrapInHtml(strings), PlatformIconGroup.actionsFind(), listener);
            });
        }
        ...
        if (searchIncludingProjectFileUsages != null) {
            resultLines.add(
                UsageLocalize.messageOccurrencesInProjectConfigurationFilesAreSkipped() + " " +
                "<a href='" + SHOW_PROJECT_FILE_OCCURRENCES_HREF_TARGET + "'>" + UsageLocalize.messageIncludeThem() + "</a>"
            );
    ...
    private static LocalizeValue detailedLargeFilesMessage(Collection<VirtualFile> largeFiles) {
        return UsageLocalize.filesAreTooLargeLargeAndCannotBeScanned(
            largeFiles.size(),
            "<br> " + StringUtil.join(
                ContainerUtil.getFirstItems(new ArrayList<>(largeFiles), 10),
                SearchForUsagesRunnable::presentableFileInfo,
                "<br> "
            ) + "<br> "
        );
    }
    ...
    private static PsiElement getPsiElement(UsageTarget[] searchFor) {
        return searchFor[0] instanceof PsiElementUsageTarget elementUsageTarget
            ? ReadAction.compute(elementUsageTarget::getElement) : null;
    }
    ...
    private static void flashUsageScriptaculously(Usage usage) {
        ...
        Predicate<Segment> processor = Processors.cancelableCollectProcessor(segments);
        ...
        if (usageCount >= 2 || usageCount == 1 && myProcessPresentation.isShowPanelIfOnlyOneUsage()) {
            ...
            if (myUsageViewRef.compareAndSet(null, usageView)) {
                ...
                if (firstUsage != null) {
                    ...
                    myProject.getApplication().runReadAction(() -> finalUsageView.appendUsage(firstUsage));
                }
            }
            else {
                UsageViewImpl finalUsageView = usageView;
    ...
    private void openView(UsageViewImpl usageView) {
    ...
    private void searchUsages(AtomicBoolean findStartedBalloonShown) {
        ...
        usageSearcher.generate(usage -> {
            ...
            if (incrementCounter) {
                ...
                if (usageView != null) {
                    application.runReadAction(() -> usageView.appendUsage(usage));
                }
            }
            return !indicator1.isCanceled();
        });
        if (getUsageView(indicator) != null) {
            application.invokeLater(() -> myUsageViewManager.showToolWindow(true), myProject.getDisposed());
        }
        Disposer.dispose(findUsagesStartedBalloon);
        application.invokeLater(
            () -> {
                if (findStartedBalloonShown.get()) {
                    Balloon balloon = myProject.getInstance(ToolWindowManager.class).getToolWindowBalloon(ToolWindowId.FIND);
                    if (balloon != null) {
                        balloon.hide();
                    }
                }
        ...
        if (usageCount == 0 && myProcessPresentation.isShowNotFoundMessage()) {
                    ...
                    public void run() {
                        if (myProcessPresentation.isCanceled()) {
                            notifyByFindBalloon(
                                ...
                                myProject,
                                Collections.singletonList(UsageLocalize.messageUsageSearchWasCanceled().get())
                            );
                        ...
                        LocalizeValue message = UsageLocalize.dialogNoUsagesFoundIn(
                        ...
                        if (notFoundActions.isEmpty()) {
                            ...
                            titles.add(UsageLocalize.dialogButtonOk().get());
                            ...
                            int option = Messages.showDialog(
                                myProject,
                                message.get(),
                                UsageLocalize.dialogTitleInformation().get(),
            ...
            application.invokeLater(
                () -> {
                    ...
                    lines.add(UsageLocalize.messageOnlyOneUsageFound().get());
                    if (myOutOfScopeUsages.get() != 0) {
            ...
            if (myOutOfScopeUsages.get() == 0 || getPsiElement(mySearchFor) == null) {
                ...
                lines = Arrays.asList(
                    UsageViewManagerImpl.outOfScopeMessage(myOutOfScopeUsages.get(), mySearchScopeToWarnOfFallingOutOf),
                    createSearchInProjectHtml()
                );
