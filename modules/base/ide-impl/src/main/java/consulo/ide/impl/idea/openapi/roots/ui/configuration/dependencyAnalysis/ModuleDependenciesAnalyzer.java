...
public class ModuleDependenciesAnalyzer {
    ...
    private void analyze() {
        ...
        Map<String, List<OrderPath>> urlExplanations = new LinkedHashMap<>();
        ...
        Map<OrderEntry, List<OrderPath>> orderExplanations = new LinkedHashMap<>();
    ...
    private class PathWalker {
         ...
         * The current stack
         */
        private final List<OrderPathElement> myStack = new ArrayList<>();
        /**
        ...
        void examine(Module m, int level) {
            ...
            try {
                ...
                e.forEach(orderEntry -> {
                    ...
                    try {
                        if (orderEntry instanceof ModuleOrderEntry o) {
                            Module dependency = o.getModule();
                            if (dependency != null) {
                                examine(dependency, level + 1);
                            }
                        }
                        else if (orderEntry instanceof ModuleSourceOrderEntry) {
                            if (!myProduction || !myCompile) {
                                ModuleCompilerPathsManager pathsManager = ModuleCompilerPathsManager.getInstance(m);
                                OrderPath p = new OrderPath(myStack);

                                addUrlPath(p, pathsManager.getCompilerOutputUrl(ProductionContentFolderTypeProvider.getInstance()));
                                addUrlPath(p, pathsManager.getCompilerOutputUrl(ProductionResourceContentFolderTypeProvider.getInstance()));
                                boolean includeTests = !myCompile ? !myProduction : level > 0 && !myProduction;
                                if (includeTests) {
                                    addUrlPath(p, pathsManager.getCompilerOutputUrl(TestContentFolderTypeProvider.getInstance()));
                                    addUrlPath(p, pathsManager.getCompilerOutputUrl(TestResourceContentFolderTypeProvider.getInstance()));
                                }
    ...
    public static class OrderPath {
        ...
        public boolean equals(@Nullable Object obj) {
            return obj == this
                || obj instanceof OrderPath that && myEntries.equals(that.myEntries);
        }
    }
    ...
    public static class OrderEntryPathElement extends OrderPathElement {
        ...
        public boolean equals(@Nullable Object obj) {
            return obj == this
                || obj instanceof OrderEntryPathElement that && myEntry == that.myEntry;
        }
    ...
    public static class UrlExplanation extends Explanation {
        ...
        public @Nullable Image getIcon() {
            ...
            return file == null ? PlatformIconGroup.generalError() : VirtualFilePresentation.getIcon(file);
        }
