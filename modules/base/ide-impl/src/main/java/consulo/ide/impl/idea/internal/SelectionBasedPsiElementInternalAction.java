...
public abstract class SelectionBasedPsiElementInternalAction<T extends PsiElement> extends AnAction {
    ...
    public final void actionPerformed(@Nonnull AnActionEvent e) {
        ...
        if (expressions.size() > 1) {
            IntroduceTargetChooser.showChooser(
                editor,
                expressions,
                expression -> performOnElement(editor, expression),
                PsiElement::getText
            );
        }
    ...
    private void performOnElement(@Nonnull Editor editor, @Nonnull T first) {
        ...
        if (informationHint != null) {
            Application.get().invokeLater(() -> HintManager.getInstance().showInformationHint(editor, informationHint));
        }
        else {
            Application.get().invokeLater(() -> HintManager.getInstance().showErrorHint(editor, getErrorHint()));
        }
    }
