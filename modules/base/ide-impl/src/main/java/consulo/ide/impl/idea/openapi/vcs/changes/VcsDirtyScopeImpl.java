...
public final class VcsDirtyScopeImpl extends VcsModifiableDirtyScope {
  ...
  private final
  @Nullable
  HashingStrategy<FilePath> myHashingStrategy;
  ...
  public VcsDirtyScopeImpl(@Nonnull AbstractVcs vcs, boolean wasEverythingDirty) {
    myVcs = vcs;
  ...
  private @Nonnull Set<FilePath> newFilePathsSet() {
  ...
  @Override
  public void iterate(final Predicate<? super FilePath> iterator) {
  ...
  @Override
  public void iterateExistingInsideScope(Predicate<? super VirtualFile> processor) {
    if (myProject.isDisposed()) return;
    ...
    for (VirtualFile root : myAffectedVcsRoots) {
      ...
      if (files != null) {
        for (FilePath file : files) {
          ...
          if (vFile != null && vFile.isValid()) {
            processor.test(vFile);
            if (vFile.isDirectory()) {
              for (VirtualFile child : vFile.getChildren()) {
                processor.test(child);
              }
            }
          }
        }
