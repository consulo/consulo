...
public class PredefinedSearchScopeProviderImpl extends PredefinedSearchScopeProvider {
    ...
    public List<SearchScope> getPredefinedScopes(
        ...
        if (openFilesScope != GlobalSearchScope.EMPTY_SCOPE) {
            ...
            result.add(new LocalSearchScope(PsiElement.EMPTY_ARRAY, IdeLocalize.scopeOpenFiles().get()));
        }

        Editor selectedTextEditor =
            UIAccess.isUIThread() ? FileEditorManager.getInstance(project).getSelectedTextEditor() : null;
        PsiFile psiFile =
        ...
        if (currentFile != null || showEmptyScopes) {
            ...
            result.add(new LocalSearchScope(scope, IdeLocalize.scopeCurrentFile().get()));
        }
                ...
                if (startElement != null) {
                    ...
                    if (endElement != null) {
                        ...
                        if (parent != null) {
                            ...
                            if (!elements.isEmpty()) {
                                SearchScope local =
                                    new LocalSearchScope(PsiUtilCore.toPsiElementArray(elements), IdeLocalize.scopeSelection().get());
                                result.add(local);
                            }
                        }
                    }
        ...
        if (usageView) {
            ...
            if (selectedUsageView != null && !selectedUsageView.isSearchInProgress()) {
                ...
                if (prevSearchFiles) {
                    ...
                    if (!results.isEmpty()) {
                        result.add(new LocalSearchScope(
                            PsiUtilCore.toPsiElementArray(results),
                            IdeLocalize.scopePreviousSearchResults().get()
                        ));
                    }
                }
            }
    ...
    protected static Set<VirtualFile> collectFiles(Set<Usage> usages, boolean findFirst) {
        ...
        for (Usage usage : usages) {
            if (usage instanceof PsiElementUsage elemUsage) {
                PsiElement psiElement = elemUsage.getElement();
