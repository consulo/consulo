...
public class PathUtil {
    ...
    private static boolean isWindowsUNCRoot(@Nonnull String path, int lastPathSeparatorPosition) {
        return Platform.CURRENT == Platform.WINDOWS && (path.startsWith("//") || path.startsWith("\\\\")) && getLastIndexOfPathSeparator(
            path,
            lastPathSeparatorPosition
        ) == 1;
    }
    ...
    public static String suggestFileName(@Nonnull String text, boolean allowDots, boolean allowSpaces) {
        ...
        for (int i = 0; i < text.length(); i++) {
            ...
            if (!isValidFileNameChar(
                c,
                Platform.CURRENT,
                true
            ) || (!allowDots && c == '.') || (!allowSpaces && Character.isWhitespace(c))) {
                result.append('_');
            }
            else {
                result.append(c);
    ...
    private static boolean isValidFileNameChar(char c, Platform os, boolean strict) {
        ...
        if ((strict || os == Platform.WINDOWS) && (c < 32 || WINDOWS_CHARS.indexOf(c) >= 0)) {
            return false;
        }
        if (strict && c == ';') {
            return false;
        ...
        if (!SystemInfoRt.isWindows && !SystemInfoRt.isMac) {
            ...
            if (property != null) {
                try {
                    ...
                    Logger.getInstance(PathUtil.class).warn("unknown JNU charset: " + property, e);
                }
            }
        }
