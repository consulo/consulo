...
public abstract class RemoteProcessSupport<Target, EntryPoint, Parameters> {
    ...
    public void stopAll(boolean wait) {
        List<ProcessHandler> allHandlers = new ArrayList<>();
    ...
    private void startProcess(Target target, Parameters configuration, Pair<Target, Parameters> key) {
        ...
        try {
            ...
            dropProcessInfo(
                key,
                e instanceof ExecutionException ? e.getMessage() : consulo.ide.impl.idea.util.ExceptionUtil.getUserStackTrace(e, LOG),
                processHandler
            );
            return;
        }
    ...
    private boolean getExistingInfo(SimpleReference<RunningInfo> ref, Pair<Target, Parameters> key) {
        Info info;
        synchronized (myProcMap) {
        ...
        if (info instanceof RunningInfo runningInfo) {
            //noinspection SynchronizationOnLocalVariableOrMethodParameter
            synchronized (ref) {
                ref.set(runningInfo);
                ref.notifyAll();
