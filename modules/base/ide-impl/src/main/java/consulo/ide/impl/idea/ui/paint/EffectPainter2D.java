...
public enum EffectPainter2D implements RegionPainter2D<Font> {
        ...
        public void paint(@Nonnull Graphics2D g, double x, double y, double width, double height, Font font) {
            if (width > 0 && height > 0) {
                if (!Registry.is("ide.text.effect.new.metrics")) {
                    ...
                    double thickness = PaintUtil.alignToInt(
                        maybeScaleFontMetricsThickness(metrics.getStrikethroughThickness(), font),
                        g,
                        RoundingMode.FLOOR
                    );
    ...
    private static void paintUnderline(
        Graphics2D g,
        double x,
        double y,
        double width,
        ...
        if (width > 0 && height > 0) {
            if (Registry.is("ide.text.effect.new.metrics")) {
                ...
                RoundingMode roundingMode = !JreHiDpiUtil.isJreHiDPIEnabled()
                    || painter != WAVE_UNDERSCORE
                    || font.getSize2D() / UISettingsUtil.getDefFontSize() > 1
    ...
    private static void drawLineCentered(
        Graphics2D g,
        double x,
        double y,
        double width,
    ...
    private enum Cached implements RegionPainter2D<Paint> {
        ...
        WAVE_UNDERSCORE {
            ...
            void paintImage(Graphics2D g, double width, double height, double period) {
                ...
                if (Registry.is("ide.text.effect.new.metrics")) {
                    ...
                    height += 2;
                    if (g.getClass().getName().equals("consulo.ide.impl.idea.util.HiDPIScaledGraphics")) {
                        lower += .5;
                        upper += .5;
                    }
                }
        ...
        public void paint(@Nonnull Graphics2D g, double x, double y, double width, double height, Paint paint) {
            ...
            BufferedImage image = paint instanceof Color color ? getImage(g, color, height) : createImage(g, paint, height);
