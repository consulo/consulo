...
public class IncrementalSearchHandler {
    private static final Key<PerEditorSearchData> SEARCH_DATA_IN_EDITOR_VIEW_KEY = Key.create(
        "IncrementalSearchHandler.SEARCH_DATA_IN_EDITOR_VIEW_KEY");
    ...
    private static void updatePosition(Editor editor, PerHintSearchData data, boolean nothingIfFailed, boolean searchBack) {
        ...
        if (index < 0) {
            ...
            data.label.setForeground(TargetAWT.to(ComponentColors.TEXT_FOREGROUND));
            if (matchLength > 0) {
                TextAttributes attributes = editor.getColorsScheme().getAttributes(EditorColors.SEARCH_RESULT_ATTRIBUTES);
                data.segmentHighlighter = editor.getMarkupModel().addRangeHighlighter(
                    index,
                    index + matchLength,
    ...
    private static class MyLabel extends JLabel {
        public MyLabel(String text) {
            super(text);
            this.setBackground(TargetAWT.to(HintColorUtil.getInformationColor()));
            this.setForeground(TargetAWT.to(ComponentColors.TEXT_FOREGROUND));
            this.setOpaque(true);
        }
    }
    ...
    public static class MyTypedHandler extends TypedActionHandlerBase {
        ...
        public void execute(Editor editor, char charTyped, DataContext dataContext) {
            ...
            if (data == null || data.hint == null) {
                ...
                MyPanel comp = (MyPanel) data.hint.getComponent();
                if (comp.getTruePreferredSize().width > comp.getSize().width) {
                    Rectangle bounds = data.hint.getBounds();
                    data.hint.pack();
                    data.hint.updateLocation(bounds.x, bounds.y);
                }
