...
public class ColorAndFontOptions extends SearchableConfigurable.Parent.Abstract implements Configurable.NoMargin, ApplicationConfigurable {
    ...
    @RequiredUIAccess
    private static boolean selectOrEdit(
        DataContext context,
        String search,
        Function<ColorAndFontOptions, SearchableConfigurable> function
    ) {
    ...
    private boolean isSchemeListModified() {
        if (mySomeSchemesDeleted) {
            return true;
        }

        if (!mySelectedScheme.getName().equals(EditorColorsManager.getInstance().getGlobalScheme().getName())) {
            return true;
        }
    ...
    public String[] getSchemeNames() {
        ...
        Collections.sort(
            schemes,
            (o1, o2) -> {
                if (isReadOnly(o1) && !isReadOnly(o2)) {
                    return -1;
                }
                if (!isReadOnly(o1) && isReadOnly(o2)) {
                    return 1;
    ...
    public void saveSchemeAs(String name) {
        ...
        EditorColorsScheme clone = scheme.getOriginalScheme().clone();
    ...
    protected List<ColorAndFontPanelFactory> createPanelFactories() {
        ...
        application.getExtensionPoint(ColorSettingsPage.class).forEach(page -> extensions.add(new ColorAndFontPanelFactoryEx() {
            @Override
    ...
    private static class FontConfigurableFactory implements ColorAndFontPanelFactory, ConfigurableWeight {
        ...
        public NewColorAndFontPanel createPanel(@Nonnull ColorAndFontOptions options) {
            ...
            return new NewColorAndFontPanel(
                ...
                previewPanel,
                LocalizeValue.localizeTODO("Font"),
                null,
                null
            ) {
                @Override
    ...
    private static void initPluggedDescriptions(
        ...
        application.getExtensionPoint(ColorSettingsPage.class)
            .forEach(page -> initDescriptions(page, descriptions, scheme));

        application.getExtensionPoint(ColorAndFontDescriptorsProvider.class)
            .forEach(provider -> initDescriptions(provider, descriptions, scheme));
    }
    ...
    private static void initScopesDescriptors(@Nonnull List<EditorSchemeAttributeDescriptor> descriptions, @Nonnull MyColorScheme scheme) {
        ...
        for (Pair<NamedScope, NamedScopesHolder> pair : list) {
            ...
            addSchemedDescription(
                descriptions,
                name,
                ApplicationLocalize.titleScopeBased(),
                textAttributesKey,
                scheme,
                holder.getIcon(),
                toolTip
    ...
    private static void addEditorSettingDescription(
        ...
        list.add(new EditorSettingColorDescription(
            ...
            backgroundKey,
            foregroundKey,
            calcType(backgroundKey, foregroundKey),
            scheme
        ));
    }
            ...
            if (fallbackKey != null) {
                ...
                if (myBaseAttributeDescriptor == null) {
                    myBaseAttributeDescriptor =
                        new Pair<>(null, new AttributesDescriptor(LocalizeValue.of(fallbackKey.getExternalName()), fallbackKey));
                }
            }
    ...
    private static class EditorSettingColorDescription extends ColorAndFontDescription {
        ...
        private EditorSettingColorDescription(
            LocalizeValue name,
            LocalizeValue group,
            EditorColorKey backgroundKey,
            EditorColorKey foregroundKey,
            String type,
    ...
    private static class MyColorScheme extends EditorColorsSchemeImpl {
        ...
        private boolean isFontModified() {
            if (!getFontPreferences().equals(myParentScheme.getFontPreferences())) {
                return true;
            }
            if (getLineSpacing() != myParentScheme.getLineSpacing()) {
                return true;
            }
