...
import consulo.ide.impl.idea.codeInsight.completion.CompletionPhaseListener;
import consulo.ide.impl.idea.codeInsight.completion.CompletionProgressIndicator;
import consulo.ide.impl.idea.codeInsight.completion.CompletionThreadingBase;
...
public final class CompletionServiceImpl extends CompletionService {
    ...
    private static class CompletionResultSetImpl extends CompletionResultSet {
        ...
        public void restartCompletionOnPrefixChange(ElementPattern<String> prefixCondition) {
            ...
            if (process instanceof CompletionProgressIndicator) {
                ((CompletionProgressIndicator) process).addWatchedPrefix(
                    myParameters.getOffset() - getPrefixMatcher().getPrefix().length(),
                    prefixCondition
                );
            }
        }
    ...
    public static void setCompletionPhase(@Nonnull CompletionPhase phase) {
        ...
        if (oldIndicator != null && !(phase instanceof CompletionPhase.BgCalculation)) {
            LOG.assertTrue(
                !oldIndicator.isRunning() || oldIndicator.isCanceled(),
        ...
        if (isCompletionRunning != wasCompletionRunning) {
            Application.get()
                .getMessageBus()
                .syncPublisher(CompletionPhaseListener.class)
    ...
    public CompletionSorterImpl defaultSorter(CompletionParameters parameters, PrefixMatcher matcher) {
        ...
        for (final Weigher weigher : WeighingService.getWeighers(CompletionService.RELEVANCE_KEY)) {
            ...
            if ("prefix".equals(id)) {
                ...
                sorter = sorter.withClassifier(new ClassifierFactory<>("stats") {
                    @Override
