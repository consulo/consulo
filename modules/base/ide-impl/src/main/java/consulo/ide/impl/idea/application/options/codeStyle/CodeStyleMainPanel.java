...
public class CodeStyleMainPanel extends JPanel implements TabbedLanguageCodeStylePanel.TabChangeListener {
    ...
    private final Action mySetFromAction = new AbstractAction("Set from...") {
        ...
        public void actionPerformed(ActionEvent event) {
            if (ensureCurrentPanel().getSelectedPanel() instanceof TabbedLanguageCodeStylePanel codeStylePanel) {
                codeStylePanel.showSetFrom((Component) event.getSource());
            }
        }
    };
    ...
    private void addWaitCard() {
        ...
        JLabel label = new JLabel(ApplicationLocalize.labelLoadingPagePleaseWait().get());
    ...
    private NewCodeStyleSettingsPanel ensurePanel(@Nonnull CodeStyleScheme scheme) {
        ...
        if (!mySettingsPanels.containsKey(name)) {
            ...
            panel.reset();
            // we need set model after #reset() due it will call refresh model, and recreate UI (and lock ui due stack overflow)
            panel.setModel(myModel);
            if (panel.getSelectedPanel() instanceof TabbedLanguageCodeStylePanel tabbedPanel) {
