...
public class MultipleBuildsView implements BuildProgressListener, Disposable {
    ...
    public MultipleBuildsView(Project project, BuildContentManager buildContentManager, AbstractViewManager viewManager) {
        ...
        myBuildsList.setFixedCellHeight(UIUtil.getListFixedCellHeight() * 2);
            ...
            if (event instanceof StartBuildEvent) {
                ...
                if (myBuildsList.getModel().getSize() > 1) {
                    ...
                    for (BuildView consoleView : myViewMap.values()) {
                        BuildTreeConsoleView buildConsoleView =
                            consoleView.getView(BuildTreeConsoleView.class.getName(), BuildTreeConsoleView.class);
                ...
                ((BuildContentManagerImpl) myBuildContentManager).startBuildNotified(
                    buildInfo,
                    buildInfo.content,
                    buildInfo.getProcessHandler()
                );
            }
            else {
        ...
        if (myContent == null) {
            ...
            if (isInitializeStarted.compareAndSet(false, true)) {
                Application.get().invokeLater(() -> {
                    ...
                    myBuildsList.addListSelectionListener(e -> {
