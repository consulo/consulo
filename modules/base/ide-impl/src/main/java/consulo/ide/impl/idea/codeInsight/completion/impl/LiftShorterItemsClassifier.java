...
public class LiftShorterItemsClassifier extends Classifier<LookupElement> {
    ...
    private Iterable<LookupElement> liftShorterElements(
        ...
        @Nullable Set<LookupElement> lifted,
        ProcessingContext context
    ) {
        Set<LookupElement> srcSet = Sets.newIdentityHashSet(source instanceof Collection collection ? collection.size() : myCount);
    ...
    private class LiftingIterable implements Iterable<LookupElement> {
        ...
        public Iterator<LookupElement> iterator() {
            final Set<LookupElement> processed = Sets.newIdentityHashSet(mySrcSet.size());
            final Set<Collection<LookupElement>> arraysProcessed = Sets.newIdentityHashSet();
            ...
            return new FlatteningIterator<>(base) {
                ...
                protected Iterator<LookupElement> createValueIterator(LookupElement element) {
                    ...
                    if (shorter != null) {
                        ...
                        return (myLiftBefore ? ContainerUtil.concat(lifted, singleton) : ContainerUtil.concat(
                            singleton,
                            lifted
                        )).iterator();
                    }
    ...
    private static <K, V> MultiMap<K, V> createMultiMap(final boolean identityKeys) {
        return new MultiMap<K, V>(identityKeys ? Maps.newIdentityHashMap() : new HashMap<>()) {
            ...
            public boolean remove(K key, V value) {
                ...
                int i = Lists.indexOfIdentity(elements, value);
                if (i >= 0) {
                    elements.remove(i);
                    if (elements.isEmpty()) {
                        remove(key);
