...
public class OutOfMemoryDialog extends DialogWrapper {
    ...
    public OutOfMemoryDialog(@Nonnull MemoryKind memoryKind) {
        ...
        myMessageLabel.setText(ExternalServiceLocalize.diagnosticOutOfMemoryError(memoryKind.optionName).get());
        myMessageLabel.setBorder(JBUI.Borders.emptyBottom(10));

        File file = VMOptions.getWriteFile();
        if (file != null) {
            mySettingsFileHintLabel.setText(ExternalServiceLocalize.diagnosticOutOfMemoryWillbesavedto(file.getPath()).get());
        ...
        myContinueAction = new DialogWrapperAction(ExternalServiceLocalize.diagnosticOutOfMemoryContinue()) {
            @Override
        ...
        myHeapDumpAction = !heapDump ? null : new DialogWrapperAction(ExternalServiceLocalize.diagnosticOutOfMemoryDump()) {
            @Override
            protected void doAction(ActionEvent e) {
                snapshot();
            }
    ...
    private void configControls(
        ...
        currentLabel.setText(ExternalServiceLocalize.diagnosticOutOfMemoryCurrentvalue(format(effective)).get());
    ...
    private static String format(int value) {
        return value == -1 ? ExternalServiceLocalize.diagnosticOutOfMemoryCurrentvalueUnknown().get() : String.valueOf(value);
    }
