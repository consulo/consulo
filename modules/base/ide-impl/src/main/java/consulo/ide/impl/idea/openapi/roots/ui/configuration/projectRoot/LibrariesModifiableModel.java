...
public class LibrariesModifiableModel implements LibraryTableBase.ModifiableModelEx {
    ...
    private final Map<Library, ExistingLibraryEditor> myLibrary2EditorMap = Maps.newHashMap(HashingStrategy.identity());
    private final Set<Library> myRemovedLibraries = Sets.newHashSet(HashingStrategy.identity());
    ...
    public void deferredCommit() {
        List<ExistingLibraryEditor> libraryEditors = new ArrayList<>(myLibrary2EditorMap.values());
        ...
        if (!libraryEditors.isEmpty() || !myRemovedLibraries.isEmpty()
            || myLibrariesModifiableModel != null && myLibrariesModifiableModel.isChanged()) {
    ...
    public void disposeUncommittedLibraries() {
        for (Library library : new ArrayList<>(myLibrary2EditorMap.keySet())) {
            Library existingLibrary = myTable.getLibraryByName(library.getName());
            if (existingLibrary != library) {
                Disposer.dispose(library);
            }
