...
public class FileUtilRt {
    ...
    public static final FileFilter ALL_DIRECTORIES = File::isDirectory;
    ...
    protected static String toCanonicalPath(
        ...
        if (index == -1) {
            if (removeLastSlash) {
                ...
                return slashIndex != -1 && slashIndex > start && slashIndex == path.length() - 1 ? path.substring(
                    0,
                    path.length() - 1
                ) : path;
            }
    ...
    public static boolean extensionEquals(@Nonnull String filePath, @Nonnull String extension) {
        ...
        return extStart >= 1 && filePath.charAt(extStart - 1) == '.' && filePath.regionMatches(
    ...
    @Nonnull
    private static File doCreateTempFile(
        @Nonnull File dir,
        @Nonnull String prefix,
        @Nullable String suffix,
        boolean isDirectory
    ...
    public static List<String> loadLines(@Nonnull String path, @Nullable String encoding) throws IOException {
        try (InputStream stream = new FileInputStream(path)) {
            try (BufferedReader reader = new BufferedReader(encoding == null ? new InputStreamReader(stream) : new InputStreamReader(
                stream,
                encoding
            ))) {
    ...
    private interface CharComparingStrategy {
        ...
        CharComparingStrategy CASE_INSENSITIVE = StringUtil::charsEqualIgnoreCase;
