...
public class EditorActionManagerImpl extends EditorActionManager {
    ...
    public ReadonlyFragmentModificationHandler getReadonlyFragmentModificationHandler(Document document) {
        Document doc = document instanceof DocumentWindow documentWindow ? documentWindow.getDelegate() : document;
        ReadonlyFragmentModificationHandler docHandler =
            doc instanceof DocumentEx documentEx ? documentEx.getReadonlyFragmentModificationHandler() : null;
    ...
    public void setReadonlyFragmentModificationHandler(Document document, ReadonlyFragmentModificationHandler handler) {
        Document doc = document instanceof DocumentWindow documentWindow ? documentWindow.getDelegate() : document;
        if (doc instanceof DocumentEx documentEx) {
            documentEx.setReadonlyFragmentModificationHandler(handler);
        }
    }
    ...
    private static class DefaultReadOnlyFragmentModificationHandler implements ReadonlyFragmentModificationHandler {
        ...
        public void handle(ReadOnlyFragmentModificationException e) {
            Messages.showErrorDialog(
                CodeEditorLocalize.guardedBlockModificationAttemptErrorMessage().get(),
                CodeEditorLocalize.guardedBlockModificationAttemptErrorTitle().get()
            );
        }
    }
}
