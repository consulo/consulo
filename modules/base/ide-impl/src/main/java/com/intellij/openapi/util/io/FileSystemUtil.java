  ...
  private static class Nio2MediatorImpl implements Mediator {
    ...
    public FileAttributes getAttributes(@Nonnull String pathStr) {
      try {
        ...
        Class<? extends BasicFileAttributes> schema = isWindows ? DosFileAttributes.class : PosixFileAttributes.class;
        BasicFileAttributes attributes = Files.readAttributes(path, schema, myNoFollowLinkOptions);
        boolean isSymbolicLink = attributes.isSymbolicLink() || isWindows && attributes.isOther() && attributes.isDirectory() && path.getParent() != null;
        if (isSymbolicLink) {
          try {
