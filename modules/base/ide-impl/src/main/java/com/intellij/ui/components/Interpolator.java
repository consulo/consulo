...
public class Interpolator {
   ...
   */
  public Interpolator(@Nonnull Supplier<Integer> input, @Nonnull Consumer<? super Integer> output) {
    myInput = input;
    myOutput = output;
  }
  ...
  public void setTarget(int value, int delay) {
    ...
    if (mySegments.isEmpty()) {
      mySegments.add(new Line(new Position(moment, myInput.get()), new Position(moment + delay * 1000000L, value)));
    }
    else {
      Segment segment = mySegments.getLast();
      mySegments.add(new Line(new Position(segment.getEnd().getMoment(), segment.getEnd().getValue()), new Position(segment.getEnd().getMoment() + elapsed, value)));
    }
