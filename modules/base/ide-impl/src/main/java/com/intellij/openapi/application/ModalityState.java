...
public abstract class ModalityState implements consulo.ui.ModalityState {
  ...
  static {
    try {
      @SuppressWarnings("unchecked") final Class<? extends ModalityState> ex = (Class<? extends ModalityState>)Class.forName("com.intellij.openapi.application.impl.ModalityStateEx");
  ...
  public static ModalityState current() {
    return (ModalityState)ApplicationManager.getApplication().getCurrentModalityState();
  }

  @Nonnull
  public static ModalityState any() {
    return (ModalityState)ApplicationManager.getApplication().getAnyModalityState();
  }

  @Nonnull
  public static ModalityState stateForComponent(Component component) {
    return (ModalityState)ApplicationManager.getApplication().getModalityStateForComponent(component);
  }

  @Nonnull
  public static ModalityState defaultModalityState() {
    return (ModalityState)ApplicationManager.getApplication().getDefaultModalityState();
  }
  ...
  public boolean dominates(@Nonnull consulo.ui.ModalityState anotherState) {
    return dominates((ModalityState)anotherState);
  }
}
