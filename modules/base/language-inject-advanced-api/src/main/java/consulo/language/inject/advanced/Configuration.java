...
import consulo.language.inject.advanced.internal.ApplicationInjectionConfiguration;
...
public class Configuration implements PersistentStateComponent<Element>, ModificationTracker {
    ...
    public static List<BaseInjection> loadDefaultInjections() {
        ...
        application.getExtensionPoint(InjectionConfigProvider.class).forEachExtensionSafe(provider -> {
            try (InputStream inputStream = provider.openConfigFileStream()) {
                ...
                PluginExceptionUtil.logPluginError(LOG, t.getClass().getName(), t, provider.getClass());
            }
        });
    ...
    public void setInjections(Collection<BaseInjection> injections) {
        for (BaseInjection injection : injections) {
