...
class FilePointerPartNode {
    ...
    private FilePointerPartNode findChildByNameId(int nameId, boolean createIfNotFound) {
        ...
        if (createIfNotFound) {
            ...
            assert index < 0
                ...
                "; UrlPart.nameEquals: " +
                FileImplUtil.PATH_CHAR_SEQUENCE_HASHING_STRATEGY.equals(child.getName(), fromNameId(nameId)) +
    ...
    void checkConsistency() {
        if (VirtualFilePointerManagerImpl.IS_UNDER_UNIT_TEST) {
            doCheckConsistency();
        }
    }
    ...
    private void doCheckConsistency() {
        ...
        if (fileAndUrl != null && fileAndUrl.second != null) {
            ...
            assert StringUtil.equal(nameFromPath, name, Platform.current().fs().isCaseSensitive())
