    ...
    for (TextRange rangeToProcess : myOuterAndRemoveRanges) {
      ...
      if (rangeToProcess instanceof RangeToRemove) {
        if (currentLeafOrLazyParseable == null) {
          Logger.getInstance(TemplateDataElementType.RangeCollector.class)
                  .error("RangeToRemove's range is out of original text bound", new Attachment("myOuterAndRemoveRanges", StringUtil.join(myOuterAndRemoveRanges, TextRange::toString, ", ")),
                         new Attachment("rangeToProcess", rangeToProcess.toString()), new Attachment("sourceCode", sourceCode.toString()));
          continue;
        }
