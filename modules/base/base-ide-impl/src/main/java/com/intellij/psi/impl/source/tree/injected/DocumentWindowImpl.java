    ...
    synchronized (myLock) {
      ...
      for (int i = 0; i < myShreds.size(); i++) {
        ...
        if (range.contains(offset) || range.getEndOffset() == offset/* in case of inserting at the end*/) {
          ...
          CharSequence toReplace = i == myShreds.size() - 1 || range.getEndOffset() + shred.getSuffix().length() >= endOffset ? s : s.subSequence(0, Math.min(hostRangeToModify.getLength(), s.length()));
  ...
  private int injectedToHost(int offset, boolean preferLeftFragment, boolean skipEmptyShreds) {
    synchronized (myLock) {
      ...
      for (int i = 0; i < myShreds.size(); i++) {
        ...
        if (skipEmptyShreds && !preferLeftFragment && currentStart == currentEnd && shred.getPrefix().isEmpty() && shred.getSuffix().isEmpty()) {
          continue;
        }
