...
public class ReplaceInProjectManager {
  ...
  private void addReplaceActions(final ReplaceContext replaceContext) {
    ...
    final AbstractAction replaceSelectedAction = new AbstractAction() {
      ...
      public Object getValue(String key) {
        return Action.NAME.equals(key) ? FindBundle.message("find.replace.selected.action", replaceContext.getUsageView().getSelectedUsages().size()) : super.getValue(key);
      }
    ...
    final AbstractAction replaceAllInThisFileAction = new AbstractAction() {
      ...
      @Override
      public Object getValue(String key) {
        return Action.NAME.equals(key) ? FindBundle.message("find.replace.this.file.action", replaceContext.getUsageView().getSelectedUsages().size()) : super.getValue(key);
      }
    ...
    final AbstractAction skipThisFileAction = new AbstractAction() {
      ...
      @Override
      public Object getValue(String key) {
        return Action.NAME.equals(key) ? FindBundle.message("find.replace.skip.this.file.action", replaceContext.getUsageView().getSelectedUsages().size()) : super.getValue(key);
      }
  ...
  private boolean getStringToReplace(int textOffset, int textEndOffset, Document document, FindModel findModel, Ref<? super String> stringToReplace)
          throws FindManager.MalformedReplacementStringException {
    ...
    public UsageSearcher create() {
      return processor -> {
        try {
          ...
          FindInProjectUtil.findUsages(myFindModelCopy, myProject, new AdapterProcessor<>(processor, UsageInfo2UsageAdapter.CONVERTER), myProcessPresentation);
        }
        finally {
