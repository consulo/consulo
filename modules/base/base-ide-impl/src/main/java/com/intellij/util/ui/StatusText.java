...
public abstract class StatusText {
  ...
  @Nullable
  private static ActionListener findListener(@Nonnull SimpleColoredComponent component, @Nonnull List<? extends ActionListener> listeners, int xCoord) {
  ...
  protected Rectangle adjustComponentBounds(@Nonnull JComponent component, @Nonnull Rectangle bounds) {
    ...
    if (myVerticalFlow) {
      ...
      return component == myComponent ? new Rectangle(bounds.x, bounds.y, size.width, size.height) : new Rectangle(bounds.x + bounds.width - size.width, bounds.y, size.width, size.height);
    }
  }
  ...
  public Dimension getPreferredSize() {
    ...
    if (myVerticalFlow) {
      return new Dimension(Math.max(componentSize.width, secondaryComponentSize.width), componentSize.height + secondaryComponentSize.height + JBUIScale.scale(Y_GAP));
    }
    else {
