...
public class CodeEditorCaretBase extends UserDataHolderBase implements Caret, Dumpable {
    ...
    if (locateBeforeSoftWrap && SoftWrapHelper.isCaretAfterSoftWrap(this)) {
      ...
      if (lineToUse >= 0) {
        ...
        if (tmpOffset == newOffset) {
          ...
          LOG.error("Invalid editor dimension mapping", new Throwable(), AttachmentFactory.createContext("Expected to map visual position '" +
                                                                                                         visualPosition +
                                                                                                         "' to offset " +
                                                                                                         newOffset +
  ...
  public void selectLineAtCaret() {
    validateContext(true);
    myCaretModel.doWithCaretMerging(() -> DesktopSelectionModelImpl.doSelectLineAtCaret(this));
  }
  ...
  class PositionMarker extends RangeMarkerImpl {
    ...
    protected void changedUpdateImpl(@Nonnull DocumentEvent e) {
      ...
      if (isValid()) {
        ...
        if (e.getOldLength() == 0 && oldOffset == e.getOffset() && !Boolean.TRUE.equals(myEditor.getUserData(EditorEx.DISABLE_CARET_SHIFT_ON_WHITESPACE_INSERTION)) && needToShiftWhiteSpaces(e)) {
