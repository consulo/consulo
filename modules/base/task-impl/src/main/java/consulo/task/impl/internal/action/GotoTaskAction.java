 ...
 */
@ActionImpl(id = GotoTaskActionInternal.ID)
public class GotoTaskAction extends GotoActionBase implements DumbAware {
        ...
        final ChooseByNamePopup popup = ChooseByNamePopup.createPopup(
            ...
            new ChooseByNameItemProvider() {
                ...
                public boolean filterElements(
                    ...
                    GotoTaskActionInternal.CREATE_NEW_TASK_ACTION.setTaskName(pattern);
                    if (!consumer.test(GotoTaskActionInternal.CREATE_NEW_TASK_ACTION)) {
                        return false;
                    }
        ...
        showNavigationPopup(
            new GotoActionCallback<>() {
                ...
                public void elementChosen(ChooseByNamePopup popup, Object element) {
                    ...
                    else if (element == GotoTaskActionInternal.CREATE_NEW_TASK_ACTION) {
                        LocalTask localTask = taskManager.createLocalTask(GotoTaskActionInternal.CREATE_NEW_TASK_ACTION.getTaskName());
    ...
    private static boolean processTasks(List<Task> tasks, Predicate<Object> consumer, ProgressIndicator cancelled, PsiManager psiManager) {
        for (Task task : tasks) {
            ...
            if (!consumer.test(new TaskPsiElementImpl(psiManager, task))) {
                return false;
            }
        }
        return true;
    ...
    private static class GotoTaskPopupModel extends SimpleChooseByNameModel {
        ...
        public String getElementName(Object element) {
            if (element instanceof TaskPsiElementImpl taskPsiElem) {
                return TaskUtil.getTrimmedSummary(taskPsiElem.getTask());
            }
            else if (element == GotoTaskActionInternal.CREATE_NEW_TASK_ACTION) {
                return GotoTaskActionInternal.CREATE_NEW_TASK_ACTION.getActionText().get();
            }
            return null;
        }
