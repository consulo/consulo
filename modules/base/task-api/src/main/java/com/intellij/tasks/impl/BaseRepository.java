...
public abstract class BaseRepository extends TaskRepository
{
	...
	public boolean equals(Object o)
	{
		...
		if(!Comparing.equal(getUrl(), that.getUrl()))
		{
			return false;
		}
		...
		if(!Comparing.equal(getUsername(), that.getUsername()))
		{
			return false;
		}
		if(!Comparing.equal(isLoginAnonymously(), that.isLoginAnonymously()))
		{
			return false;
		}
		if(!Comparing.equal(isUseProxy(), that.isUseProxy()))
		{
			return false;
		}
		if(!Comparing.equal(isUseHttpAuthentication(), that.isUseHttpAuthentication()))
		{
			return false;
		}
