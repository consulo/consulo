...
public final class SmartTodoItemPointer {
    ...
    public boolean equals(@Nullable Object obj) {
        if (!(obj instanceof SmartTodoItemPointer that)) {
            return false;
        }
        if (!myTodoItem.getFile().equals(that.myTodoItem.getFile())
            || myRangeMarker.getStartOffset() != that.myRangeMarker.getStartOffset()
            || myRangeMarker.getEndOffset() != that.myRangeMarker.getEndOffset()
            || !myTodoItem.getPattern().equals(that.myTodoItem.getPattern())
            || myAdditionalRangeMarkers.size() != that.myAdditionalRangeMarkers.size()) {
            return false;
        }
        for (int i = 0; i < myAdditionalRangeMarkers.size(); i++) {
            RangeMarker m1 = myAdditionalRangeMarkers.get(i);
            RangeMarker m2 = that.myAdditionalRangeMarkers.get(i);
