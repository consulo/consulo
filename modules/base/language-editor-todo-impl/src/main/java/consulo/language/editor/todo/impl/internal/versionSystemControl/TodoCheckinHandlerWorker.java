...
import consulo.language.editor.todo.impl.internal.search.TodoItemsCreator;
...
public class TodoCheckinHandlerWorker {
    ...
    private static class MyEditedFileProcessor {
        ...
        private void checkEditedFragment(TodoItem newTodoItem) {
            if (myBeforeFile == null) {
                myBeforeFile = ApplicationManager.getApplication().runReadAction(new Supplier<PsiFile>() {
                    @Override
                    public PsiFile get() {
            ...
            if (myOldTodoTexts == null) {
                StepIntersection<LineFragment, TodoItem> intersection = new StepIntersection<LineFragment, TodoItem>(
                    LineFragmentConvertor.getInstance(), TodoItemConvertor.getInstance(), myOldItems, () -> myBeforeContent);
                myOldTodoTexts = new HashSet<String>();
                intersection.process(Collections.singletonList(myCurrentLineFragment), (lineFragment, todoItem) -> myOldTodoTexts.add(getTodoText(todoItem, myBeforeContent)));
            }
    ...
    private static class TodoItemConvertor implements Function<TodoItem, TextRange> {
        ...
        @Override
        public TextRange apply(TodoItem o) {
            TextRange textRange = o.getTextRange();
            return new TextRange(textRange.getStartOffset(), textRange.getEndOffset() - 1);
        }
    }

    private static class LineFragmentConvertor implements Function<LineFragment, TextRange> {
        ...
        @Override
        public TextRange apply(LineFragment o) {
