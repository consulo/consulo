...
public abstract class TodoTreeBuilder implements Disposable {
    ...
    public Iterator<PsiFile> getFiles(PsiDirectory psiDirectory, boolean skip) {
        ...
        for (VirtualFile file : files) {
            Module module = psiDirectory.getModule();
            if (module != null) {
    ...
    void collectFiles(Predicate<? super VirtualFile> collector) {
        ...
        for (PsiFile psiFile : psiFiles) {
            if (mySearchHelper.getTodoItemsCount(psiFile) > 0 && treeStructure.accept(psiFile)) {
                collector.test(psiFile.getVirtualFile());
            }
        }
    }
    ...
    public TodoItemNode getFirstPointerForElement(@Nullable Object element) {
        if (element instanceof TodoItemNode todoItemNode) {
            ...
            if (firstChild instanceof TodoItemNode todoItemNode) {
                return todoItemNode;
            }
            else {
    ...
    private static TodoNodeVisitor getVisitorFor(Object obj) {
        if (obj instanceof TodoItemNode todoItemNode) {
            SmartTodoItemPointer value = todoItemNode.getValue();
            if (value != null) {
                return new TodoNodeVisitor(value::getTodoItem, value.getTodoItem().getFile().getVirtualFile());
            }
        }
        else {
            Object o = obj instanceof AbstractTreeNode abstractTreeNode ? abstractTreeNode.getValue() : null;
            return new TodoNodeVisitor(
                () -> obj instanceof AbstractTreeNode abstractTreeNode ? abstractTreeNode.getValue() : obj,
                o instanceof PsiElement element ? PsiUtilCore.getVirtualFile(element) : null
            );
        }
        return null;
    }
    ...
    static PsiFile getFileForNode(DefaultMutableTreeNode node) {
        ...
        if (obj instanceof TodoFileNode todoFileNode) {
            return todoFileNode.getValue();
        }
        else if (obj instanceof TodoItemNode todoItemNode) {
            SmartTodoItemPointer pointer = todoItemNode.getValue();
    ...
    public TodoItemNode getNextPointer(TodoItemNode pointer) {
        ...
        if (sibling instanceof TodoItemNode todoItemNode) {
            return todoItemNode;
        }
        else {
    ...
    private final class MyPsiTreeChangeListener extends PsiTreeChangeAdapter {
        ...
        public void childAdded(@Nonnull PsiTreeChangeEvent e) {
            ...
            if (e.getChild() instanceof PsiFile psiFile) {
        ...
        public void beforeChildRemoval(@Nonnull PsiTreeChangeEvent e) {
            ...
            else if (child instanceof PsiDirectory psiDirectory) { // directory will be removed
            ...
            else if (PsiTreeUtil.getParentOfType(child, PsiComment.class, false) != null) { // change inside comment
        ...
        public void childMoved(@Nonnull PsiTreeChangeEvent e) {
            ...
            if (e.getChild() instanceof PsiFile psiFile) { // file was moved
            ...
            else if (e.getChild() instanceof PsiDirectory psiDirectory) { // directory was moved. mark all its files as dirty.
        ...
        public void propertyChanged(@Nonnull PsiTreeChangeEvent e) {
            ...
            if (propertyName.equals(PsiTreeChangeEvent.PROP_ROOTS)) { // rebuild all tree when source roots were changed
                myModel.getInvoker().runOrInvokeLater(
                    () -> DumbService.getInstance(myProject).runWhenSmart(TodoTreeBuilder.this::rebuildCache)
                );
                updateTree();
            }
