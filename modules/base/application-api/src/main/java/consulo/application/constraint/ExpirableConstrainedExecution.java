...
public abstract class ExpirableConstrainedExecution<E extends ConstrainedExecution<E>> extends BaseConstrainedExecution<E> {
    ...
    public class ExpirableContextConstraint implements ContextConstraint {
        ...
        public void schedule(Runnable runnable) {
            ...
            Runnable invokeWhenCompletelyExpiredRunnable = new Runnable() {
                ...
                public void run() {
                    if (expiration.isExpired()) {
                        // At this point all the expiration handlers, including the one
                        // responsible for cancelling the coroutine job, have finished.
                        runOnce.invoke(runnable::run);
                    }
            ...
            if (runOnce.isActive()) {
                constraint.schedule(() -> runOnce.invoke(() -> {
    ...
    public ExpirableConstrainedExecution(
    ...
    protected abstract E cloneWith(
        ContextConstraint[] constraints,
        BooleanSupplier[] cancellationConditions,
        Set<? extends Expiration> expirationSet
    );
    ...
    public E withConstraint(ContextConstraint constraint, Disposable parentDisposable) {
        ...
        return cloneWith(
            ArrayUtil.append(constraints, expirableConstraint),
            cancellationConditions,
            appendSet(this.expirationSet, expirableHandle)
        );
    }

    private final Supplier<@Nullable Expiration> compositeExpiration =
        LazyValue.nullable(() -> ExpirationUtil.composeExpiration(getExpirationSet()));
