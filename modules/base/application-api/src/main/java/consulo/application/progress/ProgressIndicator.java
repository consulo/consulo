 ...
 * about to occur in the UI thread, to avoid UI freezes (see {@link consulo.application.ReadAction#nonBlocking(Runnable)}).
 ...
 * <li>{@link consulo.application.internal.ProgressIndicatorBase}: invisible, but can be made visible by subclassing:
 * remembers text/fraction inside and allows to retrieve them and possibly show in the UI. Non-modal by default.</li>
 * <li>{@link consulo.application.impl.internal.progress.ProgressWindow}: visible progress, either modal or background. Usually not created directly,
 * instantiated internally inside {@link ProgressManager#run}.</li>
 * <li>{@link consulo.application.internal.ProgressWrapper}: wraps an existing progress indicator, usually to fork another thread
 * with the same cancellation policy. Use {@link consulo.application.internal.SensitiveProgressWrapper} to allow
