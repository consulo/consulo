...
@Singleton
@ServiceImpl
public class BookmarkManagerImpl implements BookmarkManagerInternal, PersistentStateComponent<Element> {
    ...
    static void documentCreated(Document document, PsiFile ignored, Project project) {
        ...
        for (BookmarkImpl bookmark : bookmarkManager.myBookmarks) {
    ...
    @Override
    @RequiredUIAccess
    public void editDescription(Bookmark bookmark) {
    ...
    private List<BookmarkImpl> readExternal(Element element) {
        ...
        for (Element bookmarkElement : element.getChildren()) {
    ...
    public void setMnemonic(Bookmark bookmark, char c) {
        BookmarkImpl old = findBookmarkForMnemonic(c);
        if (old != null) {
            removeBookmark(old);
        }
    ...
    private class MyEditorMouseListener implements EditorMouseListener {
        ...
        public void mouseClicked(EditorMouseEvent e) {
            ...
            if ((e.getMouseEvent().getModifiers() & (Platform.current().os().isMac() ? InputEvent.META_MASK : InputEvent.CTRL_MASK)) == 0) {
                return;
            }
