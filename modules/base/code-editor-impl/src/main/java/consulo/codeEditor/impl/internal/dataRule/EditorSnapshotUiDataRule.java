 ...
 */
@ExtensionImpl
public class EditorSnapshotUiDataRule implements UiDataRule {
    @Override
    public void uiDataSnapshot(DataSink sink, DataSnapshot snapshot) {
        sink.lazyValue(EditorKeys.EDITOR_SNAPSHOT, dataSnapshot -> {
            Editor editor = dataSnapshot.get(Editor.KEY);
            return editor == null ? null : ImaginaryEditor.create(editor);
        });
    }
}
