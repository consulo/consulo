    ...
    public void uiDataSnapshot(DataSink sink, DataSnapshot snapshot) {
        ...
        if (editor instanceof EditorEx editorEx) {
            ...
            if (!editorEx.isViewer()) {
                sink.set(CutProvider.KEY, editorEx.getCutProvider());
                sink.set(PasteProvider.KEY, editorEx.getPasteProvider());
            }
        }
        if (editor != null) {
            sink.set(EditorKeys.EDITOR_EVEN_IF_INACTIVE, editor);
        }
    }
}
