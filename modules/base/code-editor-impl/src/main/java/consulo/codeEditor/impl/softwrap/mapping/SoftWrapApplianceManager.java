...
public abstract class SoftWrapApplianceManager implements Dumpable {
    public static final Key<Object> IGNORE_OLD_SOFT_WRAP_LOGIC_REGISTRY_OPTION =
        Key.create("softWrap.ignoreOldSoftWrapLogicRegistryOption");
        ...
        try {
            for (Segment range : ranges) {
                ...
                if (range.getEndOffset() > lastOffset) {
                    recalculateSoftWraps(new IncrementalCacheUpdateEvent(
                        Math.max(range.getStartOffset(), lastOffset),
                        range.getEndOffset(),
                        myEditor
                    ));
                }
    ...
    protected void recalculateSoftWraps(@Nonnull IncrementalCacheUpdateEvent event) {
        ...
        if (myInProgress) {
            LOG.error(
                "Detected race condition at soft wraps recalculation",
                new Throwable(),
                AttachmentFactoryUtil.createContext(myEditor.dumpState(), event)
            );
        }
        myInProgress = true;
        try {
            ...
            if (LOG.isDebugEnabled()) {
                LOG.debug(
                    "Soft wrap recalculation done: " + event.toString() + ". " +
                        (event.getActualEndOffset() - event.getStartOffset()) + " characters processed"
                );
            }
            if (event.getActualEndOffset() > endOffsetUpperEstimate) {
                LOG.error(
                    "Unexpected error at soft wrap recalculation",
                    AttachmentFactory.get().create("softWrapModel.txt", myEditor.getSoftWrapModel().toString())
                );
            }
    ...
    protected void doRecalculateSoftWraps(IncrementalCacheUpdateEvent event, int endOffsetUpperEstimate) {
        ...
        EditorTextRepresentationHelper textHelper = CodeEditorSoftWrapModelBase.getEditorTextRepresentationHelper(myEditor);
        if (textHelper instanceof DefaultEditorTextRepresentationHelper defaultTextHelper) {
            defaultTextHelper.updateContext();
        }
        ...
        myContext.afterLineEndInlays = myEditor.getInlayModel()
            .getAfterLineEndElementsInRange(DocumentUtil.getLineStartOffset(start, document), endOffsetUpperEstimate);
    ...
    protected void doRecalculateSoftWrapsRoughly(IncrementalCacheUpdateEvent event) {
        ...
        while (true) {
            ...
            if (offset < document.getLineEndOffset(line)) {
                ...
                if (offset > mandatoryEnd && myDataMapper.matchesOldSoftWrap(wrap, event.getLengthDiff())) {
                    break;
                }
            }
        }
    ...
    private boolean processCollapsedFoldRegion(FoldRegion foldRegion) {
        ...
        if (!foldRegion.isValid()
            ...
            LOG.error(
                "Inconsistent fold region state: fold region: " + foldRegion +
        ...
        for (int i = 0; i < placeholder.length(); i++) {
            ...
            placeholderWidthInPixels +=
                CodeEditorSoftWrapModelBase.getEditorTextRepresentationHelper(myEditor).charWidth(c, myContext.fontType);
        }
        ...
        if (myContext.delayedSoftWrap == null
            && myContext.exceedsVisualEdge(myContext.currentPosition.x + myContext.reservedWidthInPixels)) {
            softWrap = registerSoftWrap(
                myContext.softWrapStartOffset,
                myContext.tokenStartOffset,
                myContext.tokenStartOffset,
    ...
    private boolean processNonFoldToken() {
        ...
        while (myContext.currentPosition.offset < myContext.tokenEndOffset) {
            if (counter++ > limit) {
                LOG.error(
                    "Cycled soft wraps recalculation detected",
                    new Throwable(),
                    AttachmentFactoryUtil.createContext(String.format(
            ...
            if (myContext.skipToLineEnd) {
                myContext.skipToLineEnd =
                    false; // Assuming that this flag is set if no soft wrap is registered during processing the call below
    ...
    private SoftWrapImpl registerSoftWrap(int minOffset, int preferredOffset, int maxOffset, int spaceSize, LogicalLineData lineData) {
        ...
        if (softWrapOffset < 0) {
            ...
            if (DocumentUtil.isInsideSurrogatePair(document, softWrapOffset)) {
                softWrapOffset--;
            }
        }
        ...
        if (softWrapOffset >= lineData.endLineOffset
            ...
            // Prefer to wrap on fold region backwards to wrapping forwards.
            || softWrapOffset > preferredOffset && myContext.lastFoldStartPosition != null
            && myContext.lastFoldStartPosition.offset <= preferredOffset) {
            return null;
        }
    ...
    public boolean recalculateIfNecessary() {
        ...
        if (myVisibleAreaWidth != QUICK_DUMMY_WRAPPING) {
            // don't scroll after soft-wrap recalculation if it's the first time editor is showing
    ...
    private static class DefaultVisibleAreaWidthProvider implements VisibleAreaWidthProvider {
        ...
        public int getVisibleAreaWidth() {
            ...
            if (myEditor.isInDistractionFreeMode()) {
                ...
                if (rightMargin > 0) {
                    width = Math.min(width, rightMargin * EditorImplUtil.getPlainSpaceWidth(myEditor));
                }
            }
            return width;
        }
    ...
    private class ProcessingContext {
        ...
        @Override
        public String toString() {
            return "reserved width: " + reservedWidthInPixels +
                ", soft wrap start offset: " + softWrapStartOffset +
                ", range end offset: " + rangeEndOffset +
                ", token offsets: [" + tokenStartOffset + "; " + tokenEndOffset + "]" +
                ", font type: " + fontType +
                ", skip to line end: " + skipToLineEnd +
                ", delayed soft wrap: " + delayedSoftWrap +
        ...
        void onNonLineFeedSymbol(int codePoint, int[] metrics) { // {newX, actualWidth}
            ...
            if (currentPosition.offset - myOffset2widthInPixels.anchor >= myOffset2widthInPixels.data.length) {
                int newLength =
                    Math.max(myOffset2widthInPixels.data.length * 2, currentPosition.offset - myOffset2widthInPixels.anchor + 1);
        ...
        private int getAfterLineEndInlaysWidth(int logicalLine) {
            ...
            while (afterLineEndInlayIndex < afterLineEndInlays.size() && afterLineEndInlays.get(afterLineEndInlayIndex)
