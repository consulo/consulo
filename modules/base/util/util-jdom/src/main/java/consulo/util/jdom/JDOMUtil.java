...
public class JDOMUtil {
    ...
    public static int getTreeHash(@Nullable Element root) {
        return getTreeHash(root, false);
    }

    public static int getTreeHash(@Nullable Element root, boolean skipEmptyText) {
    ...
    private static int addToHash(int i, @Nullable Element element, boolean skipEmptyText) {
        if (element == null) {
            return i;
        }
        ...
        for (Content child : element.getContent()) {
            if (child instanceof Element childElem) {
                i = addToHash(i, childElem, skipEmptyText);
            }
    ...
    public static void addContent(Element targetElement, Object node) {
     ...
     */
    //public static void internStringsInElement(Element element, StringInterner interner) {
    ...
    //    }
    //    else if (o instanceof Comment comment) {
    ...
    private static class EmptyTextFilter extends AbstractFilter<Content> {
        ...
        public @Nullable Content filter(Object obj) {
            if (obj instanceof Text text && CharArrayUtil.containsOnlyWhiteSpaces(text.getText())) {
                return null;
            }
            return (Content) obj;
        }
    ...
    private static boolean contentsEqual(Content c1, Content c2) {
        ...
        return c1 instanceof Element e1 && c2 instanceof Element e2 && areElementsEqual(e1, e2);
    }

    private static boolean attListsEqual(List<Attribute> a1, List<Attribute> a2) {
        int size = a1.size();
        if (size != a2.size()) {
            return false;
        }
        for (int i = 0; i < size; i++) {
            if (!attEqual(a1.get(i), a2.get(i))) {
                return false;
            }
        }
        return true;
    ...
    private static SAXBuilder getSaxBuilder() {
        ...
        if (saxBuilder == null) {
            ...
            saxBuilder.setEntityResolver((publicId, systemId) -> new InputSource(new CharArrayReader(ArrayUtil.EMPTY_CHAR_ARRAY)));
    ...
    public static void writeParent(Parent parent, Writer writer, String lineSeparator) throws IOException {
        if (parent instanceof Element parentElem) {
            writeElement(parentElem, writer, lineSeparator);
        }
        else if (parent instanceof Document parentDoc) {
            writeDocument(parentDoc, writer, lineSeparator);
        }
    }
    ...
    public static String writeChildren(@Nullable Element element, String lineSeparator) throws IOException {
        StringWriter writer = new StringWriter();
    ...
    private static @Nullable String escapeChar(char c, boolean escapeApostrophes, boolean escapeSpaces, boolean escapeLineEnds) {
        return switch (c) {
            case '\n' -> escapeLineEnds ? "&#10;" : null;
            case '\r' -> escapeLineEnds ? "&#13;" : null;
            case '\t' -> escapeLineEnds ? "&#9;" : null;
            case ' ' -> escapeSpaces ? "&#20" : null;
            case '<' -> "&lt;";
            case '>' -> "&gt;";
            case '\"' -> "&quot;";
            case '\'' -> escapeApostrophes ? "&apos;" : null;
            case '&' -> "&amp;";
            default -> null;
        };
    }
    ...
    public static void updateFileSet(
        File[] oldFiles,
        String[] newFilePaths,
