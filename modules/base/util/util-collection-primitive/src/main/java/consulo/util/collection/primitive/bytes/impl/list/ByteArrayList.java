...
package consulo.util.collection.primitive.bytes.impl.list;
...
public class ByteArrayList implements ByteList, Cloneable {
   ...
   * @param len    the number of values to copy.
   */
  public void toArray(byte[] dest, int offset, int len) {
    if (len == 0) {
   ...
   * @return true if the procedure did not terminate prematurely.
   */
  public boolean forEachDescending(IntPredicate procedure) {
    for (int i = _pos; i-- > 0; ) {
      if (!procedure.test(_data[i])) {
        return false;
      }
   ...
   */
  public ByteArrayList grep(IntPredicate condition) {
    ByteArrayList list = new ByteArrayList();
    for (int i = 0; i < _pos; i++) {
      if (condition.test(_data[i])) {
        list.add(_data[i]);
      }
    }
    return list;
   ...
   */
  public ByteArrayList inverseGrep(IntPredicate condition) {
    ByteArrayList list = new ByteArrayList();
    for (int i = 0; i < _pos; i++) {
      if (!condition.test(_data[i])) {
        list.add(_data[i]);
