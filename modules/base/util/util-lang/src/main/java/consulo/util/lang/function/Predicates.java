 ...
 * @since 12.05.2024
 *
 * Same like {@link Conditions} with {@link Predicate}
 */
...
public class Predicates {
  ...
  public static <T> Predicate<T> and2(Predicate<? super T> c1, Predicate<? super T> c2) {
    if (c1 == TRUE || c2 == FALSE) return (Predicate<T>)c2;
    if (c2 == TRUE || c1 == FALSE) return (Predicate<T>)c1;
  ...
  public static <T> Predicate<T> or2(Predicate<? super T> c1, Predicate<? super T> c2) {
    if (c1 == FALSE || c2 == TRUE) return (Predicate<T>)c2;
    if (c2 == FALSE || c1 == TRUE) return (Predicate<T>)c1;
    return new Or<>(c1, c2);
  }
