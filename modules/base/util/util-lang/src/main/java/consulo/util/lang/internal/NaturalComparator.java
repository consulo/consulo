...
public class NaturalComparator implements Comparator<String> {
  ...
  private static int naturalCompare(String s1, String s2, int length1, int length2, boolean ignoreCase) {
    ...
    for (; i < length1 && j < length2; i++, j++) {
      ...
      if ((StringUtil.isDecimalDigit(ch1) || ch1 == ' ') && (StringUtil.isDecimalDigit(ch2) || ch2 == ' ')) {
        int start1 = skipChar(s1, skipChar(s1, i, length1, ' '), length1, '0');
        int start2 = skipChar(s2, skipChar(s2, j, length2, ' '), length2, '0');

        int end1 = skipDigits(s1, start1, length1);
        int end2 = skipDigits(s2, start2, length2);

        // numbers with more digits are always greater than shorter numbers
        int lengthDiff = (end1 - start1) - (end2 - start2);
        if (lengthDiff != 0) return lengthDiff;

        // compare numbers with equal digit count
        int numberDiff = compareCharRange(s1, s2, start1, start2, end1);
        ...
        int leadingDiff = compareCharRange(s1, s2, i, j, start1);
        ...
        int diff = compareChars(ch1, ch2, ignoreCase);
  ...
  private static int compareCharRange(String s1, String s2, int offset1, int offset2, int end1) {
