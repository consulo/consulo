...
public class CharArrayUtil {
    ...
    public static void getChars(CharSequence src, char[] dst, int srcOffset, int dstOffset, int len) {
        ...
        if (len >= GET_CHARS_THRESHOLD) {
            if (src instanceof String str) {
                str.getChars(srcOffset, srcOffset + len, dst, dstOffset);
                return;
            }
            else if (src instanceof CharBuffer buffer) {
            ...
            else if (src instanceof StringBuffer buffer) {
                buffer.getChars(srcOffset, srcOffset + len, dst, dstOffset);
                return;
            }
            else if (src instanceof StringBuilder builder) {
                builder.getChars(srcOffset, srcOffset + len, dst, dstOffset);
                return;
            }
        }
    ...
    public static char[] fromSequenceWithoutCopying(@Nullable CharSequence seq) {
        ...
        if (seq instanceof CharBuffer buffer
            && buffer.hasArray()
            && !buffer.isReadOnly()
    ...
    public static int shiftForwardCarefully(CharSequence buffer, int offset, String chars) {
        ...
        if (!isSuitable(chars, buffer.charAt(offset))) {
            return offset;
        }
        offset++;
        while (true) {
    ...
    public static boolean regionMatches(CharSequence buffer, int start, int end, CharSequence s) {
        ...
        //if (buffer instanceof String bufferStr && s instanceof String str) {
        //    return bufferStr.regionMatches(offset, str, 0, len);
        //}

        for (int i = 0; i < len; i++) {
            if (buffer.charAt(start + i) != s.charAt(i)) {
                return false;
            }
        }
        return true;
    ...
    public static boolean equals(char[] buffer1, int start1, int end1, char[] buffer2, int start2, int end2) {
        ...
        for (int i = start1; i < end1; i++) {
            if (buffer1[i] != buffer2[i - start1 + start2]) {
                return false;
            }
        }
        return true;
