...
class MapBackedFMap implements KeyFMap {
  ...
  private IntObjectMap<Object> myMap = IntMaps.newIntObjectHashMap();
  ...
  MapBackedFMap(@Nonnull int[] keys, int newKey, @Nonnull Object[] values, @Nonnull Object newValue) {
    IntMaps.newIntObjectHashMap(keys.length + 1);
    ...
    assert myMap.size() > ArrayBackedFMap.ARRAY_THRESHOLD;
  }
