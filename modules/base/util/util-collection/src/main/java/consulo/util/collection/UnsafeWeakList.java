...
public class UnsafeWeakList<T> extends AbstractList<T> {
    private static final Logger LOG = LoggerFactory.getLogger(UnsafeWeakList.class);
    ...
    public List<T> toStrongList() {
        List<T> result = new ArrayList<>(myArray.size());
    ...
    protected class MyIterator implements Iterator<T> {
        ...
        public void remove() {
            ...
            LoggerAssert.assertTrue(LOG, myArray.get(myNextIndex) == myNextElement);
        }
    }
}
