...
public class SLRUMap<K, V> {
    ...
    public SLRUMap(int protectedQueueSize, int probationalQueueSize, HashingStrategy<K> hashingStrategy) {
        ...
        myProtectedQueue = new LinkedHashMap<>(10, 0.6f, hashingStrategy) {
            @Override
        ...
        myProbationalQueue = new LinkedHashMap<>(10, 0.6f, hashingStrategy) {
            @Override
            protected boolean removeEldestEntry(Map.Entry<K, V> eldest, K key, V value) {
    ...
    protected K getStableKey(K key) {
        if (key instanceof ShareableKey shareableKey) {
            return (K) shareableKey.getStableCopy();
        }
