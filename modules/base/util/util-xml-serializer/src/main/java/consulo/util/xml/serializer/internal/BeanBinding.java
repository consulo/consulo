...
public class BeanBinding extends Binding {
    ...
    public Object deserialize(Object context, Element element) {
        if (myBeanClass.isRecord()) {
            ...
            List<Class<?>> types = new ArrayList<>(components.length);
            for (RecordComponent component : components) {
                types.add(component.getType());
                values.put(component.getName(), DefaultValue.ourDefaultValues.get(component.getType()));
            }

            deserializeIntoObject(values, element, null);

            try {
                Constructor<?> constructor = myBeanClass.getDeclaredConstructor(types.toArray(Class[]::new));
                return constructor.newInstance(values.values().toArray());
            }
            catch (Throwable e) {
            ...
            deserializeIntoObject(instance, element, null);
            return instance;
        }
    }
    ...
    public Map<String, Float> computeBindingWeights(LinkedHashSet<String> accessorNameTracker) {
    ...
    public static List<MutableAccessor> getAccessors(Class<?> aClass) {
        ...
        if (aClass.isRecord()) {
            collectRecordComponentAccessors(aClass, accessors);
        }
        else {
    ...
    private static void collectRecordComponentAccessors(Class<?> aClass, List<MutableAccessor> accessors) {
        for (RecordComponent recordComponent : aClass.getRecordComponents()) {
            ...
            accessors.add(new RecordComponentAccessor(recordComponent));
        }
    }
