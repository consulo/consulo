...
import consulo.util.concurrent.coroutine.CoroutineEnvironment;
 ...
 * @author eso
 */
public abstract class CoroutineEnvironmentImpl extends UserDataHolderBase implements CoroutineEnvironment {
	...
	public <T> Channel<T> createChannel(ChannelId<T> id, int capacity) {
		return channelLock.supplyLocked(() -> {
			...
			Channel<T> aChannel = new ChannelImpl<>(id, capacity);
