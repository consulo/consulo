 ...
 * @author eso
 */
public class ContinuationImpl<T> extends UserDataHolderBase implements Continuation<T> {
     ...
     */
    public ContinuationImpl(CoroutineScope scope, Coroutine<?, T> coroutine) {
        this.scope = (CoroutineScopeImpl) scope;
        ...
        this.scope.coroutineStarted(this);
     ...
     */
    public final <V> void resumeAsync(@Nullable CoroutineStep<V, ?> resumeStep, @Nullable V value) {
        if (!cancelled) {
