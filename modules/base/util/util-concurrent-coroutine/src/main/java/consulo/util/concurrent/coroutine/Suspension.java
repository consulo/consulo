...
import consulo.util.concurrent.coroutine.internal.ContinuationImpl;
...
public class Suspension<T extends @Nullable Object> {
     ...
     */
    public Suspension(CoroutineStep<?, T> suspendingStep, @Nullable CoroutineStep<T, ?> resumeStep, Continuation<?> continuation) {
    ...
    public void resume(@Nullable T value) {
        if (!cancelled) {
            this.value = value;
            ((ContinuationImpl<?>) continuation).suspensionResumed(this);
     ...
     */
    void resumeAsync() {
        ((ContinuationImpl<?>) continuation).resumeAsync(resumeStep, value);
    }
}
