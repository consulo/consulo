...
public abstract class IntervalTreeImpl<T> extends RedBlackTree<T> implements IntervalTree<T> {
  ...
  public static class IntervalNode<E> extends RedBlackTree.Node<E> implements MutableInterval {
    ...
    private static final byte ATTACHED_TO_TREE_FLAG = Node.COLOR_MASK << 1; // true if the node is inserted to the tree
    public final List<Supplier<E>> intervals;
    public int maxEnd; // max of all intervalEnd()s among all children.
    public int delta;  // delta of startOffset. getStartOffset() = myStartOffset + Sum of deltas up to root
    ...
    //  private int modCount; // if it equals to the consulo.ide.impl.idea.openapi.editor.impl.RedBlackTree.modCount then deltaUpToRoot can be used, otherwise it is expired
    ...
    protected Supplier<E> createGetter(@Nonnull E interval) {
      return new WeakReferencedGetter<>(interval, myIntervalTree.myReferenceQueue);
    }

    private static class WeakReferencedGetter<T> extends WeakReference<T> implements Supplier<T> {
  ...
  @Nonnull
  protected abstract IntervalNode<T> createNewNode(@Nonnull T key, int start, int end, boolean greedyToLeft, boolean greedyToRight, boolean stickingToRight, int layer);
        ...
        private IntervalNode<T> nextNode(@Nonnull IntervalNode<T> root) {
          ...
          if (right != null && (nodeFilter == null || nodeFilter.test(right))) {
            ...
            if (startOffset <= rightMaxEnd) {
              ...
              IntervalNode<T> left;
              while ((left = right.getLeft()) != null && (nodeFilter == null || nodeFilter.test(left)) && startOffset <= maxEndOf(left, rightDelta)) {
                right = left;
  ...
  @Nonnull
  public IntervalTreeImpl.IntervalNode<T> addInterval(@Nonnull T interval, int start, int end, boolean greedyToLeft, boolean greedyToRight, boolean stickingToRight, int layer) {
    try {
      l.writeLock().lock();
      if (firingBeforeRemove) {
        throw new UnsupportedOperationException("Must not add rangemarker from within beforeRemoved listener");
      }
      checkMax(true);
  ...
  void changeData(@Nonnull T interval, int start, int end, boolean greedyToLeft, boolean greedyToRight, boolean stickingToRight, int layer) {
    try {
      l.writeLock().lock();
