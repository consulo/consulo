...
public class RangeMarkerImpl extends UserDataHolderBase implements RangeMarkerEx {
  ...
  public RangeMarkerImpl(@Nonnull DocumentEx document, int start, int end, boolean register, boolean forceDocumentStrongReference) {
    this(forceDocumentStrongReference ? document : ObjectUtil.notNull(FileDocumentManager.getInstance().getFile(document), document), document.getTextLength(), start, end, register, false, false);
  }
  ...
  private static int estimateDocumentLength(@Nonnull VirtualFile virtualFile) {
    Document document = FileDocumentManager.getInstance().getCachedDocument(virtualFile);
    return document == null ? Integer.MAX_VALUE : document.getTextLength();
  }

  private RangeMarkerImpl(@Nonnull Object documentOrFile, int documentTextLength, int start, int end, boolean register, boolean greedyToLeft, boolean greedyToRight) {
    if (start < 0) {
