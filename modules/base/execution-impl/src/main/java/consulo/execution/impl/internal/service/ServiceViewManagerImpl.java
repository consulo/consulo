...
public final class ServiceViewManagerImpl implements ServiceViewManager, PersistentStateComponent<ServiceViewManagerImpl.State> {
  ...
  public @Nonnull Promise<Boolean> trackingSelect(@Nonnull Object service, @Nonnull Class<?> contributorClass,
    ...
    viewModel.addModelListener(() -> {
      ...
      if (item != null && !viewModel.getChildren(item).isEmpty() && contentManager != null) {
        AppUIExecutor.onUiThread().expireWith(contentManager).submit(() -> {
          ...
          ServiceViewModel.ServiceListModel listModel =
            new ServiceViewModel.ServiceListModel(myModel, myModelFilter, new SmartList<>(viewItem),
  ...
  private void splitByType(ContentManager contentManager, ServiceViewContributor<?> contributor) {
    ...
    ServiceViewModel.ContributorModel contributorModel = new ServiceViewModel.ContributorModel(myModel, myModelFilter, contributor, null);
    extract(contentManager, contributorModel, prepareViewState(new ServiceViewState()), true);
  }

  public @Nonnull List<Object> getChildrenSafe(@Nonnull AnActionEvent e,
  ...
  private static final class ServiceViewContentMangerListener implements ContentManagerListener {
    ...
    ServiceViewContentMangerListener(@Nonnull ServiceModelFilter modelFilter,
  ...
  private Set<String> excludeServices(@Nonnull List<ServiceViewContributor<?>> toExclude,
  ...
  private Set<String> includeServices(@Nonnull List<ServiceViewContributor<?>> toInclude,
                                      @Nullable Collection<ServiceViewContributor<?>> services) {
