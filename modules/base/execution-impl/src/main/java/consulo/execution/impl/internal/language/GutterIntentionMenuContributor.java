...
import consulo.language.editor.internal.intention.GutterIntentionAction;
...
public class GutterIntentionMenuContributor implements IntentionMenuContributor {
    ...
    public void collectActions(@Nonnull Editor hostEditor, @Nonnull PsiFile hostFile, @Nonnull IntentionsInfo intentions, int passIdToShowIntentionsFor, int offset) {
        ...
        MarkupModelEx model = DocumentMarkupModel.forDocument(hostDocument, project, true);
        ...
        for (RangeHighlighterEx highlighter : result) {
            addActions(project, highlighter, intentions.guttersToShow, hostEditor.getDataContext());
        }
    }
    ...
    private static void addActions(@Nonnull AnAction action,
        ...
        if (action instanceof LineMarkerActionWrapper) {
            List<AnAction> children = Arrays.asList(((LineMarkerActionWrapper) action).getChildren(null));
