...
public class IncompatibleEncodingDialog extends DialogWrapper {
  ...
  IncompatibleEncodingDialog(@Nonnull VirtualFile virtualFile, @Nonnull final Charset charset, @Nonnull EncodingUtil.Magic8 safeToReload, @Nonnull EncodingUtil.Magic8 safeToConvert) {
    super(false);
    ...
    DialogWrapperAction reloadAction = new DialogWrapperAction("Reload") {
      ...
      protected void doAction(ActionEvent e) {
        if (safeToReload == EncodingUtil.Magic8.NO_WAY) {
          ...
          String explanation = "<br><br>" + (failReason == null ? "" : "Why: " + failReason + "<br>") + (current.isNull() ? "" : "Current encoding: '" + current.get().displayName() + "'");
          if (bom != null) {
            Messages.showErrorDialog(XmlStringUtil.wrapInHtml("File '" + virtualFile.getName() + "' can't be reloaded in the '" + charset.displayName() + "' encoding." + explanation),
                                     "Incompatible Encoding: " + charset.displayName());
            res = -1;
          }
          else {
            res = Messages.showDialog(XmlStringUtil.wrapInHtml("File '" + virtualFile.getName() + "' most likely isn't stored in the '" + charset.displayName() + "' encoding." + explanation),
                                      "Incompatible Encoding: " + charset.displayName(), new String[]{"Reload anyway", "Cancel"}, 1, AllIcons.General.WarningDialog);
          }
          if (res != 0) {
            doCancelAction();
            return;
    ...
    DialogWrapperAction convertAction = new DialogWrapperAction("Convert") {
      ...
      protected void doAction(ActionEvent e) {
        if (safeToConvert == EncodingUtil.Magic8.NO_WAY) {
          ...
          int res = Messages.showDialog(XmlStringUtil.wrapInHtml("Please do not convert to '" +
                                                                 charset.displayName() +
                                                                 "'.<br><br>" +
                                                                 (error == null
                                                                  ? "Encoding '" + charset.displayName() + "' does not support some characters from the text."
                                                                  : EncodingUtil.reasonToString(error, virtualFile))), "Incompatible Encoding: " + charset.displayName(),
                                        new String[]{"Convert anyway", "Cancel"}, 1, AllIcons.General.WarningDialog);
          if (res != 0) {
            doCancelAction();
            return;
          }
