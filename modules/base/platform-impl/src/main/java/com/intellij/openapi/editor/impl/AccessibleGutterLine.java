...
class AccessibleGutterLine extends JPanel {
  ...
  private static class MyShortcuts {
    static final CustomShortcutSet MOVE_RIGHT =
            new CustomShortcutSet(new KeyboardShortcut(KeyStroke.getKeyStroke(KeyEvent.VK_RIGHT, 0), null), new KeyboardShortcut(KeyStroke.getKeyStroke(KeyEvent.VK_TAB, 0), null));

    static final CustomShortcutSet MOVE_LEFT = new CustomShortcutSet(new KeyboardShortcut(KeyStroke.getKeyStroke(KeyEvent.VK_LEFT, 0), null),
  ...
  private class AccessibleGutterElement extends JLabel {
    ...
    public void paint(Graphics g) {
      ...
      try {
        ...
        LinePainter2D.paint((Graphics2D)g, bounds.x, y, bounds.x + bounds.width, y, LinePainter2D.StrokeType.INSIDE, JBUIScale.scale(1));
      }
      finally {
        g.setColor(oldColor);
      }
