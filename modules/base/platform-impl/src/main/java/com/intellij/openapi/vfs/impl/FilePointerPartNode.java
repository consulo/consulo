...
class FilePointerPartNode {
  ...
  void checkConsistency() {
    if (VirtualFilePointerManagerImpl.IS_UNDER_UNIT_TEST && !ApplicationInfoImpl.isInPerformanceTest()) {
      doCheckConsistency();
    }
  }
  ...
  private static List<String> splitNames(@Nonnull String path) {
    ...
    while (true) {
      ...
      int skipSeparator = StringUtil.endsWith(path, 0, startIndex, ArchiveFileSystem.ARCHIVE_SEPARATOR) ? 2 : 1;
