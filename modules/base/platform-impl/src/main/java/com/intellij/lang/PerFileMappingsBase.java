...
public abstract class PerFileMappingsBase<T> implements PersistentStateComponent<Element>, PerFileMappingsEx<T>, Disposable {
  ...
  protected T getNotInHierarchy(@Nullable VirtualFile file, @Nonnull Map<VirtualFile, T> mappings) {
    if (getProject() == null || file == null || file.getFileSystem() instanceof NonPhysicalFileSystem || !getProject().isDefault() && ProjectFileIndex.getInstance(getProject()).isInContent(file)) {
        ...
        synchronized (myMappings) {
          ...
          for (VFileEvent event : eventsFiltered) {
            ...
            if (!file.isDirectory()) {
              ...
              if (navSet == null) {
                navSet = new TreeSet<>((f1, f2) -> Comparing.compare(f1 == null ? null : f1.getUrl(), f2 == null ? null : f2.getUrl()));
