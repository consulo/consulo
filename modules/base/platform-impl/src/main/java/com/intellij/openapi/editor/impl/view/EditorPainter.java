...
public class EditorPainter implements TextDrawingCallback {
  ...
  void paint(Graphics2D g) {
    ...
    float[] marginWidths = paintBackground(g, clip, yShift, startLine, endLine, caretData);
    ...
    paintTextWithEffects(g, clip, yShift, startLine, endLine, caretData);
  ...
  private boolean paintPlaceholderText(Graphics2D g) {
    ...
    hintText = SwingUtilities.layoutCompoundLabel(g.getFontMetrics(), hintText.toString(), null, 0, 0, 0, 0, SwingUtilities.calculateInnerArea(editorComponent, null), // account for insets
  ...
  private void paintRightMargin(Graphics g, Rectangle clip, int yShift, int startVisualLine, int endVisualLine, float[] marginWidths) {
  ...
  private float[] paintBackground(Graphics2D g, Rectangle clip, int yShift, int startVisualLine, int endVisualLine, IterationState.CaretData caretData) {
    ...
    while (!visLinesIterator.atEnd()) {
      ...
      }, calculateMarginWidths && !visLinesIterator.endsWithSoftWrap() && !visLinesIterator.startsWithSoftWrap() ? width -> marginWidths[visualLine - startVisualLine] = width : null);
  ...
  private void paintSelectionOnSecondSoftWrapLineIfNecessary(Graphics2D g,
                                                             int visualLine,
                                                             int columnEnd,
                                                             float xEnd,
                                                             int y,
  ...
  private void paintSelectionOnFirstSoftWrapLineIfNecessary(Graphics2D g,
                                                            int visualLine,
                                                            int columnStart,
                                                            float xStart,
                                                            float xEnd,
    ...
    float endX = selectionEndPosition.line == visualLine ? (float)myView.visualPositionToXY(selectionEndPosition).getX() : xEnd;
  ...
  private void paintBackground(Graphics2D g, Color color, float x, int y, float width) {
    if (width <= 0 || color == null || color.equals(myEditor.getColorsScheme().getDefaultBackground()) || color.equals(myEditor.getBackgroundColor())) return;
    g.setColor(color);
  ...
  private void paintLineMarkersSeparators(final Graphics g, final Rectangle clip, int yShift, MarkupModelEx markupModel, int startOffset, int endOffset) {
  ...
  private void paintTextWithEffects(Graphics2D g, Rectangle clip, int yShift, int startVisualLine, int endVisualLine, IterationState.CaretData caretData) {
    ...
    if (startVisualLine == 0 && prefixLayout != null) {
      ...
      paintLineLayoutWithEffect(g, prefixLayout, myCorrector.startX(startVisualLine), myView.getAscent() + yShift, attributes.getEffectColor(), attributes.getEffectType());
    }
    ...
    while (!visLinesIterator.atEnd()) {
      ...
      paintLineFragments(g, clip, visLinesIterator, caretData, y + myView.getAscent(), new LineFragmentPainter() {
        ...
        public void paint(Graphics2D g, VisualLineFragmentsIterator.Fragment fragment, int start, int end, TextAttributes attributes, float xStart, float xEnd, int y) {
          ...
          if (inlay != null) {
            inlay.getRenderer().paint(myEditor, g, new Rectangle((int)xStart, y - myView.getAscent(), inlay.getWidthInPixels(), lineHeight), attributes);
            return;
          }
          FoldRegion foldRegion = fragment.getCurrentFoldRegion();
          if (foldRegion != null && Registry.is("editor.highlight.foldings")) {
  ...
  private static void collectVisibleInnerHighlighters(@Nonnull FoldRegion region, @Nonnull MarkupModelEx markupModel, @Nonnull List<RangeHighlighterEx> highlighters) {
  ...
  private void paintWhitespace(Graphics2D g,
                               CharSequence text,
                               float x,
                               int y,
                               int start,
  ...
  private void paintHighlightersAfterEndOfLine(final Graphics2D g, int yShift, MarkupModelEx markupModel, final int startOffset, int endOffset) {
  ...
  private void paintBorderEffect(final Graphics2D g, final ClipDetector clipDetector, int yShift, MarkupModelEx markupModel, int clipStartOffset, int clipEndOffset) {
    markupModel.processRangeHighlightersOverlappingWith(clipStartOffset, clipEndOffset, rangeHighlighter -> {
      ...
      if (isBorder(attributes)) {
        paintBorderEffect(g, clipDetector, yShift, rangeHighlighter.getAffectedAreaStartOffset(), rangeHighlighter.getAffectedAreaEndOffset(), attributes);
      }
      return true;
    });
  }
        ...
        if (myDocument.getTextLength() > 0 && caret != null) {
          ...
          for (VisualLineFragmentsIterator.Fragment fragment : VisualLineFragmentsIterator.create(myView, caret.getVisualLineStart(), false)) {
            ...
            if (startVisualColumn <= targetVisualColumn && targetVisualColumn < endVisualColumn) {
              ...
              fragment.draw(g, startX, y + topOverhang + myView.getAscent(), fragment.visualColumnToOffset(targetVisualColumn - startVisualColumn),
  ...
  private void paintLineFragments(Graphics2D g,
                                  Rectangle clip,
    ...
    for (VisualLineFragmentsIterator.Fragment fragment : VisualLineFragmentsIterator.create(myView, visLineIterator, null, true)) {
      ...
      if (firstFragment) {
        ...
        if (hasSoftWrap || myEditor.isRightAligned()) {
          ...
          it = new IterationState(myEditor, offset == 0 ? 0 : DocumentUtil.getPreviousCodePointOffset(myDocument, offset), visualLineEndOffset, caretData, false, false, false, false);
      ...
      if (foldRegion == null) {
        if (start != prevEndOffset) {
          it = new IterationState(myEditor, start, fragment.isRtl() ? offset : visualLineEndOffset, caretData, false, false, false, fragment.isRtl());
        }
        prevEndOffset = end;
        assert it != null;
        if (start == end) { // special case of inlays
          ...
          while (fragment.isRtl() ? start > end : start < end) {
            ...
            if (xNew >= clip.getMinX()) {
              painter.paint(g, fragment, fragment.isRtl() ? fragmentStartOffset - start : start - fragmentStartOffset, fragment.isRtl() ? fragmentStartOffset - curEnd : curEnd - fragmentStartOffset,
                            attributes, x, xNew, y);
            }
            x = xNew;
            start = curEnd;
    ...
    if (it == null || it.getEndOffset() != visualLineEndOffset) {
      it = new IterationState(myEditor, visualLineEndOffset == offset ? visualLineEndOffset : DocumentUtil.getPreviousCodePointOffset(myDocument, visualLineEndOffset), visualLineEndOffset, caretData,
    ...
    if (marginWidthConsumer != null && !marginReached && (visualLine == myEditor.getCaretModel().getVisualPosition().line || x > marginColumns * myView.getPlainSpaceWidth())) {
  ...
  interface LineFragmentPainter {
    ...
    void paint(Graphics2D g, VisualLineFragmentsIterator.Fragment fragment, int start, int end, TextAttributes attributes, float xStart, float xEnd, int y);
  ...
  private interface XCorrector {
    ...
    @Nonnull
    default XCorrector align(@Nonnull EditorView view) {
