...
public final class WindowInfoImpl implements Cloneable, WindowInfo {
    public static float normalizeWeight(float weight) {
        ...
        if (weight >= 1) {
            return 1 - WindowInfoImpl.DEFAULT_WEIGHT;
        }
        return weight;
    }
    ...
    private static final String AUTO_HIDE_ATTR = "auto_hide";
    ...
    public void readExternal(Element element) {
        ...
        myWasRead = true;
        try {
            myActive = Boolean.valueOf(element.getAttributeValue(ACTIVE_ATTR)) && canActivateOnStart(myId);
        }
        ...
        myAutoHide = Boolean.valueOf(element.getAttributeValue(AUTO_HIDE_ATTR));
        try {
        ...
        myVisible = Boolean.valueOf(element.getAttributeValue(VISIBLE_ATTR)) && canActivateOnStart(myId);
        if (element.getAttributeValue(SHOW_STRIPE_BUTTON) != null) {
            myShowStripeButton = Boolean.valueOf(element.getAttributeValue(SHOW_STRIPE_BUTTON));
        }
        try {
        ...
        try {
            myOrder = Integer.valueOf(element.getAttributeValue(ORDER_ATTR));
        }
        catch (NumberFormatException ignored) {
        }
        try {
    ...
    public void writeExternal(Element element) {
        ...
        element.setAttribute(AUTO_HIDE_ATTR, Boolean.toString(myAutoHide));
    ...
    private static boolean canActivateOnStart(String id) {
        return Application.get().getExtensionPoint(ToolWindowFactory.class)
            .noneMatchSafe(factory -> id.equals(factory.getId()) && factory.isDoNotActivateOnStart());
    }
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof WindowInfoImpl info)) {
            return false;
        }
        return myActive == info.myActive
            ...
            && myAutoHide == info.myAutoHide
            && Objects.equals(myFloatingBounds, info.myFloatingBounds)
            && myInternalType == info.myInternalType
            && myType == info.myType
            && myVisible == info.myVisible
