...
public class BuiltInCompilerRunner implements CompilerRunner {
  ...
  private static final FileProcessingCompilerAdapterFactory FILE_PACKAGING_COMPILER_ADAPTER_FACTORY =
    (context, compiler) -> new PackagingCompilerAdapter(context,
  ...
  public boolean build(CompileDriver compileDriver,
    ...
    didSomething |= invokeFileProcessingCompilers(compileDriver,
                                                  compilerManager,
                                                  context,
    ...
    didSomething |=
      invokeFileProcessingCompilers(compileDriver,
                                    compilerManager, context, Validator.class, FILE_PROCESSING_COMPILER_ADAPTER_FACTORY, forceCompile, true,
                                    onlyCheckStatus);
  ...
  private boolean generateSources(CompileDriver compileDriver, final CompilerManager compilerManager,
                                  CompileContextEx context,
      ...
      for (final Chunk<Module> currentChunk : sortedChunks) {
        ...
        try {
          ...
          if (filesToRecompile.size() > 0) {
            sink.add(null, Collections.<TranslatingCompiler.OutputItem>emptyList(), VirtualFileUtil.toVirtualFileArray(filesToRecompile));
          }
          if (errorCount == 0) {
  ...
  private static void recalculateChunkToFilesMap(
    CompileContextEx context, List<Chunk<Module>> allChunks, VirtualFile[] snapshot, Map<Chunk<Module>, Collection<VirtualFile>> chunkMap) {
  ...
  private static boolean syncOutputDir(CompileDriver compileDriver, final CompileContextEx context,
                                       final Collection<Trinity<File, String, Boolean>> toDelete) throws CacheCorruptedException {
  ...
  private boolean invokeFileProcessingCompilers(
    ...
    boolean didSomething = false;
    final FileProcessingCompiler[] compilers = compilerManager.getCompilers(fileProcessingCompilerClass, compileDriver.getCompilerFilter());
  ...
  private boolean generateOutput(CompileDriver compileDriver,
                                 final CompileContextEx context,
  ...
  private VirtualFile getGenerationOutputDir(CompileDriver compileDriver,
