...
import consulo.versionControlSystem.log.impl.internal.data.SingleTaskController;
import consulo.versionControlSystem.log.impl.internal.data.VcsLogProgress;
import consulo.versionControlSystem.log.impl.internal.data.VcsLogStorageImpl;
import consulo.versionControlSystem.log.impl.internal.data.VcsUserRegistryImpl;
import consulo.versionControlSystem.log.impl.internal.ui.filter.VcsLogTextFilterImpl;
import consulo.versionControlSystem.log.impl.internal.util.PersistentSet;
import consulo.versionControlSystem.log.impl.internal.util.PersistentSetImpl;
...
import static consulo.versionControlSystem.log.impl.internal.data.index.VcsLogFullDetailsIndex.INDEX;
import static consulo.versionControlSystem.log.impl.internal.util.PersistentUtil.*;
...
public class VcsLogPersistentIndex implements VcsLogIndex, Disposable {
    ...
    public VcsLogPersistentIndex(
        ...
        myUserRegistry = (VcsUserRegistryImpl) myProject.getInstance(VcsUserRegistry.class);
    ...
    protected MyIndexStorage createIndexStorage(FatalErrorHandler fatalErrorHandler, String logId) {
        try {
    ...
    private void storeDetail(VcsFullCommitDetails detail) {
    ...
    @Override
    public synchronized boolean isIndexed(VirtualFile root) {
        return myRoots.contains(root) && (!myCommitsToIndex.containsKey(root) && myNumberOfTasks.get(root).get() == 0);
    }

    @Override
    public synchronized void markForIndexing(int index, VirtualFile root) {
    ...
    private <T> IntSet filter(PersistentMap<Integer, T> map, Predicate<T> condition) {
    ...
    private IntSet filterUsers(Set<VcsUser> users) {
        if (myIndexStorage != null) {
            try {
    ...
    private IntSet filterPaths(Collection<FilePath> paths) {
        if (myIndexStorage != null) {
            try {
    ...
    public IntSet filterMessages(VcsLogTextFilter filter) {
        if (myIndexStorage != null) {
            try {
                if (!filter.isRegex()) {
    ...
    private void processRuntimeException(RuntimeException e) {
        if (myIndexStorage != null) {
    ...
    private static class MyIndexStorage {
        ...
        private static void catchAndWarn(ThrowableRunnable<IOException> runnable) {
            try {
                runnable.run();
            }
            catch (IOException e) {
        ...
        private static void cleanup(String logId) {
    ...
    private class IndexingRequest {
        ...
        private boolean indexOneByOne(VirtualFile root, TIntHashSet commits) {
    ...
    private static class CommitsCounter {
        ...
        private CommitsCounter(ProgressIndicator indicator, int commits) {
