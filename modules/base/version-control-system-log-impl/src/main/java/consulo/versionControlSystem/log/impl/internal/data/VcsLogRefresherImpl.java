...
public class VcsLogRefresherImpl implements VcsLogRefresher {
    ...
    public VcsLogRefresherImpl(
        ...
        myRecentCommitCount = recentCommitCount;
        myProgress = progress;
    ...
    private List<GraphCommit<Integer>> compactCommits(List<? extends TimedVcsCommit> commits, VirtualFile root) {
        StopWatch sw = StopWatch.start("compacting commits");
        List<GraphCommit<Integer>> map = ContainerUtil.map(
            commits,
            (Function<TimedVcsCommit, GraphCommit<Integer>>) commit -> compactCommit(commit, root)
        );
        myStorage.flush();
        sw.report();
        return map;
    ...
    private GraphCommitImpl<Integer> compactCommit(TimedVcsCommit commit, VirtualFile root) {
        List<Integer> parents = ContainerUtil.map(commit.getParents(), hash -> myStorage.getCommitIndex(hash, root));
    ...
    private class MyRefreshTask extends Task.Backgroundable {
                ...
                if (permanentGraph != null) {
                    ...
                    for (int attempt = 0; attempt <= 1; attempt++) {
                        ...
                        List<? extends GraphCommit<Integer>> joinedFullLog =
                            join(permanentGraph.getAllCommits(), compoundLog, currentRefs, allNewRefs);
                    ...
                    LOG.info(
                        "Couldn't join " + commitCount / 5 + " recent commits to the log (" +
        ...
        private Map<VirtualFile, CompressedRefs> getAllNewRefs(LogInfo newInfo, Map<VirtualFile, CompressedRefs> previousRefs) {
        ...
        private void loadLogAndRefs(Collection<VirtualFile> roots, Map<VirtualFile, CompressedRefs> prevRefs, int commitCount)
            throws VcsException {
        ...
        private @Nullable List<? extends GraphCommit<Integer>> join(
            ...
            catch (VcsLogRefreshNotEnoughDataException | IllegalStateException e) {
                LOG.info(e);
                return null;
            }
        }
