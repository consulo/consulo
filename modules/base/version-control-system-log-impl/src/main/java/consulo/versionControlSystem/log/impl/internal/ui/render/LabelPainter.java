...
public class LabelPainter {
    ...
    public LabelPainter(VcsLogDataImpl data, boolean compact, boolean showTagNames) {
        myLogData = data;
        myCompact = compact;
    ...
    public void customizePainter(JComponent component,
        ...
        List<RefGroup> refGroups = manager == null ? List.of() : manager.groupForTable(references, myCompact, myShowTagNames);
    ...
    private static Pair<List<Pair<String, LabelIcon>>, Integer> calculatePresentation(
        ...
        if (compact) {
            return calculateCompactPresentation(refGroups, fontMetrics, height, background, availableWidth);
        }
    ...
    private static Color[] getColors(Collection<RefGroup> groups) {
        SequencedMap<Color, Integer> usedColors = new LinkedHashMap<>();
    ...
    private static String getGroupText(RefGroup group, FontMetrics fontMetrics, int availableWidth) {
        if (!group.isExpanded()) {
    ...
    private static Color calculateGreyBackground(List<RefGroup> refGroups,
        ...
        if (!isCompact) {
            return ColorUtil.mix(background, BACKGROUND, BALANCE);
        }
    ...
    private static String shortenRefName(String refName, FontMetrics fontMetrics, int availableWidth) {
        ...
        for (Pair<String, LabelIcon> label : myLabels) {
            ...
            if (myGreyBackground != null && !myCompact) {
                ...
                g2.fill(new RoundRectangle2D.Double(x - LEFT_PADDING, y + baseLine - fontMetrics.getAscent() - TOP_TEXT_PADDING,
                    ...
                    fontMetrics.getHeight() + TOP_TEXT_PADDING + BOTTOM_TEXT_PADDING, RectanglePainter.LABEL_ARC, RectanglePainter.LABEL_ARC));
            }
