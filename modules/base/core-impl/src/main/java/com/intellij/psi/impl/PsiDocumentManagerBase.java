...
public abstract class PsiDocumentManagerBase extends PsiDocumentManager implements Disposable, DocumentListener, ProjectComponent {
  ...
  protected PsiDocumentManagerBase(@Nonnull final Project project, @Nonnull PsiManager psiManager, @NonNls @Nonnull final DocumentCommitProcessor documentCommitProcessor) {
    myProject = project;
  ...
  private FileViewProvider getCachedViewProvider(@Nonnull VirtualFile virtualFile) {
  ...
  private boolean commitToExistingPsi(@Nonnull Document document,
                                      ...
                                      @Nonnull FileViewProvider viewProvider) {
  ...
  private void doCommit(@Nonnull final Document document) {
  ...
  public boolean performWhenAllCommitted(@Nonnull final Runnable action) {
    ...
    if (current != ModalityState.NON_MODAL) {
      ...
      for (Document document : myUncommittedDocuments) {
        myDocumentCommitProcessor.commitAsynchronously(myProject, document, "re-added with modality " + current + " because performWhenAllCommitted(" + current + ") was called", current);
      }
    }
    return false;
  }
  ...
  public void performLaterWhenAllCommitted(@Nonnull final Runnable runnable, final ModalityState modalityState) {
    ...
    if (ApplicationManager.getApplication().isDispatchThread() && isInsideCommitHandler()) {
      ...
      if (Platform.current().isDesktop()) {
        ...
        Application.get().invokeLater(() -> performWhenAllCommitted(whenAllCommitted));
      }
    }
  }
  ...
  private void runAfterCommitActions(@Nonnull Document document) {
    ...
    if (!hasUncommitedDocuments() && !actionsWhenAllDocumentsAreCommitted.isEmpty()) {
      List<Map.Entry<Object, Runnable>> entries = new ArrayList<>(new LinkedHashMap<>(actionsWhenAllDocumentsAreCommitted).entrySet());
      beforeCommitHandler();
