...
public class ShowCoveringTestsAction extends AnAction {
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        if (ProgressManager.getInstance().runProcessWithProgressSynchronously(
            ...
            if (!elements.isEmpty()) {
                var factory = Application.get().getInstance(ImplementationViewComponentFactory.class);

                component = factory.create(PsiUtilCore.toPsiElementArray(elements), 0);
                popupBuilder = JBPopupFactory.getInstance()
                    .createComponentPopupBuilder(component.getComponent(), component.getPreferredFocusableComponent())
