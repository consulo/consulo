...
public class SrcFileAnnotator implements Disposable {
    ...
    public static final Key<Map<FileEditor, EditorNotificationBuilderEx>> NOTIFICATION_PANELS = Key.create("NOTIFICATION_PANELS");
    ...
    public void hideCoverageData() {
        ...
        Map<FileEditor, EditorNotificationBuilderEx> map = myFile.getCopyableUserData(NOTIFICATION_PANELS);
        if (map != null) {
            ...
            for (FileEditor fileEditor : map.keySet()) {
                ...
                fileEditorManager.removeTopComponent(fileEditor, map.get(fileEditor).getComponent());
            }
        }
    ...
    public void showCoverageInformation(final CoverageSuitesBundle suite) {
        ...
        Module module = myProject.getApplication().runReadAction((Supplier<Module>) () -> myFile.getModule());
        if (module != null) {
            ...
            public void documentChanged(DocumentEvent e) {
                ...
                if (!myUpdateAlarm.isDisposed()) {
                    myUpdateAlarm.addRequest(
                        () -> {
                            Int2IntMap newToOldLineMapping = getNewToOldLineMapping(suite.getLastCoverageTimeStamp());
    ...
    private RangeHighlighter createRangeHighlighter(
        ...
        Int2IntFunction newToOldConverter = newLine -> {
            if (myEditor == null) {
                return -1;
            }
            Int2IntMap oldLineMapping = getNewToOldLineMapping(date);
            return oldLineMapping != null ? oldLineMapping.getOrDefault(newLine, 0) : newLine;
        };
        Int2IntFunction oldToNewConverter = newLine -> {
            if (myEditor == null) {
                return -1;
            }
            Int2IntMap newLineMapping = getOldToNewLineMapping(date);
        ...
        myProject.getApplication().invokeLater(() -> {
            ...
            for (FileEditor editor : editors) {
                if (isCurrentEditor(editor)) {
                    ...
                    builder.withIcon(PlatformIconGroup.generalExclmark());
                    builder.withText(message);
                    builder.withAction(CommonLocalize.closeActionName(), event -> fileEditorManager.removeTopComponent(editor, builder.getComponent()));

                    fileEditorManager.addTopComponent(editor, builder);
                    break;
                }
            }
        });
