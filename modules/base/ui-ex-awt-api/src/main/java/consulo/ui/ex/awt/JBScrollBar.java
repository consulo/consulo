...
*/
    ...
    public int getUnitIncrement(int direction) {
        ...
        if (viewport != null) {
            ...
            if (scrollable != null) {
                return scrollable.getScrollableUnitIncrement(viewport.getViewRect(), orientation, direction);
            }
            if (!isUnitIncrementSet) {
                Font font = getViewFont(viewport);
                if (font != null) {
                    return font.getSize(); // increase default unit increment for fast scrolling
                }
            }
        }
    ...
    public int getBlockIncrement(int direction) {
        ...
        if (viewport != null) {
            Scrollable scrollable = getScrollableViewToCalculateIncrement(viewport.getView());
            if (scrollable != null) {
                return scrollable.getScrollableBlockIncrement(viewport.getViewRect(), orientation, direction);
            }
    ...
    public boolean handleMouseWheelEvent(MouseWheelEvent event) {
        if (!isSupportedScrollType(event)) {
            return false;
        }
        if (event.isShiftDown() == (orientation == VERTICAL)) {
            return false;
        }
