...
public class ScrollingUtil {
    ...
    private static int visibleRowCount(JComponent c) {
        if (c instanceof JList) {
            return getVisibleRowCount((JList) c);
        }
        if (c instanceof JTable) {
            return getVisibleRowCount((JTable) c);
        }
        return -1;
    }
    ...
    static class MoveAction extends AbstractAction {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            else if (SCROLLDOWN_ACTION_ID.equals(myId)) {
                doPageDown();
            }
            else if (SELECT_PREVIOUS_ROW_ACTION_ID.equals(myId)) {
                doMoveUp(modifiers);
            }
            else if (SELECT_NEXT_ROW_ACTION_ID.equals(myId)) {
                doMoveDown(modifiers);
            }
            else if (SELECT_LAST_ROW_ACTION_ID.equals(myId)) {
                doMoveEnd();
            }
            else if (SELECT_FIRST_ROW_ACTION_ID.equals(myId)) {
                doMoveHome();
            }
            else if (MOVE_END_ID.equals(myId)) {
                doMoveEnd();
            }
            else if (MOVE_HOME_ID.equals(myId)) {
                doMoveHome();
            }
        }
        ...
        private void doMoveEnd() {
            if (myComponent instanceof JList) {
                moveEnd((JList) myComponent);
            }
            else if (myComponent instanceof JTable) {
                moveEnd((JTable) myComponent);
            }
            else {
                throw new IllegalArgumentException("MoveEnd is not implemented for " + myComponent.getClass());
            }
        }

        private void doMoveHome() {
            if (myComponent instanceof JList) {
                moveHome((JList) myComponent);
            }
            else if (myComponent instanceof JTable) {
                moveHome((JTable) myComponent);
            }
            else {
                throw new IllegalArgumentException("MoveHome is not implemented for " + myComponent.getClass());
            }
        }
        ...
        private void doMoveUp(@JdkConstants.InputEventMask int modifiers) {
            if (myComponent instanceof JList) {
                ...
                moveUp((JTable) myComponent, modifiers, isCycleScrolling());
            }
            else {
                throw new IllegalArgumentException("MoveUp is not implemented for " + myComponent.getClass());
            }
        }
        ...
        private void doMoveDown(@JdkConstants.InputEventMask int modifiers) {
            if (myComponent instanceof JList) {
                ...
                moveDown((JTable) myComponent, modifiers, isCycleScrolling());
            }
            else {
                throw new IllegalArgumentException("MoveDown is not implemented for " + myComponent.getClass());
            }
        }

        private void doPageUp() {
            if (myComponent instanceof JList) {
                movePageUp((JList) myComponent);
            }
            else if (myComponent instanceof JTable) {
                movePageUp((JTable) myComponent);
            }
            else {
                throw new IllegalArgumentException("PageUp is not implemented for " + myComponent.getClass());
            }
        }

        private void doPageDown() {
            if (myComponent instanceof JList) {
                movePageDown((JList) myComponent);
            }
            else if (myComponent instanceof JTable) {
                movePageDown((JTable) myComponent);
            }
            else {
                throw new IllegalArgumentException("PageDown is not implemented for " + myComponent.getClass());
            }
        }
