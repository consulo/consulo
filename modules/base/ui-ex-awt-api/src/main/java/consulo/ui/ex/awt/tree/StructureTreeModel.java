...
public class StructureTreeModel<Structure extends AbstractTreeStructure> extends AbstractTreeModel implements Disposable, InvokerSupplier, ChildrenProvider<TreeNode> {
    ...
    private Node getNode(Object object, boolean validateChildren) {
        if (disposed || !(object instanceof Node) || !isValidThread()) {
            return null;
        }
    ...
    private static final class Node extends DefaultMutableTreeNode implements LeafState.Supplier {
        ...
        private Node(NodeDescriptor descriptor, LeafState leafState, int hashCode) {
            ...
            if (leafState == LeafState.ALWAYS) {
                children.set(null); // validate children for leaf node
            }
        ...
        private Node findChild(Object element) {
            ...
            if (list != null) {
                if (!list.isEmpty()) {
                    ...
                    if (result.isPresent()) {
                        return result.get(); // found child node that matches given element
                    }
                }
                if (LOG.isTraceEnabled()) {
                    LOG.debug("node '", getElement(), "' have no child: ", element);
                }
            }
            else {
                if (LOG.isTraceEnabled()) {
                    LOG.debug("node '", getElement(), "' have no loaded children");
                }
            }
            return null;
        }
        ...
        private boolean isLeaf(@Nullable Consumer<? super Node> validator) {
            ...
            if (leafState == LeafState.NEVER) {
                return false;
            }
            if (leafState == LeafState.DEFAULT && validator != null) {
