...
public abstract class JBListTable extends JPanel {
    ...
    public JBListTable(@Nonnull final JTable t) {
        ...
        mainTable = new JBTable(model) {
            ...
            protected boolean processKeyBinding(KeyStroke ks, KeyEvent e, int condition, boolean pressed) {
                ...
                boolean isUp = e.getKeyCode() == VK_UP;
                boolean isDown = e.getKeyCode() == VK_DOWN;
            ...
            public TableCellEditor getCellEditor(int row, int column) {
                JBTableRowEditor editor = getRowEditor(row);
                if (editor != null) {
    ...
    protected JBTableRow getRowAt(int row) {
        return column -> myInternalTable.getValueAt(row, column);
    }
    ...
    private static class RowResizeAnimator extends Thread {
        ...
        public void run() {
            ...
            try {
                sleep(50);
                JScrollBar bar = myScrollPane.getVerticalScrollBar();
    ...
    private class MyCellEditor extends AbstractTableCellEditor implements Animated {
        ...
        public Component getTableCellEditorComponent(final JTable table, Object value, boolean isSelected, final int row, int column) {
            ...
            JPanel p = new JPanel(new BorderLayout()) {
                ...
                public void removeNotify() {
                    if (myCellEditor != null) {
                        myCellEditor.saveFocusIndex();
                    }
                    super.removeNotify();
        ...
        private void removeEmptyRow() {
            ...
            if (row != -1 && isRowEmpty(row)) {
                JBListTableModel model = (JBListTableModel) curTable.getModel();
                int count = model.getRowCount();
                model.removeRow(row);
                int newRow = count == row + 1 ? row - 1 : row;
