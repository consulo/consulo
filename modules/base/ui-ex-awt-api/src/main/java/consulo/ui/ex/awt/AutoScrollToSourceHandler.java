...
public abstract class AutoScrollToSourceHandler {
    ...
    getReady(dataContext).doWhenDone(new Runnable() {
      ...
      public void run() {
        ...
        if (vFile != null) {
          ...
          if (RawFileLoader.getInstance().isLargeForContentLoading(vFile.getLength())) return;
        }
  ...
  private class AutoscrollToSourceAction extends ToggleAction implements DumbAware {
    public AutoscrollToSourceAction() {
      super(UIBundle.message("autoscroll.to.source.action.name"), UIBundle.message("autoscroll.to.source.action.description"), AllIcons.General.AutoscrollToSource);
    }
