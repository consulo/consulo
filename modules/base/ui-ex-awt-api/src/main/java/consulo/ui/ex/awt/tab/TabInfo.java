...
public final class TabInfo implements Queryable, PlaceProvider<String> {
    ...
    private static final Image DEFAULT_ALERT_ICON = new AlertIcon(TargetAWT.to(PlatformIconGroup.nodesTabalert()));
    ...
    public TabInfo setText(LocalizeValue text) {
        ...
        if (!myText.toString().equals(text.get()) || !Objects.equals(textAttributes, defaultAttributes)) {
            clearText(false);
    ...
    private SimpleTextAttributes getDefaultAttributes() {
        ...
        if (attributes == null) {
            myDefaultAttributes = attributes = new SimpleTextAttributes(
                myDefaultStyle != -1 ? myDefaultStyle : SimpleTextAttributes.STYLE_PLAIN,
                myDefaultForeground,
                myDefaultWaveColor
            );
        }
    ...
    public TabInfo setForeSideComponent(JComponent comp) {
