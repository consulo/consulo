...
public class ArtifactElementType extends ComplexPackagingElementType<ArtifactPackagingElement> {
    ...
    @Override
    @RequiredUIAccess
    public List<? extends ArtifactPackagingElement> chooseAndCreate(
        ArtifactEditorContext context,
        Artifact artifact,
        CompositePackagingElement<?> parent
    ) {
        List<Artifact> artifacts = context.chooseArtifacts(
            getAvailableArtifacts(context, artifact, false),
            CompilerLocalize.dialogTitleChooseArtifacts()
        );
    ...
    public static List<? extends Artifact> getAvailableArtifacts(
        ArtifactEditorContext context,
        Artifact artifact,
        ...
        while (iterator.hasNext()) {
            ...
            boolean notContainThis = ArtifactUtil.processPackagingElements(
                another,
                getInstance(),
                element -> !artifact.getName().equals(element.getArtifactName()),
                context,
                true
            );
            if (!notContainThis) {
