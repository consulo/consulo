...
public class ThriftTestExecutionUtil {
    ...
    private static void attachEventsProcessors(
        TestConsoleProperties consoleProperties,
    ...
    public static TServer open(int port, TestInterface.Iface iface) {
        ...
        TestInterface.Processor<TestInterface.Iface> processor = new TestInterface.Processor<>(iface);

        TSimpleServer server = new TSimpleServer(new TServer.Args(localhost).processor(processor));
        Application.get().executeOnPooledThread((Runnable) server::serve);
