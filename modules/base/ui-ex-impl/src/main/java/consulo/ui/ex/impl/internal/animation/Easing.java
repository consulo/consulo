...
public interface Easing {
     ...
     */
    default Easing invert() {
        return (x) -> 1.0 - calc(1 - x);
    }

    /**
     * Run animation from the start to the end and then back.
     */
    default Easing mirror() {
        return (x) -> calc(x < 0.5 ? (x * 2) : (1 - (x - 0.5) * 2));
    }
    ...
    final class Stateful implements Easing {
        private final Easing myDelegate;
        public double value;
