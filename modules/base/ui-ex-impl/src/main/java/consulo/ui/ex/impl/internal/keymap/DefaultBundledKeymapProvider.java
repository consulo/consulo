...
public class DefaultBundledKeymapProvider implements BundledKeymapProvider {
    ...
    @Override
    public void loadKeymaps(BiConsumer<String, InputStream> consumer) {
        for (String keymapFile : KEYMAPS) {
            InputStream stream = getClass().getResourceAsStream(keymapFile);
            if (stream != null) {
                consumer.accept(keymapFile, stream);
            }
        }
    }
}
