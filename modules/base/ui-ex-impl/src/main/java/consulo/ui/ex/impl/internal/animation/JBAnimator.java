...
public final class JBAnimator implements Disposable {
    ...
    private Type myType = Type.IN_TIME;
    private boolean myCyclic = false;
    private boolean myIgnorePowerSaveMode = false;
    private @Nullable String myName = null;

    private final @Nullable ScheduledExecutorService myPooledService;
    private final AtomicLong myRunning = new AtomicLong();
    private final AtomicBoolean myDisposed = new AtomicBoolean();

    private @Nullable Statistic myStatistic = null;
    private volatile Future<?> myCurrentAnimatorFuture = CompletableFuture.completedFuture(null);
    ...
    public JBAnimator(Thread threadToUse, @Nullable Disposable parentDisposable) {
        myPooledService = threadToUse == Thread.SWING_THREAD
            ? null
    ...
    public long animate(Collection<Animation> animations) {
        ...
        ScheduledExecutorService service = myPooledService != null ? myPooledService : UIAccess.current().getScheduler();
        ...
        int delay = animations.isEmpty() ? 0 : from;
        int duration = animations.isEmpty() ? 0 : to - from;

        long taskId = myRunning.incrementAndGet();

        if (!myIgnorePowerSaveMode && PowerSaveMode.isEnabled()
            || Registry.is("ui.no.bangs.and.whistles")
            || RemoteDesktopService.isRemoteSession()
            || duration == 0) {
            myCurrentAnimatorFuture = service.schedule(() -> {
        ...
        Statistic stat = new Statistic(myName, taskId);
        stat.myStart = System.nanoTime();
        if (myPeriod < 16) { // do not enable this until it's really necessary
            JBAnimatorHelper.requestHighPrecisionTimer(this);
        }

        myCurrentAnimatorFuture = service.schedule(new Runnable() {
            private final Type myRunType = myType;
            private final int myRunPeriod = myPeriod;
            private final boolean myRunCycle = myCyclic;
            private @Nullable FrameCounter myFrameCounter;
            private Set<Animation> myScheduledAnimations = new LinkedHashSet<>();
            private long myNextScheduleTime = System.nanoTime() + TimeUnit.MILLISECONDS.toNanos(delay);

            private FrameCounter prepareAnimations() {
                FrameCounter frameCounter = create(myRunType, myRunPeriod, duration);
                myFrameCounter = frameCounter;
                myScheduledAnimations = new LinkedHashSet<>(animations);
                for (Animation animation : myScheduledAnimations) {
                    animation.fireEvent(Animation.Phase.SCHEDULED);
                }
                return frameCounter;
            }
            ...
            private void finalizeRunning() {
                ...
                stat.myEnd = System.nanoTime();
                myStatistic = stat;
            }

            @Override
            public void run() {
                stat.myCount.incrementAndGet();
                ...
                long wasLate = System.nanoTime() - myNextScheduleTime;
                if (wasLate < 0) {
                ...
                FrameCounter frameCounter = myFrameCounter;
                if (frameCounter == null) {
                    frameCounter = prepareAnimations();
                }
                long totalFrames = frameCounter.getTotalFrames();
                long currentFrame = Math.min(frameCounter.getNextFrame(myRunCycle), totalFrames);
                long currentDelay = frameCounter.getDelay(currentFrame);
                double timeline = (double) currentFrame / totalFrames;
                if (currentFrame >= totalFrames && myRunCycle) {
                    myFrameCounter = null;
                }
                List<Animation> expired = new LinkedList<>();
                for (Animation animation : myScheduledAnimations) {
                ...
                expired.forEach(myScheduledAnimations::remove);
                boolean isProceed = currentFrame < totalFrames || myRunCycle;
                if (isProceed) {
                    long nextDelay = Math.max(TimeUnit.MILLISECONDS.toNanos(currentDelay) - wasLate, TimeUnit.MILLISECONDS.toNanos(1));
                    myNextScheduleTime = System.nanoTime() + nextDelay;
                    myCurrentAnimatorFuture = service.schedule(this, nextDelay, TimeUnit.NANOSECONDS);
                }
                else {
                ...
                for (Animation animation : isProceed ? expired : myScheduledAnimations) {
                    animation.fireEvent(Animation.Phase.EXPIRED);
                }
            }
        }, delay, TimeUnit.MILLISECONDS);
    ...
    public void dispose() {
        ...
        if (!myDisposed.getAndSet(true) && myPooledService != null) {
            myPooledService.shutdownNow();
        }
    ...
    private static FrameCounter create(Type type, int period, int duration) {
        return switch (type) {
            case EACH_FRAME -> new FrameCounter() {
                private final long myFrames = duration / period + ((duration % period == 0) ? 0 : 1);
                private long myFrame = 0;
            ...
            case IN_TIME -> new FrameCounter() {
                private final long myStartTime = TimeUnit.NANOSECONDS.toMillis(System.nanoTime());

                @Override
                public long getNextFrame(boolean isCyclic) {
                    return TimeUnit.NANOSECONDS.toMillis(System.nanoTime()) - myStartTime;
                }
    ...
    public static class Statistic {
        ...
        private final AtomicLong myCount = new AtomicLong(0);
        ...
        public long getDuration() {
            return TimeUnit.NANOSECONDS.toMillis(myEnd - myStart);
        }
        ...
        public String toString() {
            return "Statistic{" +
                ...
                ", count=" + myCount +
                ", updates=" + (myCount.get() * 1000 / getDuration()) +
                '}';
        }
    }
}
