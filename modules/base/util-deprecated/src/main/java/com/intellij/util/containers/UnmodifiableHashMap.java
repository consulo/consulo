...
public final class UnmodifiableHashMap<K, V> implements Map<K, V> {
  ...
  public static
  @Nonnull
  <K, V> UnmodifiableHashMap<K, V> empty() {
  ...
  public static
  @Nonnull
  <K, V> UnmodifiableHashMap<K, V> fromMap(@Nonnull Map<? extends K, ? extends V> map) {
  ...
  public
  @Nonnull
  UnmodifiableHashMap<K, V> without(@Nonnull K key) {
  ...
  public
  @Nonnull
  UnmodifiableHashMap<K, V> with(@Nonnull K key, @Nullable V value) {
    int pos = data.length == 0 ? -1 : tablePos(strategy, data, key);
    if (pos >= 0) {
      ...
      Iterator<Entry<K, V>> iterator() {
        return new MyIterator<Entry<K, V>>() {
          ...
          Entry<K, V> fieldElement(int offset) {
            return offset == 0 ? new AbstractMap.SimpleImmutableEntry<>(k1, v1) : offset == 1 ? new AbstractMap.SimpleImmutableEntry<>(k2, v2) : new AbstractMap.SimpleImmutableEntry<>(k3, v3);
          }
