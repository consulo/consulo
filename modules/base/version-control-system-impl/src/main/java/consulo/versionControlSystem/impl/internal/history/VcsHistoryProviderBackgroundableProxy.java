...
import consulo.versionControlSystem.impl.internal.VcsBackgroundableComputable;
...
public class VcsHistoryProviderBackgroundableProxy {
    ...
    public VcsHistoryProviderBackgroundableProxy(AbstractVcs vcs, VcsHistoryProvider delegate, DiffProvider diffProvider) {
        ...
        myType = vcs.getType();
        myHistoryComputerFactory = (filePath, consumer, vcsKey) -> {
            if (myCachesHistory) {
    ...
    public void createSessionFor(
        VcsKey vcsKey, FilePath filePath, Consumer<VcsHistorySession> continuation,
        ...
        if (silent) {
            ...
            VcsBackgroundableComputable.createAndRun(
                myProject,
                resultingActionKey,
                key,
                VcsLocalize.loadingFileHistoryProgress().get(),
    ...
    @RequiredUIAccess
    public void executeAppendableSession(
        VcsKey vcsKey,
        FilePath filePath,
    ...
    public void executeAppendableSession(
        ...
        if (!(myDelegate instanceof VcsHistoryProviderEx)) {
    ...
    @RequiredUIAccess
    private void doExecuteAppendableSession(
        VcsKey vcsKey,
        FilePath filePath,
        @Nullable VcsRevisionNumber startRevisionNumber,
        VcsAppendableHistorySessionPartner partner,
    ...
    private VcsAbstractHistorySession getFullHistoryFromCache(VcsKey vcsKey, FilePath filePath) {
        VcsAbstractHistorySession full = myVcsHistoryCache.getFull(
            filePath,
            vcsKey,
            (VcsCacheableHistorySessionFactory<Serializable, VcsAbstractHistorySession>) myDelegate
        );
        if (full != null && myConfiguration.LIMIT_HISTORY && myConfiguration.MAXIMUM_HISTORY_ROWS < full.getRevisionList().size()) {
    ...
    private void reportHistory(
        ...
        final @Nullable VcsRevisionNumber startRevisionNumber,
        final VcsKey vcsKey,
        ...
        ProgressManager.getInstance().run(new Task.Backgroundable(myProject, VcsLocalize.loadingFileHistoryProgress(), true) {
            @Override
            public void run(ProgressIndicator indicator) {
                ...
                try {
                    ...
                    else if (myDelegate instanceof VcsHistoryProviderEx vcsHistoryProviderEx) {
                        vcsHistoryProviderEx.reportAppendableHistory(filePath, startRevisionNumber, cachedPartner);
                    }
                    else {
                    ...
                    Application.get().invokeLater(() -> handler.completed(resultingActionKey), ModalityState.nonModal());
                }
            }
        });
    }
