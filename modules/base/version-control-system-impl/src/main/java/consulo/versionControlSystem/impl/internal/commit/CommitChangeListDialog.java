...
import consulo.versionControlSystem.impl.internal.change.PseudoMap;
...
public class CommitChangeListDialog extends DialogWrapper implements CheckinProjectPanel, TypeSafeDataProvider {
    ...
    private CommitChangeListDialog(
        ...
        if (isAlien) {
            ...
            InternalMultipleChangeListBrowser browser = new InternalMultipleChangeListBrowser(
                project,
                changeLists,
                (List) changes,
                initialSelection,
    ...
    private void showDetailsIfSaved() {
        boolean showDetails = ApplicationPropertiesComponent.getInstance().getBoolean(DETAILS_SHOW_OPTION, DETAILS_SHOW_OPTION_DEFAULT);
        if (showDetails) {
    ...
    public LocalizeValue getCommitActionName() {
        LocalizeValue name = LocalizeValue.absent();
        for (AbstractVcs vcs : getAffectedVcses()) {
            CheckinEnvironment checkinEnvironment = vcs.getCheckinEnvironment();
            if (name.isAbsent() && checkinEnvironment != null) {
        ...
        return name.or(VcsLocalize.commitDialogDefaultCommitOperationName());
    }
    ...
    protected JComponent createCenterPanel() {
        ...
        myDetailsSplitter = new SplitterWithSecondHideable(true, "Diff", rootPane, new SplitterWithSecondHideable.OnOffListener<>() {
            ...
            protected float getSplitterInitialProportion() {
                float value = ApplicationPropertiesComponent.getInstance()
    ...
    private void initMainSplitter() {
        mySplitter.setProportion(
            ApplicationPropertiesComponent.getInstance().getFloat(SPLITTER_PROPORTION_OPTION, SPLITTER_PROPORTION_OPTION_DEFAULT)
        );
    }
