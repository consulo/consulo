...
import consulo.versionControlSystem.impl.internal.action.CachedVcsContext;
...
public class VcsContextWrapper implements VcsContext {
  ...
  @SafeVarargs
  private static <T> Stream<T> concat(Stream<T>... args) {
    return Stream.of(args).reduce(Stream.empty(), Stream::concat);
  }

  @SafeVarargs
  @Nonnull
  private static <T> Stream<T> stream(T... args) {
