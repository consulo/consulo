...
public class ChangesDnDSupport implements DnDDropHandler, DnDTargetChecker {
    ...
    private DnDImage createDraggedImage(DnDActionInfo info) {
        LocalizeValue imageText = VcsLocalize.changesViewDndLabel(getSelectionCount());
        Image image = DnDAwareTree.getDragImage(myTree, imageText.get(), null).getFirst();
    ...
    public boolean update(DnDEvent aEvent) {
        ...
        if (!(aEvent.getAttachedObject() instanceof ChangeListDragBean dragBean)) {
            return false;
        }
    ...
    private boolean fitsInBounds(Rectangle rect) {
        if (myTree.getParent() instanceof JViewport viewport && viewport.getParent() instanceof JScrollPane scrollPane) {
