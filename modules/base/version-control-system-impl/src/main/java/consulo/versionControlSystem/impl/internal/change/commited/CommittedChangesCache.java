...
public class CommittedChangesCache implements PersistentStateComponent<CommittedChangesCache.State> {
  ...
  @Inject
  public CommittedChangesCache(Project project, ProjectLevelVcsManager vcsManager) {
    myProject = project;
      ...
      final VcsListener vcsListener = new VcsListener() {
        ...
        public void directoryMappingChanged() {
          ...
          myTaskQueue.run(new Runnable() {
            @Override
            public void run() {
              List<ChangesCacheFile> files = myCachesHolder.getAllCaches();
  ...
  public boolean isMaxCountSupportedForProject() {
    for (AbstractVcs vcs : myVcsManager.getAllActiveVcss()) {
      ...
      if (provider instanceof CachingCommittedChangesProvider) {
        CachingCommittedChangesProvider cachingProvider = (CachingCommittedChangesProvider)provider;
  ...
  private class MyProjectChangesLoader implements Runnable {
    ...
    private final SequencedSet<CommittedChangeList> myResult = new LinkedHashSet<>();
    ...
    public void run() {
      for (AbstractVcs vcs : myVcsManager.getAllActiveVcss()) {
        ...
        Map<VirtualFile, RepositoryLocation> map = myCachesHolder.getAllRootsUnderVcs(vcs);
        ...
        for (VirtualFile root : map.keySet()) {
          ...
          try {
            List<CommittedChangeList> lists = getChanges(mySettings, root, vcs, myMaxCount, myCacheOnly, provider, location);
            if (lists != null) {
              if (zipSupported) {
  ...
  public void getProjectChangesAsync(ChangeBrowserSettings settings,
                                     int maxCount,
                                     boolean cacheOnly,
                                     Consumer<List<CommittedChangeList>> consumer,
                                     Consumer<List<VcsException>> errorConsumer) {
  ...
  public List<CommittedChangeList> getChanges(ChangeBrowserSettings settings,
                                              ...
                                              RepositoryLocation location) throws VcsException {
  ...
  private boolean canGetFromCache(AbstractVcs vcs,
  ...
  private List<CommittedChangeList> getChangesWithCaching(AbstractVcs vcs,
                                                          ChangeBrowserSettings settings,
                                                          VirtualFile root,
                                                          RepositoryLocation location,
                                                          int maxCount)
  ...
  private List<CommittedChangeList> refreshCache(ChangesCacheFile cacheFile) throws VcsException, IOException {
    List<CommittedChangeList> newLists = new ArrayList<>();

    CachingCommittedChangesProvider provider = cacheFile.getProvider();
    RepositoryLocation location = cacheFile.getLocation();

    Pair<Long, List<CommittedChangeList>> externalLists = myExternallyLoadedChangeLists.get(location.getKey());
    long latestChangeList = getLatestListForFile(cacheFile);
    ...
    List<CommittedChangeList> newChanges = provider.getCommittedChanges(defaultSettings, location, maxCount);
  ...
  private List<CommittedChangeList> appendLoadedChanges(ChangesCacheFile cacheFile,
                                                        RepositoryLocation location,
                                                        List<CommittedChangeList> newChanges) throws IOException {
    List<CommittedChangeList> savedChanges = writeChangesInReadAction(cacheFile, newChanges);
  ...
  public List<CommittedChangeList> loadIncomingChanges(boolean inBackground) {
    ...
    Collection<ChangesCacheFile> caches = myCachesHolder.getAllCaches();

    MultiMap<AbstractVcs, Pair<RepositoryLocation, List<CommittedChangeList>>> byVcs =
      new MultiMap<>();
    ...
    for (AbstractVcs vcs : byVcs.keySet()) {
      ...
      if (vcsZipper != null) {
        VcsCommittedListsZipper incomingZipper = new IncomingListsZipper(vcsZipper);
        CommittedListsSequencesZipper zipper = new CommittedListsSequencesZipper(incomingZipper);
  ...
  private static class IncomingListsZipper extends VcsCommittedListsZipperAdapter {
    ...
    public CommittedChangeList zip(RepositoryLocationGroup group, List<CommittedChangeList> lists) {
      ...
      Set<Change> baseChanges = new HashSet<>();
  ...
  public void commitMessageChanged(AbstractVcs vcs, final RepositoryLocation location, final long number, final String newMessage) {
    myTaskQueue.run(new Runnable() {
      @Override
      public void run() {
        ChangesCacheFile file = myCachesHolder.haveCache(location);
        if (file != null) {
          try {
  ...
  public void loadIncomingChangesAsync(@Nullable final Consumer<List<CommittedChangeList>> consumer, final boolean inBackground) {
    ...
    Runnable task = new Runnable() {
      @Override
      public void run() {
        List<CommittedChangeList> list = loadIncomingChanges(inBackground);
        if (consumer != null) {
  ...
  private void notifyIncomingChangesUpdated(@Nullable Collection<CommittedChangeList> receivedChanges) {
    Collection<CommittedChangeList> changes = receivedChanges == null ? myCachedIncomingChangeLists : receivedChanges;
    if (changes == null) {
  ...
  private CommittedChangesListener getPublisher(Consumer<CommittedChangesListener> listener) {
  ...
  private static class MyRefreshRunnable implements Runnable {
    ...
    @Override
    public void run() {
      CommittedChangesCache cache = myCache;
