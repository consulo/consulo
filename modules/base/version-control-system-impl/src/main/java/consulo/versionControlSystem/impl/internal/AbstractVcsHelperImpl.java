...
import consulo.versionControlSystem.impl.internal.history.FileHistoryRefresher;
import consulo.versionControlSystem.impl.internal.merge.MultipleFileMergeDialog;
import consulo.versionControlSystem.impl.internal.ui.awt.ChangeListViewerDialog;
import consulo.versionControlSystem.impl.internal.ui.awt.ConfirmationDialog;
import consulo.versionControlSystem.impl.internal.ui.awt.SelectFilePathsDialog;
import consulo.versionControlSystem.impl.internal.ui.awt.SelectFilesDialog;
...
public class AbstractVcsHelperImpl extends AbstractVcsHelper {
    ...
    @RequiredUIAccess
    public void openMessagesView(NewErrorTreeViewPanel errorTreeView, String tabDisplayName) {
    ...
    public List<VirtualFile> selectFilesToProcess(
        ...
        if (files.size() == 1 && singleFilePromptGenerator != LocalizeValue.absent()) {
    ...
    public void showErrors(Map<HotfixData, List<VcsException>> exceptionGroups, @Nonnull LocalizeValue tabDisplayName) {
        showErrorsImpl(
            ...
            () -> {
                List<VcsException> exceptionList = exceptionGroups.values().iterator().next();
            ...
            vcsErrorViewPanel -> {
                for (Map.Entry<HotfixData, List<VcsException>> entry : exceptionGroups.entrySet()) {
                    if (entry.getKey() == null) {
                        ...
                        List<VcsException> exceptionList = entry.getValue();
                        List<SimpleErrorData> list = new ArrayList<>(exceptionList.size());
    ...
    public void showAnnotation(FileAnnotation annotation, VirtualFile file, AbstractVcs vcs, int line) {
        ...
        if (textFileEditor != null) {
            ...
            OpenFileDescriptor openFileDescriptor = OpenFileDescriptorFactory.getInstance(myProject).newBuilder(file).line(line).build();
    ...
    public void showChangesBrowser(List<CommittedChangeList> changelists) {
        showChangesBrowser(changelists, LocalizeValue.absent());
    }
    ...
    public void showChangesBrowser(
        ...
        CommittedChangesFilterDialog committedChangesFilterDialog = new CommittedChangesFilterDialog(myProject, filterUI, settings);
        committedChangesFilterDialog.show();
        if (committedChangesFilterDialog.getExitCode() != DialogWrapper.OK_EXIT_CODE) {
            return;
        }

        settings = committedChangesFilterDialog.getSettings();

        if (myProject.isDefault() || (ProjectLevelVcsManager.getInstance(myProject).getAllActiveVcss().length == 0)
            || (!ModalityState.nonModal().equals(Application.get().getCurrentModalityState()))) {
            List<CommittedChangeList> versions = new ArrayList<>();
            ...
            ChangesBrowserDialog dlg = createChangesBrowserDialog(
                model,
                title,
                true,
                parent,
    ...
    public <T extends CommittedChangeList, U extends ChangeBrowserSettings> T chooseCommittedChangeList(
        ...
        ChangesBrowserDialog dlg = new ChangesBrowserDialog(
            myProject,
    ...
    public List<VirtualFile> showMergeDialog(
        ...
        VirtualFileUtil.markDirtyAndRefresh(false, false, false, ArrayUtil.toObjectArray(files, VirtualFile.class));
        MultipleFileMergeDialog fileMergeDialog = new MultipleFileMergeDialog(myProject, files, provider, mergeDialogCustomizer);
    ...
    public void openCommittedChangesTab(
        ...
        if (title.isAbsent() && location != null) {
    ...
    @Override
    public void showSubmittedFiles(
    ...
    public void loadAndShowCommittedChangesDetails(
        ...
        ProgressManagerEx progressManager = (ProgressManagerEx) ProgressManager.getInstance();

        ProgressIndicator backgroundIndicator = progressManager.newBackgroundableProcessIndicator(task);

        progressManager.runProcessWithProgressAsynchronously(
            task,
            backgroundIndicator,
            null,
            myProject.getApplication().getCurrentModalityState()
        );
    }
                        ...
                        public void finished() {
                            ...
                            if (!myRevisionsReturned) {
                                application.invokeLater(
                                    () -> myDlg.close(-1),
                                    application.getModalityStateForComponent(myDlg.getWindow())
                                );
                            }
                        }
                    }
