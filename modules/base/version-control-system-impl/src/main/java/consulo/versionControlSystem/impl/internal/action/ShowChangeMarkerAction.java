...
public abstract class ShowChangeMarkerAction extends AbstractVcsAction {
    ...
    protected abstract VcsRange extractRange(LineStatusTrackerI lineStatusTracker, int line, Editor editor);

    public ShowChangeMarkerAction(final VcsRange range, final LineStatusTrackerI lineStatusTracker, final Editor editor) {
        myChangeMarkerContext = new ChangeMarkerContext() {
            ...
            @Override
            public LineStatusTrackerI getLineStatusTracker(VcsContext dataContext) {
    ...
    public ShowChangeMarkerAction(@Nonnull LocalizeValue text, @Nonnull LocalizeValue description, @Nonnull Image icon) {
        ...
        myChangeMarkerContext = new ChangeMarkerContext() {
            ...
            public VcsRange getRange(VcsContext context) {
                ...
                LineStatusTrackerI lineStatusTracker = getLineStatusTracker(context);
            ...
            public LineStatusTrackerI getLineStatusTracker(VcsContext dataContext) {
                ...
                return LineStatusTrackerManagerI.getInstance(project).getLineStatusTracker(editor.getDocument());
            }
    ...
    protected void actionPerformed(@Nonnull VcsContext context) {
        ...
        VersionControlSystemInternal.getInstance().moveToRange(range, editor, lineStatusTracker);
    }

    protected interface ChangeMarkerContext {
        VcsRange getRange(VcsContext dataContext);

        LineStatusTrackerI getLineStatusTracker(VcsContext dataContext);
