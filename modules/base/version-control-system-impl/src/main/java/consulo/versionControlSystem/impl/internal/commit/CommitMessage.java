...
public class CommitMessage extends AbstractDataProviderPanel implements Disposable, CommitMessageI {
  ...
  public CommitMessage(Project project, boolean withSeparator) {
    ...
    if (withSeparator) {
      mySeparator = SeparatorFactory.createSeparator(VcsLocalize.labelCommitComment().get(), myEditorField.getComponent(), true, true);
  ...
  public static EditorTextField createCommitTextEditor(Project project, boolean forceSpellCheckOn) {
    ...
    EditorTextFieldProvider service = project.getInstance(EditorTextFieldProvider.class);
