        ...
        return NotificationAction.create(
            ...
            (event, notification) -> {
                if (!myProject.isDisposed()) {
                    ShowConfigurableService configurableService = myProject.getApplication().getInstance(ShowConfigurableService.class);

                    configurableService.showAndSelect(myProject, StandardConfigurableIds.VCS_GROUP)
                        .whenComplete((o, throwable) -> BackgroundTaskUtil.executeOnPooledThread(
                            myProject,
                            () -> {
