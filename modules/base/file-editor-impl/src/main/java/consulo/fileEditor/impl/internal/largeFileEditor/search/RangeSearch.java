...
import consulo.fileEditor.impl.internal.largeFileEditor.search.action.FindFurtherAction;
...
import consulo.ui.ex.awt.SingleSelectionModel;
...
public final class RangeSearch implements RangeSearchTask.Callback {
    ...
    private final List<EdtRangeSearchEventsListener> myEdtRangeSearchEventsListeners = Lists.newLockFreeCopyOnWriteList();
    ...
    public void updateTabName() {
        if (myContent != null && lastExecutedRangeSearchTask != null) {
            String name = FileEditorLocalize.largeFileEditorTabNameSomeStringInSomeFile(lastExecutedRangeSearchTask.getOptions().stringToFind, myVirtualFile.getName()).get();
    ...
    public void callScheduledUpdate() {
        if (isScheduledUpdateCalled.compareAndSet(false, true)) {
            Application.get().getLastUIAccess().getScheduler().schedule(() -> {
    ...
    private void updateStatusStringInfo() {
        ...
        LocalizeValue newStatus;
        if (pagesAmount == -1 || myRightBorderPageNumber == UNDEFINED || myLeftBorderPageNumber == UNDEFINED) {
            newStatus = FileEditorLocalize.largeFileEditorMessageFoundSomeMatches(getAmountOfStoredSearchResults());
        }
        else {
            if (myLeftBorderPageNumber == 0 && myRightBorderPageNumber == pagesAmount - 1) {
                newStatus = FileEditorLocalize.largeFileEditorMessageFoundSomeMatchesInTheWholeFile(getAmountOfStoredSearchResults());
            }
            else {
                newStatus = FileEditorLocalize.largeFileEditorMessageFoundSomeMatchesInSomeBoundsOfFile(getAmountOfStoredSearchResults(), Utils.calculatePagePositionPercent(myLeftBorderPageNumber, pagesAmount), Utils.calculatePagePositionPercent(myRightBorderPageNumber, pagesAmount));
            }
        }
    ...
    private final class SearchFurtherBtnWrapper implements ListElementWrapper {
        private final SimpleTextAttributes linkText = new SimpleTextAttributes(
            SimpleTextAttributes.STYLE_PLAIN, JBCurrentTheme.Link.linkPressedColor());
        ...
        public void render(ColoredListCellRenderer coloredListCellRenderer, boolean selected, boolean hasFocus) {
            ...
            if (isForwardDirection) {
                text = FileEditorLocalize.largeFileEditorTextFindNextMatches();
            }
            else {
                text = FileEditorLocalize.largeFileEditorTextFindPreviousMatches();
            }
