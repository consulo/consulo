...
import consulo.fileEditor.impl.internal.ui.NonProjectFileWritingAccessDialog;
...
public class NonProjectFileWritingAccessProvider extends WritingAccessProvider {
    ...
    public static boolean isWriteAccessAllowed(VirtualFile file, Project project) {
        ...
        if (ArrayUtil.contains(file, IdeDocumentHistory.getInstance(project).getChangedFiles())) {
            return true;
        }
        ...
        while (each != null) {
            if (ACCESS_ALLOWED.getValue(each).get() > 0) {
                return true;
            }
            each = each.getParent();
        }
