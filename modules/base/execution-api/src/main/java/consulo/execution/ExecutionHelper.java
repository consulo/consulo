...
public class ExecutionHelper {
    ...
    public static void showOutput(
        ...
        project.getApplication().invokeLater(() -> {
            ...
            errorTreeView.addMessage(
                MessageCategory.SIMPLE,
                new String[]{"Process finished with exit code " + output.getExitCode()},
                null,
                -1,
                -1,
                null
    ...
    public static void selectContentDescriptor(
        ...
        if (consoles.size() == 1) {
            ...
            IPopupChooserBuilder<RunContentDescriptor> builder =
                JBPopupFactory.getInstance().createPopupChooserBuilder(new ArrayList<>(consoles));
    ...
    private static Runnable createCancelableExecutionProcess(
        ...
        return new Runnable() {
                ...
                public void run() {
                    for (; ; ) {
                        if ((myProgressIndicator != null && (myProgressIndicator.isCanceled() || !myProgressIndicator.isRunning()))
                            || (cancelableFun != null && cancelableFun.apply(null))
            ...
            public void run() {
                ...
                LOG.assertTrue(
                    myProgressIndicator != null || cancelableFun != null,
                    "Cancelable process must have an opportunity to be canceled!"
                );
                mySemaphore.down();
                Application.get().executeOnPooledThread(myWaitThread);
                Application.get().executeOnPooledThread(myCancelListener);
    ...
    private static Runnable createTimelimitedExecutionProcess(
        ...
        return new Runnable() {
            ...
            public void run() {
                mySemaphore.down();
                Application.get().executeOnPooledThread(myProcessThread);
