...
class VelocityWrapper {
    ...
    try {
      ...
      if (interfaces.length != 1 || !interfaces[0].equals(ResourceManager.class)) {
        throw new IllegalStateException("Incorrect velocity version in the classpath" +
                                        ", ResourceManager in " +
                                        ClassPathUtil.getJarPathForClass(ResourceManager.class) +
                                        ", ResourceManagerImpl in " +
                                        ClassPathUtil.getJarPathForClass(ResourceManagerImpl.class));
      }
  ...
  static boolean evaluate(@Nullable Project project, Context context, Writer writer, String templateContent) throws ParseErrorException, MethodInvocationException, ResourceNotFoundException {
    try {
