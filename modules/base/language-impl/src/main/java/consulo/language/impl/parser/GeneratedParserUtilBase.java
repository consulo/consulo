...
public class GeneratedParserUtilBase {
  ...
  private static void exit_section_impl_(ErrorState state, Frame frame, PsiBuilder builder, @Nullable IElementType elementType, boolean result, boolean pinned, @Nullable Parser eatMore) {
    ...
    if (!state.suppressErrors && eatMore != null) {
      ...
      PsiBuilderImpl.ProductionMarker latestDoneMarker =
              (pinned || result) && (state.altMode || elementType != null) && eatMoreFlagOnce ? (PsiBuilderImpl.ProductionMarker)builder.getLatestDoneMarker() : null;
    ...
    if (elementType != null && marker != null) {
      if (result || pinned) {
        if ((frame.modifiers & _COLLAPSE_) != 0) {
          ...
          if (last != null && last.getStartIndex() == frame.position && state.typeExtends(last.getTokenType(), elementType) && wasAutoSkipped(builder, builder.rawTokenIndex() - last.getEndIndex())) {
  ...
  private static void replace_variants_with_name_(ErrorState state, Frame frame, PsiBuilder builder, boolean result, boolean pinned) {
    int initialPos = builder.rawTokenIndex();
    boolean willFail = !result && !pinned;
  ...
  private static boolean reportError(PsiBuilder builder, ErrorState state, Frame frame, IElementType elementType, boolean force, boolean advance) {
    ...
    String actual = "'" + StringUtil.first(StringUtil.notNullize(builder.getTokenText(), "null"), MAX_ERROR_TOKEN_TEXT, true) + "'";
  ...
  public static class ErrorState {
    ...
    public BiPredicate<IElementType, IElementType> altExtendsChecker;
    ...
    public boolean typeExtends(IElementType child, IElementType parent) {
      ...
      return altExtendsChecker != null && altExtendsChecker.test(child, parent);
    }
  }
  ...
  private static class Variant {
    ...
    @Override
    public int hashCode() {
      return 31 * position + object.hashCode();
    }
  }
  ...
  public static boolean parseAsTree(ErrorState state, PsiBuilder builder, int level, IElementType chunkType, boolean checkBraces, Parser parser, Parser eatMoreCondition) {
    Deque<Couple<PsiBuilder.Marker>> parenList = new LinkedList<>();
    Deque<Pair<PsiBuilder.Marker, Integer>> siblingList = new LinkedList<>();
    ...
    if (checkParens) {
      ...
      if (doneMarker != null && doneMarker.getStartOffset() == builder.rawTokenTypeStart(tokenIdx) && doneMarker.getTokenType() == TokenType.ERROR_ELEMENT) {
        parenList.add(Couple.of(((PsiBuilder.Marker)doneMarker).precede(), null));
      }
    }
    ...
    while (true) {
      ...
      if (checkParens && (tokenType == state.braces[0].getLeftBraceType() || tokenType == state.braces[0].getRightBraceType() && !parenList.isEmpty())) {
        ...
        if (tokenType == state.braces[0].getLeftBraceType()) {
          ...
          parenList.addFirst(Couple.of(builder.mark(), prev == null ? null : prev.first));
        }
        checkSiblingsRunnable.run();
        builder.advanceLexer();
        if (tokenType == state.braces[0].getRightBraceType()) {
          Couple<PsiBuilder.Marker> pair = parenList.removeFirst();
    ...
    if (marker != null) marker.drop();
    for (Couple<PsiBuilder.Marker> pair : parenList) {
      pair.first.drop();
    }
    return totalCount != 0;
  }
