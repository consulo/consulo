...
public class CodeEditUtil {
    ...
    private static final ThreadLocal<Function<ASTNode, Boolean>> NODE_REFORMAT_STRATEGY = new ThreadLocal<>();
    ...
    private static boolean needToForceReformat(ASTNode parent, ASTNode first, ASTNode last) {
        return parent == null ||
            ...
            parent.getText().trim().length() == getTrimmedTextLength(first, last) && needToForceReformat(
    ...
    public static boolean isMarkedToReformat(ASTNode node) {
        ...
        Function<ASTNode, Boolean> strategy = NODE_REFORMAT_STRATEGY.get();
        return strategy == null || strategy.apply(node);
    }
     ...
     */
    public static void setNodeReformatStrategy(@Nullable Function<ASTNode, Boolean> strategy) {
