...
import static consulo.http.impl.internal.ssl.CertificateWrapper.CommonField;
...
public class CertificateInfoPanel extends JPanel {
    ...
    public CertificateInfoPanel(@Nonnull X509Certificate certificate) {
        ...
        builder = builder
            ...
            .addLabeledComponent(
                "Valid from:",
                createColoredComponent(notBefore, "not yet valid", myCertificateWrapper.isNotYetValid())
            )
            .addLabeledComponent(
                "Valid until:",
                createColoredComponent(notAfter, "expired", myCertificateWrapper.isExpired())
            );
        builder.setIndent(0);
