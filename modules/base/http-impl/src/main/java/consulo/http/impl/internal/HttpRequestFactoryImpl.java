...
public class HttpRequestFactoryImpl implements HttpRequestFactory {
    ...
    private static class RequestBuilderImpl extends RequestBuilder {
        ...
        private HttpRequests.ConnectionTuner myTuner;

        private RequestBuilderImpl(@Nonnull Application application, @Nonnull String url) {
    ...
    private static class RequestImpl implements HttpRequests.Request, AutoCloseable {
    ...
    private static <T> T process(RequestBuilderImpl builder, HttpRequests.RequestProcessor<T> processor) throws IOException {
    ...
    private static <T> T doProcess(RequestBuilderImpl builder, HttpRequests.RequestProcessor<T> processor) throws IOException {
        try (RequestImpl request = new RequestImpl(builder)) {
            return processor.process(request);
        }
    }

    private static Charset getCharset(HttpRequests.Request request) throws IOException {
        ...
        for (int i = 0; i < builder.myRedirectLimit; i++) {
            ...
            if (connection instanceof HttpURLConnection) {
                ...
                if (responseCode < 200 || responseCode >= 300 && responseCode != HttpURLConnection.HTTP_NOT_MODIFIED) {
                    ...
                    throw new HttpRequests.HttpStatusException(message, responseCode, StringUtil.notNullize(url, "Empty URL"));
                }
            }
    ...
    @Nonnull
    public static String createErrorMessage(@Nonnull IOException e, @Nonnull HttpRequests.Request request, boolean includeHeaders) {
    ...
    @Inject
    public HttpRequestFactoryImpl(Application application) {
        myApplication = application;
    }

    @Nonnull
    @Override
    public RequestBuilder request(@Nonnull String url) {
        return new RequestBuilderImpl(myApplication, url);
    }
}
