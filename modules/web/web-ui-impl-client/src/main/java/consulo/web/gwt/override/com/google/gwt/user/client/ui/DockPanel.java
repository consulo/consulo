...
public class DockPanel extends CellPanel implements HasAlignment {
  ...
  public void setCellStyleProperty(Widget w, String name, String value) {
    LayoutData data = (LayoutData)w.getLayoutData();
    if(value != null) {
    ...
    if (data.td != null) {
      data.td.getStyle().setProperty(name, value);
    }
  }
  ...
  private void realizeTable() {
    ...
    for (Iterator<Widget> it = getChildren().iterator(); it.hasNext(); ) {
      ...
      for (Map.Entry<String, String> entry : layout.properties.entrySet()) {
        layout.td.getStyle().setProperty(entry.getKey(), entry.getValue());
      }
