...
/**
 * Copied from GWT [VISTALL]
 * <p>
 * For com.google.gwt.user.cellview.client.TreeNode#getChild(int)
 * <p>
...
class CellTreeNodeView<T> extends UIObject {
    ...
    private class View implements HasDataPresenter.View<C> {
      ...
      public void render(SafeHtmlBuilder sb, List<C> values, int start, SelectionModel<? super C> selectionModel) {
        ...
        for (int i = start; i < end; i++) {
          ...
          SafeStyles innerPadding = SafeStylesUtils.fromTrustedString("padding-" + paddingDirection + ": " + imageWidth + "px;");
          SafeHtml innerDiv = template.innerDiv(innerPadding, innerClasses.toString(), image, itemValueStyle, cellBuilder.toSafeHtml());
          SafeStyles outerPadding = SafeStylesUtils.fromTrustedString("padding-" + paddingDirection + ": " + paddingAmount + "px;");
      ...
      public void setKeyboardSelected(int index, boolean selected, boolean stealFocus) {
        ...
        setStyleName(getSelectionElement(elem), nodeView.tree.getStyle().cellTreeKeyboardSelectedItem(), selected);
      }
  ...
  public static class TreeNodeImpl implements TreeNode {
    ...
    public TreeNode getChild(int index) {
      ...
      checkChildBounds(index);
      flush();
      return new TreeNodeChildImpl(nodeView.getChildNode(index));
    }
  ...
  public static class TreeNodeChildImpl implements TreeNode {

    private CellTreeNodeView<?> nodeView;

    public TreeNodeChildImpl(CellTreeNodeView<?> nodeView) {
      this.nodeView = nodeView;
    }
   ...
   */
  protected <C> CellTreeNodeView<C> createTreeNodeView(NodeInfo<C> nodeInfo, Element childElem, C childValue, Object viewData) {
