...
public class mxCurve {
  ...
  public mxPoint[] getCurveSection(String index, double start, double end) {
    ...
    if (pointsCurve != null &&
      ...
      mxPoint startPoint =
        new mxPoint(firstPointOfSeg.getX() + segVectorX * distanceAlongSeg, firstPointOfSeg.getY() + segVectorY * distanceAlongSeg);
  ...
  public boolean intersectsRect(Rectangle rect) {
    ...
    if (pointsCurve != null && pointsCurve.length > 1) {
      ...
      for (int i = 1; i < pointsCurve.length; i++) {
        if (mxRect.intersectLine(pointsCurve[i].getX(), pointsCurve[i].getY(), pointsCurve[i - 1].getX(), pointsCurve[i - 1].getY()) !=
            null) {
          return true;
        }
      }
  ...
  protected int intersectRectPerimeterSeg(String index, mxRectangle rect, int startSegment) {
    ...
    if (pointsCurve != null && pointsCurve.length > 1) {
      for (int i = startSegment; i < pointsCurve.length; i++) {
        if (rect.intersectLine(pointsCurve[i].getX(), pointsCurve[i].getY(), pointsCurve[i - 1].getX(), pointsCurve[i - 1].getY()) !=
            null) {
          return i;
        }
      }
  ...
  protected void createLabelCurve() {
    ...
    for (int i = 1; i < currentCurve.length; i++) {
      ...
      if (i == 1) {
        ...
        mxPoint startPoint =
          new mxPoint(segEndPoint.getX() - (normSegVectorY * labelBuffer), segEndPoint.getY() + (normSegVectorX * labelBuffer));
      ...
      if (i == currentCurve.length - 1) {
        ...
        // the start of the curve
        mxPoint endPoint =
          new mxPoint(segStartPoint.getX() - (normSegVectorY * labelBuffer), segStartPoint.getY() + (normSegVectorX * labelBuffer));
