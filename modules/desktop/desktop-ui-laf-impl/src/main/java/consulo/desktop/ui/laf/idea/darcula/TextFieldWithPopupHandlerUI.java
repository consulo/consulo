...
public abstract class TextFieldWithPopupHandlerUI extends BasicTextFieldUI implements Condition {
  ...
  private void updateIconsLayout(Rectangle bounds) {
    ...
    Insets margin = UIUtil.getParentOfType(JComboBox.class, c) != null || UIUtil.getParentOfType(JSpinner.class, c) != null ? JBUI.emptyInsets() : getDefaultMargins();
  ...
  protected SearchAction getActionUnder(@Nonnull Point p) {
    return null;
  }
  ...
  @TestOnly
  @Nonnull
  public Point getExtensionIconLocation(@Nonnull final String extensionName) {
  ...
  @Nonnull
  public Rectangle getExtensionIconBounds(@Nonnull Extension extension) {
    for (IconHolder holder : icons.values()) {
  ...
  @Nullable
  private IconHolder getIconHolder(@Nonnull JTextComponent component, int x, int y) {
  ...
  static class MouseDragAwareCaret extends MarginAwareCaret {
    ...
    public void mouseDragged(MouseEvent e) {
      if (e.getID() == MouseEvent.MOUSE_DRAGGED && !getComponent().getText().contains("\n")) {
        ...
        e = new MouseEvent(e.getComponent(), e.getID(), e.getWhen(), e.getModifiers() | e.getModifiersEx(), e.getX(), getComponent().getHeight() / 2, e.getClickCount(), e.isPopupTrigger(), e.getButton());
