 ...
 */
public class DarculaTextBorder_New implements Border, UIResource, ErrorBorderCapable {
  @Override
  ...
  public static void paintDarculaSearchArea(Graphics2D g, Rectangle r, JTextComponent c, boolean fillBackground) {
    ...
    try {
      ...
      if (c.getClientProperty("JTextField.Search.noBorderRing") != Boolean.TRUE) {
        ...
        g2.setColor(DarculaUIUtil_New.getOutlineColor(c.isEnabled() && c.isEditable(), c.hasFocus()));
        g2.fill(path);
      }
    }
    finally {
  ...
  protected Color getOutlineColor(boolean enabled, boolean focused) {
    return DarculaUIUtil_New.getOutlineColor(enabled, focused);
  }

  protected Insets paddings() {
    return DarculaUIUtil_New.paddings();
  }
}
