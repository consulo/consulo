...
import static com.intellij.ide.ui.laf.darcula.ui.TextFieldWithPopupHandlerUI_New.isSearchFieldWithHistoryPopup;
...
public class DarculaUIUtil_New {
  ...
  public static class MacEditorTextFieldBorder extends DarculaEditorTextFieldBorder_New {
    ...
    public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
      if (DarculaEditorTextFieldBorder_New.isComboBoxEditor(c)) {
      ...
      if (isTableCellEditor(c)) {
        ...
        try {
          ...
          outline.append(new Rectangle2D.Float((float)rect.getX() + bw, (float)rect.getY() + bw, (float)rect.getWidth() - 2 * bw, (float)rect.getHeight() - 2 * bw), false);
    ...
    public Insets getBorderInsets(Component c) {
      return isTableCellEditor(c) || isCompact(c) || DarculaEditorTextFieldBorder_New.isComboBoxEditor(c) ? JBUI.insets(2, 3).asUIResource() : JBUI.insets(5, 8).asUIResource();
    }
  }

  public static class WinEditorTextFieldBorder extends DarculaEditorTextFieldBorder_New {
    ...
    public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
      ...
      try {
        ...
        if (editorTextField.isEnabled() && op != null) {
          ...
          else if (editorTextField.isEnabled() && editorTextField.getClientProperty(HOVER_PROPERTY) == Boolean.TRUE) {
    ...
    public Insets getBorderInsets(Component c) {
      if (UIUtil.getParentOfType(ComboBoxCompositeEditor.class, c) != null) {
        ...
        return (isTableCellEditor(c) ? JBUI.insets(1) : DarculaEditorTextFieldBorder_New.isComboBoxEditor(c) ? JBUI.insets(1, 6) : JBUI.insets(4, 6)).asUIResource();
      }
    }
