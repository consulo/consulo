...
public class LookupCellRenderer implements ListCellRenderer<LookupElement> {
    ...
    public LookupCellRenderer(LookupImpl lookup, JComponent editorComponent) {
        ...
        SingleAlarm alarm = new SingleAlarm(
            this::updateLookupWidthFromVisibleItems,
            50,
            Alarm.ThreadToUse.SWING_THREAD,
            IdeaModalityState.stateForComponent(editorComponent),
            lookup
        );
    ...
    public Component getListCellRendererComponent(
        ...
        LookupElementPresentation presentation = myAsyncRendering.getLastComputed(value);
        ...
        allowedWidth -= setItemTextLabel(value, itemColor, isSelected, presentation, allowedWidth);

        Font font = myLookup.getCustomFont(value, false);
        if (font == null) {
            font = myNormalFont;
        }
        ...
        Color grayedForeground = getGrayedForeground(isSelected);
        ...
        double w = myNameComponent.getPreferredSize().getWidth() +
    ...
    private int getMaxWidth() {
        if (myMaxWidth < 0) {
            Point p = myLookup.getComponent().getLocationOnScreen();
    ...
    private void setTailTextLabel(
        ...
        for (LookupElementPresentation.TextFragment fragment : presentation.getTailFragments()) {
            ...
            myTailComponent.append(
                trimmed,
                new SimpleTextAttributes(fragmentStyle, getTailTextColor(isSelected, fragment, foreground, nonFocusedSelection))
            );
    ...
    private String trimLabelText(@Nullable String text, int maxWidth, FontMetrics metrics) {
        ...
        int strWidth = getStringWidth(text, metrics);
        ...
        while (i + 1 < j) {
            ...
            int width = getStringWidth(candidate, metrics);
            if (width <= maxWidth) {
                i = mid;
            }
            else {
    ...
    private static Color getTailTextColor(
        ...
        if (!isSelected) {
            ColorValue tailForeground = fragment.getForegroundColor();
    ...
    private int setItemTextLabel(
        ...
        FontMetrics metrics = getRealFontMetrics(item, bold);
        String name = trimLabelText(presentation.getItemText(), allowedWidth, metrics);
    ...
    private void renderItemName(
        ...
        String prefix = item instanceof EmptyLookupItem ? "" : myLookup.itemPattern(item);
        if (prefix.length() > 0) {
    ...
    private int setTypeTextLabel(
        ...
        String labelText = trimLabelText(StringUtil.isEmpty(givenText) ? "" : " " + givenText, allowedWidth, normalMetrics);

        int used = getStringWidth(labelText, normalMetrics);

        Image icon = presentation.getTypeIcon();
        if (icon != null) {
    ...
    private void updateIconWidth(@Nullable Image baseIcon) {
        ...
        if (icon.getWidth() > myEmptyIcon.getWidth() || icon.getHeight() > myEmptyIcon.getHeight()) {
            myEmptyIcon = Image.empty(
                Math.max(icon.getWidth(), myEmptyIcon.getWidth()),
