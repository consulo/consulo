...
public class DesktopDockManagerImpl extends BaseDockManager implements FileEditorDockManager {
    ...
    private class DockWindow extends FrameWrapper implements Predicate<AWTEvent>, BaseDockManager.DockWindow {
        ...
        private final Map<Class<? extends IdeRootPaneNorthExtension>, IdeRootPaneNorthExtension> myNorthExtensions = new LinkedHashMap<>();
        ...
        private void updateNorthPanel() {
            ...
            HashSet<Class<?>> processedKeys = new HashSet<>();
            myProject.getExtensionPoint(IdeRootPaneNorthExtension.class).forEachExtensionSafe(each -> {
                processedKeys.add(each.getApiClass());
                if (myNorthExtensions.containsKey(each.getApiClass())) {
                    return;
                }

                IdeRootPaneNorthExtension toInstall = each.copy();
                myNorthExtensions.put(toInstall.getApiClass(), toInstall);
                myNorthPanel.add(toInstall.getComponent());
            });

            Iterator<Class<? extends IdeRootPaneNorthExtension>> existing = myNorthExtensions.keySet().iterator();
            while (existing.hasNext()) {
                Class<? extends IdeRootPaneNorthExtension> each = existing.next();
