...
class DesktopColorPicker extends JPanel implements DocumentListener {
    ...
    private JComponent buildTopPanel(boolean enablePipette) throws ParseException {
        ...
        if (enablePipette && ColorPipette.isAvailable()) {
            ...
            pipette.setIcon(TargetAWT.to(PlatformIconGroup.idePipette()));
            pipette.setBorder(JBUI.Borders.empty());
            pipette.setRolloverIcon(TargetAWT.to(PlatformIconGroup.idePipette_rollover()));
            ...
            pipette.addActionListener(e -> {
                ...
                //  .createBalloon().show(new RelativePoint(pipette, new Point(pipette.getWidth() / 2, 0)), StaticPosition.ABOVE);
            });
    ...
    private static class ColorWheel extends JComponent {
        ...
        private final List<ColorListener> myListeners = Lists.newLockFreeCopyOnWriteList();
    ...
    private static class ColorPreviewComponent extends JComponent {
        ...
        protected void paintComponent(Graphics g) {
            ...
            g.setColor(Color.BLACK);
            g.drawRect(i.left, i.top, width - JBUI.scale(1), height - JBUI.scale(1));

            g.setColor(Color.WHITE);
            g.drawRect(i.left + JBUI.scale(1), i.top + JBUI.scale(1), width - JBUI.scale(3), height - JBUI.scale(3));
        }
    }
    ...
    private class RecentColorsComponent extends JComponent {
                ...
                for (String color : colors) {
                    if (color.contains("-")) {
                        ...
                        if (components.size() == 4) {
                            myRecentColors.add(new Color(
                                Integer.parseInt(components.get(0)),
        ...
        protected void paintComponent(Graphics g) {
            ...
            g.drawLine(
                ...
                left + getComponentWidth() - JBUI.scale(3),
                i.top + getComponentHeight() / 2
            );
            g.drawRect(
                left + JBUI.scale(1),
            ...
            for (int k = 1; k < 10; k++) {
                g.drawLine(
                    left + JBUI.scale(1) + JBUI.scale(k * 31),
                    top + JBUI.scale(1),
                    left + JBUI.scale(1) + JBUI.scale(k * 31),
            ...
            for (int r = 0; r < myRecentColors.size(); r++) {
                ...
                g.setColor(color);
                g.fillRect(
                    left + JBUI.scale(2) + JBUI.scale(col * 30) + JBUI.scale(col) + JBUI.scale(1),
                    top + JBUI.scale(2) + JBUI.scale(row * 30) + JBUI.scale(row) + JBUI.scale(1),
                    JBUI.scale(28),
                    JBUI.scale(28)
                );
            }
    ...
    private static class ColorPipette implements ImageObserver {
        ...
        private Dialog getPicker() {
            if (myPickerFrame == null) {
                ...
                TargetAWT.to(PlatformIconGroup.idePipette()).paintIcon(null, graphics, 14, 0);

                graphics.dispose();

                myImage = myParent.getGraphicsConfiguration()
                    .createCompatibleImage(myMagnifierImage.getWidth(), myMagnifierImage.getHeight(), Transparency.TRANSLUCENT);
                ...
                myTimer = new Timer(5, e -> updatePipette());
            }
            ...
            if (myPickerFrame != null && myPickerFrame.isShowing()) {
                ...
                if (!c.equals(myPreviousColor) || !mouseLoc.equals(myPreviousLocation)) {
                    ...
                    if (myColorListener != null) {
                        ...
                        myColorListenersNotifier.addRequest(() -> myColorListener.colorChanged(c, ColorPipette.this), 300);
                    }
                }
            }
        }
