...
public class AWTDiffUtil {
    ...
    public static List<JComponent> createTextTitles(@Nonnull ContentDiffRequest request, @Nonnull List<? extends Editor> editors) {
        ...
        for (int i = 0; i < contents.size(); i++) {
            JComponent title =
                createTitle(StringUtil.notNullize(titles.get(i)), contents.get(i), equalCharsets, equalSeparators, editors.get(i));
            title = createTitleWithNotifications(title, contents.get(i));
            result.add(title);
        }
    ...
    public static JComponent createTitle(
        ...
        if (readOnly) {
            title += " " + DiffLocalize.diffContentReadOnlyContentTitleSuffix().get();
        }
