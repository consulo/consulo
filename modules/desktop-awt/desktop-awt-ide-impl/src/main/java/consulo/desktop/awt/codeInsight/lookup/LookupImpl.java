...
public class LookupImpl extends LightweightHint implements LookupEx, Disposable, LookupElementListPresenter {
  ...
  private static int insertLookupInDocumentWindowIfNeeded(Project project,
    ...
    return document.injectedToHost(insertLookupInDocument(offset,
                                                          document,
                                                          prefix - diff,
  ...
  public boolean showLookup() {
    ...
    if (ScreenReader.isActive()) {
      ...
      AnActionEvent actionEvent =
        AnActionEvent.createFromDataContext(ActionPlaces.EDITOR_POPUP, null, ((EditorEx)myEditor).getDataContext());
  ...
  private void delegateActionToEditor(@Nonnull String actionID,
    ...
    DumbAwareAction.create(e -> ActionUtil.performActionDumbAware(delegateActionSupplier == null ? action : delegateActionSupplier.get(),
                                                                  actionEvent))
      ...
      public boolean onClick(@Nonnull MouseEvent e, int clickCount) {
        ...
        if (clickCount == 2) {
          CommandProcessor.getInstance()
                          .executeCommand(myProject, () -> finishLookup(NORMAL_SELECT_CHAR), "", null, myEditor.getDocument());
        }
        return true;
      }
    }.installOn(myList);
  ...
  public void showElementActions(@Nullable InputEvent event) {
    ...
    Point p = (itemBounds.intersects(visibleRect) || event == null)
      ...
      : SwingUtilities.convertPoint(event.getComponent(),
