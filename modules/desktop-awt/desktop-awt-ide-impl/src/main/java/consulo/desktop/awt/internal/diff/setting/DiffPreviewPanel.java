...
import consulo.desktop.awt.internal.diff.simple.SimpleThreesideDiffChange;
import consulo.desktop.awt.internal.diff.simple.SimpleThreesideDiffViewer;
import consulo.ide.impl.diff.DiffLineSeparatorRenderer;
...
import consulo.diff.util.TextDiffTypeFactory.TextDiffTypeImpl;
...
import static consulo.diff.impl.internal.TextDiffSettingsHolder.TextDiffSettings;
...
class DiffPreviewPanel implements PreviewPanel {
  ...
  private SimpleThreesideDiffChange getChange(@Nonnull ThreeSide side, int line) {
    for (SimpleThreesideDiffChange change : myViewer.getChanges()) {
      ...
      if (DiffImplUtil.isSelectedByLine(line, startLine, endLine) && change.isChange(side)) {
        return change;
      }
    }
    return null;
