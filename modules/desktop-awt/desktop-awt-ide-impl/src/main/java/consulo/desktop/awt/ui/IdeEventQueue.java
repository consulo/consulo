...
public class IdeEventQueue extends EventQueue {
    ...
    public String runnablesWaitingForFocusChangeState() {
        return StringUtil.join(
            focusEventsList,
            event -> "[" + event.getID() + "; " + event.getSource().getClass().getName() + "]",
            ", "
        );
    }
    ...
    private void ifFocusEventsInTheQueue(Consumer<? super AWTEvent> yes, Runnable no) {
        if (!focusEventsList.isEmpty()) {
            ...
            if (first != null) {
                if (FOCUS_AWARE_RUNNABLES_LOG.isDebugEnabled()) {
                    FOCUS_AWARE_RUNNABLES_LOG.debug(
                        "    runnable saved for : [" + first.getID() + "; " + first.getSource() + "] -> " + no.getClass().getName()
                    );
                }
                yes.accept(first);
            }
    ...
    private void abracadabraDaberBoreh() {
        ...
        //    AAAA we missed event, because postEventPrivate() can't be overridden.
    ...
    public void addDispatcher(Predicate<AWTEvent> dispatcher, Disposable parent) {
    ...
    private static void _addProcessor(
        Predicate<AWTEvent> dispatcher,
        Disposable parent,
        Collection<Predicate<AWTEvent>> set
    ) {
        set.add(dispatcher);
        if (parent != null) {
    ...
    private void fixNestedSequenceEvent(AWTEvent e) {
    ...
    private static AWTEvent fixNonEnglishKeyboardLayouts(AWTEvent e) {
        ...
        if (!Registry.is("ide.non.english.keyboard.layout.fix")
            || externalizable == null
            || !externalizable.isNonEnglishKeyboardSupportEnabled()) {
            return e;
        }
    ...
    private static AWTEvent mapXWindowMouseEvent(MouseEvent src) {
        if (src.getButton() < 6) {
            ...
            return new MouseWheelEvent(
                ...
                src.getWhen(),
                src.getModifiers() | InputEvent.SHIFT_DOWN_MASK,
                src.getX(),
                src.getY(),
                0,
                false,
        ...
        return new MouseEvent(
            ...
            src.getID(),
            src.getWhen(),
            src.getModifiers() | (1 << 8 + src.getButton()),
            src.getX(),
            src.getY(),
            1,
            src.isPopupTrigger(),
    ...
    private AWTEvent mapMetaState(AWTEvent e) {
        if (myWinMetaPressed) {
            ...
            weAreNotActive |= e instanceof FocusEvent fe && fe.getOppositeComponent() == null;
            if (weAreNotActive) {
        ...
        if (e instanceof KeyEvent ke) {
            ...
            if (myWinMetaPressed) {
                return new KeyEvent(
                    ...
                    ke.getID(),
                    ke.getWhen(),
                    ke.getModifiers() | ke.getModifiersEx() | InputEvent.META_MASK,
                    ke.getKeyCode(),
                    ke.getKeyChar(),
                    ke.getKeyLocation()
                );
        ...
        if (myWinMetaPressed && e instanceof MouseEvent me && me.getButton() != 0) {
            return new MouseEvent(
                me.getComponent(),
                me.getID(),
                me.getWhen(),
                me.getModifiers() | me.getModifiersEx() | InputEvent.META_MASK,
                me.getX(),
                me.getY(),
                me.getClickCount(),
                me.isPopupTrigger(),
    ...
    public void _dispatchEvent(AWTEvent e) {
        ...
        if (e instanceof MouseWheelEvent mwe) {
            ...
            if (selectedPath.length > 0 && !(selectedPath[0] instanceof ComboPopup)) {
                mwe.consume();
                if (selectedPath[0].getComponent() instanceof JBPopupMenu menu) {
                    menu.processMouseWheelEvent(mwe);
                }
                return;
            }
        }
        ...
        if (e instanceof ComponentEvent ce && myWindowManager != null) {
            myWindowManager.dispatchComponentEvent(ce);
        }
        ...
        if (e instanceof KeyEvent ke) {
            ...
            if (me.getID() == MOUSE_PRESSED && me.getModifiers() > 0 && me.getModifiersEx() == 0) {
                ...
                // from inner WeakReference (see mouseEventTarget field) even if the component has been already removed from component
                // hierarchy. So we have to reset this WeakReference with synthetic event just before processing of actual event
            ...
            if (IdeMouseEventDispatcher.patchClickCount(me) && me.getID() == MouseEvent.MOUSE_CLICKED) {
                MouseEvent toDispatch = new MouseEvent(
                    me.getComponent(),
                    me.getID(),
    ...
    private boolean dispatchByCustomDispatchers(AWTEvent e) {
    ...
    private static void fixStickyWindow(KeyboardFocusManager mgr, Window wnd, String resetMethod) {
        if (wnd != null && !wnd.isShowing()) {
            ...
            if (showingWindow != null && showingWindow != wnd) {
                Method setActive = ReflectionUtil.findMethod(
                    ReflectionUtil.getClassDeclaredMethods(KeyboardFocusManager.class, false),
                    resetMethod,
                    Window.class
                );
                if (setActive != null) {
    ...
    private static boolean consumeUnrelatedEvent(Component modalComponent, AWTEvent event) {
        boolean consumed = false;
        if (event instanceof InputEvent ie && event.getSource() instanceof Component c) {
    ...
    private static class EditingCanceller implements Predicate<AWTEvent> {
        ...
        public boolean test(AWTEvent e) {
            if (e instanceof KeyEvent ke && ke.getID() == KeyEvent.KEY_PRESSED && ke.getKeyCode() == KeyEvent.VK_ESCAPE) {
                Component owner =
                    UIUtil.findParentByCondition(
                        KeyboardFocusManager.getCurrentKeyboardFocusManager().getFocusOwner(),
                        component -> component instanceof JTable || component instanceof JTree
                    );

                if (owner instanceof JTable jTable && jTable.isEditing()) {
                    jTable.editingCanceled(null);
                    return true;
                }
                if (owner instanceof JTree jTree && jTree.isEditing()) {
                    jTree.cancelEditing();
                    return true;
                }
            }
    ...
    private void onFocusEvent(AWTEvent event) {
        if (FOCUS_AWARE_RUNNABLES_LOG.isDebugEnabled()) {
        ...
        for (AWTEvent entry : events) {
            ...
            for (Runnable r : runnables) {
                if (r != null && !(r instanceof ExpirableRunnable expirableRunnable && expirableRunnable.isExpired())) {
                    try {
                        r.run();
                    }
                    catch (Throwable e) {
    ...
    boolean doPostEvent(AWTEvent event) {
    ...
    private static boolean isKeyboardEvent(AWTEvent event) {
    ...
    private static void dispatchTrayIconEvent(AWTEvent e) {
        if (e instanceof ActionEvent ae) {
    ...
    public interface PostEventHook extends EventListener {
        /**
         * @return true if event is handled by the listener and shouldn't be added to event queue at all
         */
        boolean consumePostedEvent(AWTEvent event);
    }

    public void addPostEventListener(PostEventHook listener, Disposable parentDisposable) {
