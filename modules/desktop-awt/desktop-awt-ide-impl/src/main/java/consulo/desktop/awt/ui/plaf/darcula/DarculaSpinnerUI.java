...
public class DarculaSpinnerUI extends BasicSpinnerUI {
  ...
  protected Dimension getSizeWithButtons(Insets i, Dimension size) {
    ...
    return new Dimension(Math.max(size.width, i.left + m.left + editorSize.width + m.right + arrowSize.width), Math.max(size.height, i.top + m.top + editorSize.height + m.bottom + i.bottom));
  }
  ...
  protected void layout() {
    ...
    if (editor != null) {
      ...
      editor.setBounds(i.left + m.left, i.top + m.top + editorOffset, w - (i.left + abSize.width + m.left + m.right), editorHeight);
    }
  }
    ...
    BasicArrowButton b = new BasicArrowButton(direction) {
      ...
      public void paintTriangle(Graphics g, int x, int y, int size, int direction, boolean isEnabled) {
        ...
        try {
          ...
          g2.setColor(JBCurrentTheme.Arrow.backgroundColor(isEnabled, true));
          g2.fill(getInnerShape(lw, bw));

          // Paint side line
          int h = getHeight() - JBUIScale.scale(1);
          Rectangle2D sideLine = direction == NORTH ? new Rectangle2D.Float(0, bw + lw, lw, h - (bw + lw)) : new Rectangle2D.Float(0, 0, lw, h - (bw + lw));
          ...
          // Paint arrow
          g2.translate(x, y);
          g2.setColor(JBCurrentTheme.Arrow.foregroundColor(isEnabled));
      ...
      public Dimension getPreferredSize() {
        ...
        return new Dimension(ARROW_BUTTON_WIDTH.get() + i.left, minHeight + (direction == SwingConstants.NORTH ? i.top : i.bottom));
      }
    };
