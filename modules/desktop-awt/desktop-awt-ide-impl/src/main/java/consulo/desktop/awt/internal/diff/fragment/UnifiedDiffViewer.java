...
public class UnifiedDiffViewer extends ListenerDiffViewerBase {
  ...
  private EditorHighlighter buildHighlighter(@Nullable Project project,
    ...
    if (highlighter1 == null) highlighter1 = DiffImplUtil.initEmptyEditorHighlighter(text1);
    if (highlighter2 == null) highlighter2 = DiffImplUtil.initEmptyEditorHighlighter(text2);
  ...
  private abstract class ApplySelectedChangesActionBase extends AnAction implements DumbAware {
    ...
    protected boolean isSomeChangeSelected() {
      ...
      for (UnifiedDiffChange change : changes) {
        if (DiffImplUtil.isSelectedByLine(line, change.getLine1(), change.getLine2())) return true;
      }
      return false;
    }
  ...
  public boolean isEditable(@Nonnull Side side, boolean respectReadOnlyLock) {
    return !(myReadOnlyLockSet && respectReadOnlyLock) && !side.select(myForceReadOnlyFlags) && DiffImplUtil.canMakeWritable(getDocument(side));
  }
  ...
  protected UnifiedDiffChange getCurrentChange() {
    ...
    for (UnifiedDiffChange change : myChangedBlockData.getDiffChanges()) {
      if (DiffImplUtil.isSelectedByLine(caretLine, change.getLine1(), change.getLine2())) return change;
    }
    return null;
  }
