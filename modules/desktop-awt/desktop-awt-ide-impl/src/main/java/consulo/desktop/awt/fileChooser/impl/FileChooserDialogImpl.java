...
public class FileChooserDialogImpl extends DialogWrapper implements FileChooserDialog, PathChooserDialog, FileLookup {
    ...
    protected JComponent createCenterPanel() {
        ...
        myPath.setRenderer(SimpleListCellRenderer.<String>create((label, value, index) -> {
            ...
            label.setIcon(
                file == null
                    ...
                    : VirtualFileManager.getInstance().getFileIcon(file, null, Iconable.ICON_FLAG_READ_STATUS)
            );
        }));
        ...
        Application.get()
            ...
            .subscribe(
                ApplicationActivationListener.class,
    ...
    protected JTree createTree() {
        ...
        myFileSystemTree.addListener(selection -> updatePathFromTree(selection, false), myDisposable);
    ...
    private VirtualFile[] getSelectedFilesInt() {
        if (myTreeIsUpdating || !myUiUpdater.isEmpty()) {
            if (isTextFieldActive() && !StringUtil.isEmpty(myPathTextField.getTextFieldText())) {
                ...
                if (toFind instanceof LocalFsFinder.VfsFile vfsFile && toFind.exists()) {
                    VirtualFile file = vfsFile.getFile();
                    if (file != null) {
        ...
        public void valueChanged(TreeSelectionEvent e) {
            ...
            for (TreePath treePath : paths) {
                if (e.isAddedPath(treePath)) {
                    ...
                    if (file == null || !myChooserDescriptor.isFileSelectable(file)) {
                        enabled = false;
                    }
                }
            }
    ...
    private void updateTreeFromPath(final String text) {
        ...
        if (myPathTextField.isPathUpdating()) {
            return;
        }
        if (text == null) {
            return;
