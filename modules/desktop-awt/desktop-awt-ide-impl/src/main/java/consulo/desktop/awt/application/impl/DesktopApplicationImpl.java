...
public class DesktopApplicationImpl extends BaseApplication {
  ...
  public DesktopApplicationImpl(
    ...
    if (!isHeadless) {
      ...
      WindowsCommandLineProcessor.LISTENER = (currentDirectory, commandLine) -> {
        LOG.info(
          "Received external Windows command line: current directory " + currentDirectory + "," +
  ...
  private static boolean confirmExitIfNeeded(boolean exitConfirmed) {
    ...
    DialogWrapper.DoNotAskOption option = new DialogWrapper.DoNotAskOption() {
      ...
      public String getDoNotShowMessage() {
        return LocalizeValue.localizeTODO("Do not ask me again").get();
      }
    };
    ...
    if (hasUnsafeBgTasks || option.isToBeShown()) {
      ...
      if (MessageDialogBuilder.yesNo(ApplicationLocalize.exitConfirmTitle().get(), message)
  ...
  private ProgressWindow createProgressWindow(
    ...
    ProgressWindow progress =
      new ProgressWindow(canBeCanceled, !shouldShowModalWindow, (Project)project, parentComponent, cancelText);
  ...
  public void assertReadAccessAllowed() {
    if (!isReadAccessAllowed()) {
      LOG.error(
        ...
        "; dispatch thread: " + EventQueue.isDispatchThread() +
  ...
  private void assertIsDispatchThread(@Nonnull String message) {
    if (isDispatchThread()) return;
    final Attachment dump = AttachmentFactory.get()
      .create("threadDump.txt", ThreadDumper.dumpThreadsToString());
  ...
  private void assertIsIsWriteThread(@Nonnull String message) {
    if (isWriteThread()) return;
    final consulo.logging.attachment.Attachment dump = AttachmentFactory.get()
      .create("threadDump.txt", ThreadDumper.dumpThreadsToString());
    throw new LogEventException(
      message,
      " EventQueue.isDispatchThread()=" +
