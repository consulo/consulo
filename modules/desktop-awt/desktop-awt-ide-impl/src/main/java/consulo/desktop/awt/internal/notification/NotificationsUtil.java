...
public class NotificationsUtil {
    ...
    public static String buildHtml(@Nonnull Notification notification, @Nullable String style) {
        ...
        if (title.length() > TITLE_LIMIT || content.length() > CONTENT_LIMIT) {
            LOG.info(
                "Too large notification " + notification + " of " + notification.getClass() +
    ...
    public static HyperlinkListener wrapListener(@Nonnull Notification notification) {
        ...
        return e -> {
            if (e.getEventType() == HyperlinkEvent.EventType.ACTIVATED) {
                NotificationListener listener1 = notification.getListener();
                if (listener1 != null) {
    ...
    public static Image getIcon(@Nonnull Notification notification) {
        ...
        return switch (notification.getType()) {
            case WARNING -> PlatformIconGroup.generalBalloonwarning();
            case ERROR -> PlatformIconGroup.ideFatalerror();
            case INFORMATION -> PlatformIconGroup.generalBallooninformation();
        };
    }
    ...
    public static MessageType getMessageType(@Nonnull Notification notification) {
        return switch (notification.getType()) {
            ...
            case INFORMATION -> MessageType.INFO;
        };
    }
}
