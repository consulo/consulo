...
public class NotificationsManagerImpl extends NotificationsManager {
    ...
    public void doNotify(@Nonnull Notification notification, @Nullable Project project) {
        ...
        if (NotificationsConfigurationImpl.getInstanceImpl().SHOW_BALLOONS) {
            DumbAwareRunnable runnable = () -> showNotification(notification, project);
            if (project == null) {
        ...
        switch (type) {
            ...
            case TOOL_WINDOW:
                ...
                HyperlinkListener listener =
                    notificationListener == null ? null : e -> notificationListener.hyperlinkUpdate(notification, e);
                ...
                ToolWindowManager.getInstance(project)
                    .notifyByBalloon(toolWindowId, messageType.toUI(), msg, notification.getIcon(), listener);
        }
    }
    ...
    private Balloon notifyByBalloon(
        ...
        if (ideFrame != null) {
            ...
            SimpleReference<Object> layoutDataRef = SimpleReference.create();
            if (project == null || project.isDefault()) {
    ...
    public Balloon createBalloon(
        ...
        if (layoutData.showFullContent) {
            style =
                prefSize > BalloonLayoutConfiguration.MaxFullContentWidth() ? BalloonLayoutConfiguration.MaxFullContentWidthStyle() : null;
        }
        ...
        if (showFullContent) {
            ...
            expandAction = new LinkLabel<>(
                null,
                PlatformIconGroup.ideNotificationExpand(),
                (LinkListener<Void>) (link, ignored) -> {
                    ...
                    if (layoutData.showMinSize) {
                        ...
                        pane.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_NEVER);
                        link.setIcon(TargetAWT.to(PlatformIconGroup.ideNotificationExpand()));
                        link.setHoveringIcon(TargetAWT.to(PlatformIconGroup.ideNotificationExpandhover()));
                    }
                    else {
                        text.select(0, 0);
                        size1.height = layoutData.fullHeight;
                        pane.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED);
                        link.setIcon(TargetAWT.to(PlatformIconGroup.ideNotificationCollapse()));
                        link.setHoveringIcon(TargetAWT.to(PlatformIconGroup.ideNotificationCollapsehover()));
                    }
            ...
            expandAction.setHoveringIcon(TargetAWT.to(PlatformIconGroup.ideNotificationExpandhover()));
        }
        ...
        if (!layoutData.welcomeScreen) {
            ...
            iconComponent.setPreferredSize(new Dimension(
                layoutData.configuration.iconPanelWidth,
    ...
    private static JPanel createButtons(
        ...
        if (notification instanceof NotificationActionProvider provider) {
            ...
            final SimpleReference<JButton> defaultButton = new SimpleReference<>();
    ...
    private static void createMergeAction(@Nonnull final BalloonLayoutData layoutData, @Nonnull JPanel panel) {
        ...
        LinkLabel<BalloonLayoutData> action = new LinkLabel<>(
            ...
            null,
            (aSource, layoutData1) ->
                EventLog.showNotification(layoutData1.project, layoutData1.groupId, layoutData1.getMergeIds()),
            layoutData
        ) {
            @Override
    ...
    public static int calculateContentHeight(int lines) {
        ...
        text.setText(NotificationsUtil.buildHtml(
            null,
            null,
    ...
    private static class DropDownAction extends LinkLabel<Void> {
        Image myIcon = PlatformIconGroup.ideNotificationDroptriangle();
    ...
    private static class CenteredLayoutWithActions extends BorderLayout {
        ...
        private Dimension layoutSize(@Nonnull Function<Component, Dimension> size) {
            ...
            int height = myLayoutData.configuration.topSpaceHeight + titleSize.height + centeredSize.height + Math.max(
                actionSize.height,
                expandSize.height
