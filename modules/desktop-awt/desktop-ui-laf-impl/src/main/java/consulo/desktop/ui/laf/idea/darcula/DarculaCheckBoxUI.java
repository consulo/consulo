...
public class DarculaCheckBoxUI extends MetalCheckBoxUI {
  ...
  private static void updateTextPosition(AbstractButton b) {
    b.setVerticalTextPosition(DarculaUIUtil.isMultiLineHTML(b.getText()) ? SwingConstants.TOP : SwingConstants.CENTER);
  }
  ...
  protected void drawCheckIcon(JComponent c, Graphics2D g, AbstractButton b, Rectangle iconRect, boolean selected, boolean enabled) {
    ...
    try {
      ...
      if (op != null) {
        ...
        outline.append(new RoundRectangle2D.Float(iconRect.x + JBUIScale.scale(1), iconRect.y, JBUIScale.scale(18), JBUIScale.scale(18), JBUIScale.scale(8), JBUIScale.scale(8)), false);
        outline.append(new RoundRectangle2D.Float(iconRect.x + JBUIScale.scale(4), iconRect.y + JBUIScale.scale(3), JBUIScale.scale(12), JBUIScale.scale(12), JBUIScale.scale(3), JBUIScale.scale(3)),
                       false);
  ...
  protected void drawText(JComponent c, Graphics2D g, AbstractButton b, FontMetrics fm, Rectangle textRect, String text) {
    if (text != null) {
      ...
      if (v != null) {
        ...
        BasicGraphicsUtils.drawStringUnderlineCharAt(b, g, text, getMnemonicIndex(b), textRect.x, textRect.y + fm.getAscent());
      }
    }
  }
  ...
  static Dimension computeCheckboxPreferredSize(JComponent c, Icon defaultIcon) {
    ...
    SwingUtilities
            .layoutCompoundLabel(c, fm, text, buttonIcon, b.getVerticalAlignment(), b.getHorizontalAlignment(), b.getVerticalTextPosition(), b.getHorizontalTextPosition(), prefViewRect, prefIconRect,
                                 prefTextRect, text == null ? 0 : b.getIconTextGap());
