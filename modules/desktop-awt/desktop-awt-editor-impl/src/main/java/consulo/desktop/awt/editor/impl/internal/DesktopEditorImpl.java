...
import consulo.desktop.awt.editor.impl.internal.stickyLine.VisualStickyLines;
...
public final class DesktopEditorImpl extends CodeEditorBase
    ...
    private int logicalToVisualLine(int logicalLine) {
        return logicalLine < myDocument.getLineCount() ? offsetToVisualLine(myDocument.getLineStartOffset(logicalLine)) : logicalToVisualPosition(
            new LogicalPosition(logicalLine, 0)).line;
    }
    ...
    private boolean checkIgnore(MouseEvent e) {
        ...
        if (myInitialMouseEvent != null && (e.getComponent() != myInitialMouseEvent.getComponent() || !e.getPoint()
            .equals(myInitialMouseEvent.getPoint()))) {
    ...
    private void processMouseDragged(MouseEvent e) {
        ...
        if (eventArea == EditorMouseEventArea.LINE_MARKERS_AREA || eventArea == EditorMouseEventArea.FOLDING_OUTLINE_AREA && !isInsideGutterWhitespaceArea(
            e)) {
    ...
    private void onSubstantialDrag(MouseEvent mouseDragEvent) {
        IdeEventQueueProxy.getInstance().blockNextEventsForActions(mouseDragEvent);
    ...
    private void setMouseSelectionState(@MouseSelectionState int mouseSelectionState) {
        if (getMouseSelectionState() == mouseSelectionState) {
            return;
        }
    ...
    private class MyInputMethodHandler implements InputMethodRequests {
        ...
        private Point getLocationOnScreen(Component component) {
            ...
            if (LOG.isDebugEnabled() && !component.isShowing()) {
                ...
                if (parent != null) {
                    message += " on visible  " + parent.getClass().getName();
                }
                LOG.debug(message);
            }
            return location;
        ...
        private boolean processMousePressed(MouseEvent e) {
            ...
            if (moveCaret) {
                if (e.isShiftDown() && !e.isControlDown() && !e.isAltDown() && !e.isMetaDown()) {
                    ...
                    && !isCreateRectangularSelectionEvent(e) && e.getClickCount() == 1) {
                    if (!toggleCaret) {
                        setMouseSelectionState(MOUSE_SELECTION_STATE_NONE);
    ...
    private static boolean isMouseActionEvent(MouseEvent e, String actionId) {
        ...
        if (mappedActions.length < 2 || e.getID() == MouseEvent.MOUSE_DRAGGED /* 'normal' actions are not invoked on mouse drag */) {
            return true;
        }
        ...
        public void executePaint(Component component, Graphics2D g) {
            ...
            if (currentTimeMillis - lastRepaintTime < TIME_PER_FRAME) {
                ...
                application
                    .getLastUIAccess()
                    .getScheduler()
                    .schedule(() -> component.repaint(x, y, myWidth, myHeight), TIME_PER_FRAME, TimeUnit.MILLISECONDS);
                return;
            }
            ...
            if (frameIndex == TOTAL_FRAMES) {
                nrp.set(false);
                application.invokeLater(() -> Disposer.dispose(myPainterListenersDisposable));
    ...
    public void codeStyleSettingsChanged(CodeStyleSettingsChangeEvent event) {
        if (myProject != null) {
            if (event.getPsiFile() != null) {
                ...
                if (editorFile != event.getPsiFile()) {
                    return;
                }
            }
    ...
    private class MyScrollPane extends JBScrollPane {
        ...
        public void setUI(ScrollPaneUI ui) {
            super.setUI(ui);
            // disable standard Swing key-bindings
