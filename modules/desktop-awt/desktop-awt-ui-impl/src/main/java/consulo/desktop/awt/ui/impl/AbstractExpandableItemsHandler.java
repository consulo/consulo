...
public abstract class AbstractExpandableItemsHandler<KeyType, ComponentType extends JComponent> implements ExpandableItemsHandler<KeyType> {
    ...
    public static void setRelativeBounds(Component parent, Rectangle bounds,
                                         Component child, Container validationParent) {
    ...
    private boolean isHandleSelectionEnabled(KeyType selected, boolean processIfUnfocused) {
        return myEnabled &&
    ...
    private static boolean isHintsAllowed(Window window) {
        if (window instanceof RootPaneContainer rootPaneContainer) {
            ...
            if (pane != null) {
                return Boolean.TRUE.equals(pane.getClientProperty(JBPopup.SHOW_HINTS));
            }
        }
        return false;
    }
