...
import consulo.desktop.awt.ui.impl.tabs.singleRow.SingleRowLayout;
import consulo.desktop.awt.ui.impl.tabs.singleRow.SingleRowPassInfo;
import consulo.desktop.awt.ui.impl.tabs.table.TableLayout;
...
public abstract class JBTabsImpl extends JComponent
            ...
            new LazyUiDisposable<JBTabsImpl>(parent, this, this) {
                ...
                protected void initialize(Disposable parent, JBTabsImpl child, @Nullable Project project) {
                    ...
                    UIUtil.addAwtListener(event -> {
                        if (mySingleRowLayout.myMorePopup != null) {
                            return;
                        }
                        processFocusChange();
    ...
    public CompletableFuture<?> updateTabActionsAsync(UIAccess uiAccess, boolean validateNow) {
        ...
        for (TabInfo eachInfo : myInfo2Label.keySet()) {
            changedFutures.add(updateTabAsync(myInfo2Label.get(eachInfo).updateTabActions(uiAccess), eachInfo, changedRef));
        }

        CompletableFuture<Void> res = CompletableFuture.allOf(changedFutures.toArray(CompletableFuture[]::new));
        if (validateNow) {
            res.whenCompleteAsync((unused, throwable) -> {
                if (changedRef.get()) {
                    validate();
    ...
    private void updateAll(boolean forcedRelayout, boolean now) {
        ...
        removeDeferred();
        updateListeners();
        updateTabActionsAsync(UIAccess.current(), false);
        updateEnabling();
    }
    ...
    private boolean isMyChildIsFocusedNow() {
        ...
        if (mySelectedInfo != null) {
            if (!SwingUtilities.isDescendingFrom(owner, mySelectedInfo.getComponent())) {
                return false;
            }
        }
    ...
    private void updateTab(Supplier<Boolean> update, TabInfo info) {
        TabLabel label = myInfo2Label.get(info);
        Boolean changes = update.get();
    ...
    private @Nullable TabInfo getToSelectOnRemoveOf(TabInfo info) {
        ...
        if (mySelectedInfo != info) {
            return null;
        }

        if (myVisibleInfos.size() == 1) {
            return null;
        }
...
//todo [kirillk] dirty hack, should rely on ActionManager to understand that menu item was either chosen on or cancelled
    ...
    private boolean isNavigationVisible() {
        if (myStealthTabMode && getTabCount() == 1) {
            return false;
        }
    ...
    public TabLayout getEffectiveLayout() {
        if (myLayout == myTableLayout && getTabsPosition() == JBTabsPosition.top) {
    ...
    public JBTabsPresentation setTabLabelActionsMouseDeadzone(TimedDeadzone.Length length) {
        ...
        for (TabInfo each : all) {
            TabLabel eachLabel = myInfo2Label.get(each);
            eachLabel.updateTabActions(UIAccess.current());
        }
        return this;
    }
