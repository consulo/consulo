...
public class mxGraphHandler extends mxMouseAdapter implements DropTargetListener {
  ...
  public void updateDragImage(Object[] cells) {
    ...
    if (cells != null && cells.length > 0) {
      Image img = mxCellRenderer.createBufferedImage(graphComponent.getGraph(), cells, graphComponent.getGraph().getView().getScale(), null,
  ...
  public void mouseDragged(MouseEvent e) {
    ...
    if (!e.isConsumed()) {
      ...
      if (constrainedEvent && first != null) {
        ...
        e = new MouseEvent(e.getComponent(), e.getID(), e.getWhen(), e.getModifiers(), x, y, e.getClickCount(), e.isPopupTrigger(),
                           e.getButton());
      }
    ...
    if (isVisible() && previewBounds != null) {
      if (dragImage != null) {
        ...
        mxSwingConstants.PREVIEW_BORDER
          .paintBorder(graphComponent, g, previewBounds.x, previewBounds.y, previewBounds.width, previewBounds.height);
      }
    }
  }
