...
class WeakKeyIntValueHashMap<K> implements ObjectIntMap<K> {
  ...
  private static class MyReference<T> extends WeakReference<T> {
    ...
    private MyReference(@Nonnull T key, ReferenceQueue<? super T> q) {
      super(key, q);
  ...
  @Override
  public final int get(@Nonnull K key) {
    MyReference<K> ref = new MyReference<K>(key, null);
    return myMap.get(ref);
  }

  @Override
  public final int put(@Nonnull K key, int value) {
    processQueue();
  ...
  @Override
  public final int remove(@Nonnull K key) {
    processQueue();
    MyReference<K> ref = new MyReference<K>(key, myQueue);
