...
final class ConcurrentSoftHashMap<K, V> extends ConcurrentRefHashMap<K, V> {
  private static class SoftKey<K, V> extends SoftReference<K> implements KeyReference<K, V> {
    ...
    private SoftKey(@Nonnull K k, final int hash, @Nonnull TObjectHashingStrategy<K> strategy, V v, @Nonnull ReferenceQueue<K> q) {
      super(k, q);
      myStrategy = strategy;
      value = v;
      myHash = hash;
  ...
  public ConcurrentSoftHashMap(@Nonnull TObjectHashingStrategy<K> hashingStrategy) {
