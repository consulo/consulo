 ...
 * @since 2026-08-16
 */
public class DesktopQtFileChooserDialog implements FileChooserDialog, PathChooserDialog {
    private final FileChooserDescriptor myDescriptor;

    public DesktopQtFileChooserDialog(FileChooserDescriptor descriptor) {
    ...
    public CompletableFuture<VirtualFile[]> chooseAsync(@Nullable ComponentManager project, VirtualFile[] toSelect) {
        ...
        QWidget parent = focusedWindow == null ? null : TargetQt.to(focusedWindow);

        CompletableFuture<VirtualFile[]> result = new CompletableFuture<>();
        UIAccess.current().give(() -> {
            String path = QFileDialog.getExistingDirectory(parent, myDescriptor.getTitle().get());
