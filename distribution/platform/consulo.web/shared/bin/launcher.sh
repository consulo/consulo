...
#
# ---------------------------------------------------------------------
# Consulo Web startup script.
# ---------------------------------------------------------------------
#

message()
{
  echo "ERROR: Cannot start Consulo Web" >&2
  echo "$1" >&2
}
...
if [ -z "$UNAME" -o -z "$GREP" -o -z "$CUT" -o -z "$CAT" -o -z "$TR" ]; then
...
MAIN_CLASS_NAME="consulo.web.boot.main.Main"

VM_OPTIONS_FILE="$ROOT_DIR/consulo64.vmoptions"
...
COMMON_JVM_ARGS="-classpath \"$IDE_HOME/boot/*\" -Dconsulo.home.path=\"$IDE_HOME\" -Didea.home.path=\"$IDE_HOME\" -Didea.properties.file=\"$ROOT_DIR/consulo.properties\" -Dconsulo.properties.file=\"$ROOT_DIR/consulo.properties\""
IDE_JVM_ARGS=""
