      ...
      public GitCommit fun(GitLogRecord record) {
        try {
          ...
          return GitCommit.builder()
            .setHash(Hash.create(record.getHash()))
            .setAuthorName(record.getAuthorName())
            .setAuthorEmail(record.getAuthorEmail())
            .setAuthorTime(record.getAuthorTimeStamp())
            .setCommitterName(record.getCommitterName())
            .setCommitterEmail(record.getCommitterEmail())
            .setCommitTime(record.getLongTimeStamp())
            .setSubject(record.getSubject())
            .setMessage(record.getFullMessage())
            .setParents(parents)
            .setChanges(record.parseChanges(project, root))
            .build();
        }
        catch (VcsException e) {
          LOG.error(e);
