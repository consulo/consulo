...
public class PluginBuildParticipant extends BuildParticipant {
 ...
 /*   Sdk jdk = ConsuloSdkType.findIdeaJdk(ModuleRootManager.getInstance(myModule).getSdk());
    if (jdk != null && ConsuloSdkType.isFromIDEAProject(jdk.getHomePath())) {
      return null;
    }
  ...
  private boolean checkDependencies(CompileContext context) {
    ...
    if (wrongSetDependencies.length != 0) {
      ...
      for (Module dependency : wrongSetDependencies) {
        if (ModuleUtil.getExtension(dependency, PluginModuleExtension.class) == null) {
          realProblems = true;
          ...
          final XmlFile pluginXml = PluginModuleUtil.getPluginXml(dependency);
          ...
          if (!isDeclared) {
            ...
            realProblems = true;
            context.addMessage(CompilerMessageCategory.ERROR,
                               DevKitBundle.message("incorrect.dependency.not-declared", dependency.getName(), myModule.getName()), null,
                               -1, -1);
          }
        }
      }
