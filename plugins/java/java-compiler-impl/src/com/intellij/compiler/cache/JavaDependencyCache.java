...
public class JavaDependencyCache implements DependencyCache {
  ...
  */
  ...
  public void update() throws CacheCorruptedException {
    ...
    final JavaDependencyCacheNavigator navigator = getCacheNavigator();
  ...
  public void syncOutDir(Trinity<File, String, Boolean> trinity) throws CacheCorruptedException{
    ...
    if (className != null) {
      final int id = getSymbolTable().getId(className);
      addTraverseRoot(id);
  ...
  private void buildForwardDependencies(final int classQName, final Collection<ReferenceInfo> references) throws CacheCorruptedException {
    ...
    for (final MethodInfo methodInfo : cache.getMethods(classQName)) {
      ...
      final String returnTypeClassName = JavaMakeUtil.parseObjectType(methodInfo.getReturnTypeDescriptor(symbolTable), 0);
