      ...
      public boolean process(final int subclassQName) throws CacheCorruptedException {
            ...
            myJavaDependencyCache.getCacheNavigator().walkSuperInterfaces(subclassQName, new ClassInfoProcessor() {
              ...
              public boolean process(final int ifaceQName) throws CacheCorruptedException {
                ...
                if (implementee != null) {
                  ...
                  if (myJavaDependencyCache.markClass(subclassQName)) {
                    if (LOG.isDebugEnabled()) {
                      LOG.debug("Mark dependent class " + myJavaDependencyCache.resolve(subclassQName) + "; reason: changed base method, implementing corresponding method inherited from an interface" + oldMethod);
                    }
                  }
                }
                return !found;
        ...
        if (!ClsUtil.isAbstract(oldCache.getFlags(subclassQName))) {
          if (hasUnimplementedAbstractMethods(subclassQName, new HashSet<MemberInfo>(removedConcreteMethods))) {
            if (myJavaDependencyCache.markClass(subclassQName)) {
              if (LOG.isDebugEnabled()) {
                LOG.debug("Mark dependent class " + myJavaDependencyCache.resolve(subclassQName) + "; reason: the class should be declared abstract because abstract method implementation was removed from its superclass: " +
                          myJavaDependencyCache.resolve(myQName));
              }
            }
            return true;
        ...
        if (!removedOverridableMethods.isEmpty() && !myJavaDependencyCache.isClassInfoMarked(subclassQName) && !myJavaDependencyCache.getNewClassesCache().containsClass(subclassQName) /*not compiled in this session*/) {
            ...
            if (!subclassMethod.isConstructor()) {
              for (MethodInfo removedMethod : removedOverridableMethods) {
                if (removedMethod.getName() == subclassMethod.getName() /*todo: check param signatures here for better accuracy*/) {
                  ...
                  if (myJavaDependencyCache.markClass(subclassQName)) {
                    if (LOG.isDebugEnabled()) {
                      LOG.debug("Mark dependent subclass " + myJavaDependencyCache.resolve(subclassQName) + "; reason: the class has methods annotated with @Override and some methods were changed or removed in a base class" +
                                myJavaDependencyCache.resolve(myQName));
                    }
                  }
                  return true;
