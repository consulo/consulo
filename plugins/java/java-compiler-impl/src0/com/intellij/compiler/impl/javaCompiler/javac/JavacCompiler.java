...
public class JavacCompiler extends ExternalCompiler {
  ...
  public Configurable createConfigurable() {
    return new JavacConfigurable(JavacCompilerConfiguration.getInstance(myProject));
  }
        ...
        public String[] compute() {
          try {
            ...
            createStartupCommand(chunk, commandLine, outputPath, JavacCompilerConfiguration
              .getInstance(myProject), context.isAnnotationProcessorsEnabled());
  ...
  private Sdk getJdkForStartupCommand(final ModuleChunk chunk) {
    ...
    if (ApplicationManager.getApplication().isUnitTestMode() && JavacCompilerConfiguration
      .getInstance(myProject).isTestsUseExternalCompiler()) {
